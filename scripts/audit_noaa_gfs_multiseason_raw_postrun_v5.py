#!/usr/bin/env python
"""Offline, read-only V5 postrun audit for the frozen Track A raw producer.
...
V5 is an append-only source fork of the frozen V4 auditor.  The transaction,
raw, decoded and full offline replay audit is unchanged.  V5 replaces a
transport-truncated V4 review with a compact, separately authorized
independent-review/GO chain.  The immutable V4 authorization remains the sole
full recovered-afterstate source; V5 controls carry only a fixed commitment.
"""
...
AUDITOR_TEST = REPO / "tests" / "test_noaa_gfs_multiseason_raw_postrun_v5.py"
...
def audit(
    ...
    *,
    ...
    if contract == PRODUCTION_CONTRACT:
        _require(
            _V5_CANONICAL_MAIN_ACTIVE,
            "V5 production audit is callable only from the canonical guarded main",
        )
    authority: dict[str, Any] | None = None
    v5_afterstate_start: dict[str, Any] | None = None
    v5_afterstate_end: dict[str, Any] | None = None
    if contract == PRODUCTION_CONTRACT or authorization_path is not None or independent_go_path is not None:
        _require(
            ...
            "V5 production audit requires authorization and independent GO",
        )
        authority = _v5_validate_predata_authority(
            root, authorization_path.resolve(), independent_go_path.resolve()
        )
        v5_afterstate_start = _v5_validate_postauthority_afterstate(root, authority)
    ...
    if authority is not None:
        v5_afterstate_end = _v5_validate_postauthority_afterstate(root, authority)
        _require(
            v5_afterstate_end == v5_afterstate_start,
            "V5 recovered afterstate commitment changed during full audit",
        )
    return {
        "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V5",
        "schema_version": 5,
        "status": "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V5",
        ...
        "provenance": {
            ...
            "executing_v5_auditor": (
                authority["v5_source_identities"]["v5_auditor"]
                if authority is not None
                else identity(Path(__file__).resolve())
            ),
            "executing_v5_auditor_test": (
                authority["v5_source_identities"]["v5_auditor_test"]
                if authority is not None
                else identity(AUDITOR_TEST)
            ),
            "executing_v5_sealer": (
                authority["v5_source_identities"]["v5_sealer"]
                if authority is not None else None
            ),
            "executing_v5_sealer_test": (
                authority["v5_source_identities"]["v5_sealer_test"]
                if authority is not None else None
            ),
            ...
            "frozen_v4_source_baseline": (
                authority["frozen_v4_source_identities"]
                if authority is not None else FROZEN_V4_SOURCE_IDENTITIES
            ),
            "v5_audit_authorization": (
            ...
            "v4_authorization_base": (
                authority["v4_base"]["base"] if authority is not None else None
            ),
            "v4_review_transport_truncation_incident": (
                authority["incident_chain"]["incident_identity"]
                if authority is not None else None
            ...
            "v4_review_transport_truncation_state": (
                authority["rejected_v4_review"]["state"]
                if authority is not None else None
            ),
            "recovered_afterstate_commitment": (
        ...
        "audit_models_fit": 0,
        "audit_files_written": 0,
        "v5_authorized_afterstate_recheck": {
            "start": v5_afterstate_start,
            "end": v5_afterstate_end,
        },
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    global _V5_CANONICAL_MAIN_ACTIVE
    args = parse_args(argv)
    root = args.root.resolve()
    try:
        _require(
            args.authorization is not None and args.independent_go is not None,
            "V5 canonical command requires explicit --authorization and --independent-go",
        )
        authorization_path = args.authorization.resolve()
        independent_go_path = args.independent_go.resolve()
        _v5_require_canonical_runtime_entrypoint(
            root, authorization_path, independent_go_path
        )
        _V5_CANONICAL_MAIN_ACTIVE = True
        report = audit(
            root,
            ...
            json.dumps(
                {
                    "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V5",
                    "schema_version": 5,
                    "status": "CONDITIONAL_SKIP_V5_POSTRUN_AUDIT_ACTIVE",
                    "reason": str(exc),
            ...
            json.dumps(
                {
                    "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V5",
                    "schema_version": 5,
                    "status": "FAIL_PRODUCER_POSTRUN_AUDIT_V5",
                    "reason": str(exc),
                    "audit_network_requests": 0,
                    "audit_files_written": 0,
                },
