#!/usr/bin/env python
"""Offline, read-only V6 postrun audit for the frozen Track A raw producer.
...
V6 is an append-only source fork of the frozen V5 auditor.  The transaction,
raw, decoded and full offline replay audit remains unchanged.  V6 fixes only
the V5 conflation of immutable V1 historical records with active V2 paths and
moves that exact historical check ahead of every Parquet/data/replay route.
"""
...
AUDITOR_TEST = REPO / "tests" / "test_noaa_gfs_multiseason_raw_postrun_v6.py"
...
def audit(
    ...
    *,
    ...
    if contract == PRODUCTION_CONTRACT:
        _require(
            _V6_CANONICAL_MAIN_ACTIVE,
            "V6 production audit is callable only from the canonical guarded main",
        )
    authority: dict[str, Any] | None = None
    v6_afterstate_start: dict[str, Any] | None = None
    v6_afterstate_end: dict[str, Any] | None = None
    if contract == PRODUCTION_CONTRACT or authorization_path is not None or independent_go_path is not None:
        _require(
            ...
            "V6 production audit requires authorization and independent GO",
        )
        authority = _v6_validate_predata_authority(
            root, authorization_path.resolve(), independent_go_path.resolve()
        )
        v6_afterstate_start = _v6_validate_postauthority_afterstate(root, authority)
    ...
    if authority is not None:
        v6_afterstate_end = _v6_validate_postauthority_afterstate(root, authority)
        _require(
            v6_afterstate_end == v6_afterstate_start,
            "V6 recovered afterstate commitment changed during full audit",
        )
    return {
        "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V6",
        "schema_version": 6,
        "status": "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V6",
        ...
        "provenance": {
            ...
            "executing_v6_auditor": (
                authority["v6_source_identities"]["v6_auditor"]
                if authority is not None
                else identity(Path(__file__).resolve())
            ),
            "executing_v6_auditor_test": (
                authority["v6_source_identities"]["v6_auditor_test"]
                if authority is not None
                else identity(AUDITOR_TEST)
            ),
            "executing_v6_sealer": (
                authority["v6_source_identities"]["v6_sealer"]
                if authority is not None else None
            ),
            "executing_v6_sealer_test": (
                authority["v6_source_identities"]["v6_sealer_test"]
                if authority is not None else None
            ),
            ...
            "frozen_v5_source_baseline": (
                authority["frozen_v5_source_identities"]
                if authority is not None else FROZEN_V5_SOURCE_IDENTITIES
            ),
            "v6_audit_authorization": (
                authority["authorization_identity"] if authority is not None else None
            ),
            "v6_independent_review": (
        ...
        "audit_models_fit": 0,
        "audit_files_written": 0,
        "v6_authorized_afterstate_recheck": {
            "start": v6_afterstate_start,
            "end": v6_afterstate_end,
        },
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    global _V6_CANONICAL_MAIN_ACTIVE
    args = parse_args(argv)
    root = args.root.resolve()
    try:
        _require(
            args.authorization is not None and args.independent_go is not None,
            "V6 canonical command requires explicit --authorization and --independent-go",
        )
        ...
        _V6_CANONICAL_MAIN_ACTIVE = True
        report = audit(
            root,
            ...
            json.dumps(
                {
                    "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V6",
                    "schema_version": 6,
                    "status": "CONDITIONAL_SKIP_V6_POSTRUN_AUDIT_ACTIVE",
                    "reason": str(exc),
            ...
            json.dumps(
                {
                    "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V6",
                    "schema_version": 6,
                    "status": "FAIL_PRODUCER_POSTRUN_AUDIT_V6",
                    "reason": str(exc),
                    "audit_network_requests": 0,
                    "audit_files_written": 0,
                },
