#!/usr/bin/env python
"""Offline, read-only V3 postrun audit for the frozen Track A raw producer.
...
V3 is an append-only source fork of the frozen V2 auditor.  The data audit is
unchanged.  V3 adds a separately authorized, independent-review/GO gate before
any Parquet or GRIB access and fixes exactly one immutable provenance identity:
``independent_prelaunch_audit`` has the fourth, exact field
``status=PASS_TO_CREATE_FINAL_AUTH_ONLY``.  The recovery outputs continue to
bind the superseded V2 auditor/test that were planned before recovery; the V3
authorization separately binds this executing V3 auditor/test.
"""
...
AUDITOR_TEST = REPO / "tests" / "test_noaa_gfs_multiseason_raw_postrun_v3.py"
...
V3_AUTH_RELATIVE = "prereg/decode_recovery_postrun_audit_authorization_v3.json"
V3_REVIEW_RELATIVE = (
...
V3_SEALER = REPO / "scripts" / "seal_noaa_gfs_multiseason_postrun_audit_v3.py"
V3_SEALER_TEST = REPO / "tests" / "test_seal_noaa_gfs_multiseason_postrun_audit_v3.py"
V3_FALSE_REJECT_INCIDENT_RELATIVE = (
    "incidents/"
    "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_V2_IDENTITY_SCHEMA_FALSE_REJECT_V1.json"
)
V3_FALSE_REJECT_INCIDENT_SIZE_BYTES = 17_309
V3_FALSE_REJECT_INCIDENT_SHA256 = (
    "4c8133de1dba5e7677f1dab4b001c694a3f655dad415ca2a62ddefb6249d5c7a"
)
V3_RECOVERY_ATTEMPT_ID = "decode_recovery_v2__20260810T211017500046Z"
V3_RUNTIME_IDENTITY_SHA256 = (
    "a81cdbeafa3906e4d1bbc02455b156c58d226323a90b7f8911017283e3135d4b"
)
V3_PROGRESS_INVENTORY_CANONICAL_SHA256 = (
    "49e1f24641657aa1aa7ceb014a0ef1d5e3d743b64887cc4c1237824fe420c2b6"
)
V3_PROGRESS_INVENTORY_CANONICAL_BYTES = 21_217
...
V3_TRANSACTION_IDENTITIES = {
    "plan": {
        "path": f"raw/output_transactions/{V3_RECOVERY_ATTEMPT_ID}__plan.json",
        "size_bytes": 7_247,
        "sha256": "0a093971f5b263b0fef9d37fdb4eaf0bb5116f00fe531ad8ec1f51c47f2ce2c7",
    },
    "commit": {
        "path": f"raw/output_transactions/{V3_RECOVERY_ATTEMPT_ID}__committed.json",
        "size_bytes": 1_997,
        "sha256": "cf091132ff812812d3095c2b51da41901e5ca3bc8f43b899721ea699fa781bcf",
    },
}
V3_COMPLETION_LOCK_IDENTITIES = {
    "decode_mutex_complete": {
        "path": f"decoded/recovery_history/{V3_RECOVERY_ATTEMPT_ID}__decode_mutex__complete.lock",
        "size_bytes": 206,
        "sha256": "0a121a5464a0437d7f7efcf972653f8610424858d96917b10410060c4409a6a2",
    },
    "raw_mutex_complete": {
        "path": f"decoded/recovery_history/{V3_RECOVERY_ATTEMPT_ID}__raw_mutex__complete.lock",
        "size_bytes": 206,
        "sha256": "0a121a5464a0437d7f7efcf972653f8610424858d96917b10410060c4409a6a2",
    },
}
V3_POSTCOMMIT_INPUT_AUDIT_IDENTITY = {
    "path": f"decoded/recovery_history/{V3_RECOVERY_ATTEMPT_ID}__postcommit_input_audit.json",
    "size_bytes": 1_252,
    "sha256": "559b14d8effcede9259d2d0e9d520aee60fe0fa4ff810d0cdf39941bbc15332a",
}
V3_FINAL_PROGRESS_IDENTITY = {
    "path": f"decoded/recovery_progress/decoded_messages__{V3_RECOVERY_ATTEMPT_ID}__010368.json",
    "size_bytes": 221,
    "sha256": "d0d228ab900858919047c80c446454f627d23132f7037f3f557cc13cb3c2843f",
}
V3_CANONICAL_OUTPUT_IDENTITIES = {
...
V3_REQUIRED_TEST_NAMES = (
    ...
    "test_v3_auditor_is_stdout_only_and_writes_zero_files",
    "test_network_target_label_model_and_submission_routes_remain_absent",
    "test_v3_worker_is_importable_in_exactly_seven_spawn_processes",
    "test_v3_authority_and_go_precede_any_parquet_or_data_read",
)
V3_TEST_RUN_KEYS = {
    "v3_auditor_tests", "v3_sealer_tests", "frozen_v2_auditor_tests",
}
V3_TEST_RUN_RECORD_KEYS = {
    "command", "exit_code", "summary", "stdout_sha256", "stderr_sha256",
    "network_guard_installed", "cacheprovider_disabled",
}
V3_REVIEW_TEST_RECHECK_KEYS = {
...
V3_REVIEW_CHECKS = {
    ...
    "v3_code_test_sealer_identities_exact",
    ...
    "real_seven_spawn_v3_worker_importability_pass",
    "recovered_afterstate_exact_and_closed",
    "full_offline_raw_to_decoded_replay_retained",
    "frozen_v2_auditor_suite_green",
    "executing_v3_vs_frozen_v2_role_distinction_exact",
    "stdout_only_zero_write_contract_retained",
    "network_zero_target_free_no_model_submission_scope_retained",
}
V3_INCIDENT_POSTFAILURE_STATE_CANONICAL_SHA256 = (
    "311642243f2261bf8f50a07ac430f365c2e49a7971f58f478cce1cc026d91ce1"
)
V3_RECOVERED_AFTERSTATE_CANONICAL_SHA256 = (
    "8e434429e6768e14dbfde2c2592609bcd8167e991fa5eaea65b5ba40e7d991cb"
)
V3_RECOVERED_AFTERSTATE_CANONICAL_BYTES = 24_234
_V3_CANONICAL_MAIN_ACTIVE = False
V3_PYTEST_NETWORK_GUARD_SOURCE = """
import socket
import sys

def _deny_network(*_args, **_kwargs):
    raise RuntimeError("network forbidden in immutable V3 postrun-auditor test subprocess")
...
def _deny_network_audit(event, _args):
    if event in {
        ...
        raise RuntimeError("network audit event forbidden in immutable V3 postrun-auditor test subprocess")
...
def v3_spawn_probe_barrier_initializer(barrier: Any) -> None:
...
def _v3_created_utc(value: Any, *, label: str) -> datetime:
    _require(
        isinstance(value, str)
...
def _v3_require_exact_declared_identity(
...
def _v3_verify_exact_root_identity(
    root: Path, record: Any, expected: Mapping[str, Any], *, label: str
) -> dict[str, Any]:
    _v3_require_exact_declared_identity(record, expected, label=label)
...
def _v3_verify_external_identity(
    record: Any,
    expected_path: Path,
    *,
    label: str,
...
def _v3_required_command(root: Path, authorization_path: Path, go_path: Path) -> list[str]:
    return [
        str(Path(sys.executable).resolve()),
        "-B",
        "-m",
        "scripts.audit_noaa_gfs_multiseason_raw_postrun_v3",
        "--root",
        str(root.resolve()),
        "--authorization",
...
def _v3_require_canonical_runtime_entrypoint(
    root: Path, authorization_path: Path, go_path: Path
) -> None:
    _require(
        __spec__ is not None
        and __spec__.name == "scripts.audit_noaa_gfs_multiseason_raw_postrun_v3"
        and __package__ == "scripts",
        "V3 production audit requires the canonical -m module entrypoint",
    )
    _require(
        ...
        "V3 production audit bytecode/cache environment mismatch",
    )
    expected_argv = [
    ...
    _require(actual_argv == expected_argv, "V3 production argv is not exact canonical order")
    actual_orig_argv = [
        str(Path(sys.orig_argv[0]).resolve()), *sys.orig_argv[1:]
    ]
    _require(
        actual_orig_argv
        == _v3_required_command(root, authorization_path, go_path),
        "V3 production interpreter/module argv is not exact authorized command",
    )


def _v3_test_command(relative: str) -> list[str]:
    return [
        str(Path(sys.executable).resolve()),
        "-B",
        "-c",
...
def _v3_control_temporary_files(root: Path) -> list[str]:
    matches: list[str] = []
    for relative in (V3_AUTH_RELATIVE, V3_REVIEW_RELATIVE, V3_GO_RELATIVE):
        destination = path_under(root, relative)
        _require(
            destination.parent.is_dir() and not _linklike(destination.parent),
            f"V3 control parent is absent or link-like: {relative}",
        )
...
def _v3_validate_false_reject_incident(root: Path, record: Any) -> dict[str, Any]:
    expected = {
        "path": V3_FALSE_REJECT_INCIDENT_RELATIVE,
        "size_bytes": V3_FALSE_REJECT_INCIDENT_SIZE_BYTES,
        "sha256": V3_FALSE_REJECT_INCIDENT_SHA256,
    }
    incident_path = path_under(root, V3_FALSE_REJECT_INCIDENT_RELATIVE)
    _v3_verify_exact_root_identity(root, record, expected, label="V3 false-reject incident")
    incident = load_json(incident_path)
    _require(
        set(incident)
        == {
        ...
        "V3 false-reject incident schema mismatch",
    )
    _require(
        ...
        "V3 false-reject incident header mismatch",
    )
    _require(
        ...
        == {
            ...
            "size_bytes": 300_379,
            "sha256": V3_SUPERSEDED_V2_AUDITOR_IDENTITY["sha256"],
        }
        and incident.get("failed_auditor_test")
        == {
            "path": "tests/test_noaa_gfs_multiseason_raw_postrun_v2.py",
            "size_bytes": 133_607,
            "sha256": V3_SUPERSEDED_V2_AUDITOR_TEST_IDENTITY["sha256"],
        },
        "V3 incident frozen V2 auditor/test identities mismatch",
    )
    ...
    _require(
        ...
        "V3 incident frozen support schema mismatch",
    )
    for role in frozen_support:
        expected_path = V3_V2_SUPPORT_PATHS[role].relative_to(REPO).as_posix()
        expected_size, expected_sha = V3_V2_SUPPORT_SIZE_SHA256[role]
        _require(
            frozen_support[role]
            == {"path": expected_path, "size_bytes": expected_size, "sha256": expected_sha},
            f"V3 incident frozen support mismatch: {role}",
        )
    ...
    _require(
        ...
        == V3_ORIGINAL_V1_PROVENANCE["original_independent_prelaunch_audit_v1"]
        ...
        is True,
        "V3 incident root-cause mismatch",
    )
    _require(
        ...
        == {
            "authorization": V3_AUTH_RELATIVE,
            "independent_review": V3_REVIEW_RELATIVE,
            "independent_go": V3_GO_RELATIVE,
        },
        "V3 incident supersession contract mismatch",
    )
    _require(
        ...
        "V3 incident authority boundary mismatch",
    )
    _require(
        canonical_payload_sha256(incident.get("postfailure_state"))
        == V3_INCIDENT_POSTFAILURE_STATE_CANONICAL_SHA256,
        "V3 incident postfailure-state digest mismatch",
    )
    return incident


def _v3_validate_progress_declaration(progress: Any) -> None:
    _require(
        isinstance(progress, Mapping)
        and set(progress)
        == {"file_count", "inventory", "inventory_canonical_sha256", "final_checkpoint"},
        "V3 recovery-progress declaration schema mismatch",
    )
    ...
    _require(
        ...
        "V3 recovery-progress declaration count mismatch",
    )
    paths: list[str] = []
    for record in inventory:
        _require(
            ...
            and str(record["path"]).startswith(
                f"decoded/recovery_progress/decoded_messages__{V3_RECOVERY_ATTEMPT_ID}__"
            )
            and type(record.get("size_bytes")) is int
            and int(record["size_bytes"]) > 0
            and isinstance(record.get("sha256"), str)
            and re.fullmatch(r"[0-9a-f]{64}", str(record["sha256"])) is not None,
            "V3 recovery-progress inventory identity invalid",
        )
        paths.append(str(record["path"]))
    _require(paths == sorted(paths) and len(set(paths)) == 104, "V3 progress order/uniqueness mismatch")
    encoded = _v3_canonical_bytes(inventory)
    _require(
        len(encoded) == V3_PROGRESS_INVENTORY_CANONICAL_BYTES
        and hashlib.sha256(encoded).hexdigest()
        == V3_PROGRESS_INVENTORY_CANONICAL_SHA256
        and progress.get("inventory_canonical_sha256")
        == V3_PROGRESS_INVENTORY_CANONICAL_SHA256
        and progress.get("final_checkpoint") == V3_FINAL_PROGRESS_IDENTITY,
        "V3 recovery-progress canonical digest/final mismatch",
    )


def _v3_recovered_afterstate_from_authorization(
    authorization: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    _require(
        ...
        and transaction == V3_TRANSACTION_IDENTITIES,
        "V3 recovery-transaction declaration mismatch",
    )
    _require(
        isinstance(history, Mapping)
        and set(history) == {"file_count", "completion_locks", "postcommit_input_audit"}
        and history.get("file_count") == 3
        and history.get("completion_locks") == V3_COMPLETION_LOCK_IDENTITIES
        and history.get("postcommit_input_audit") == V3_POSTCOMMIT_INPUT_AUDIT_IDENTITY,
        "V3 recovery-history declaration mismatch",
    )
    _v3_validate_progress_declaration(progress)
    _require(
        isinstance(outputs, Mapping) and dict(outputs) == V3_CANONICAL_OUTPUT_IDENTITIES,
        "V3 canonical-output declaration mismatch",
    )
    afterstate = {
        "attempt_id": V3_RECOVERY_ATTEMPT_ID,
    ...
    encoded = _v3_canonical_bytes(afterstate)
    _require(
        len(encoded) == V3_RECOVERED_AFTERSTATE_CANONICAL_BYTES
        and hashlib.sha256(encoded).hexdigest()
        == V3_RECOVERED_AFTERSTATE_CANONICAL_SHA256,
        "V3 recovered-afterstate canonical closure mismatch",
    )
    return afterstate


def _v3_validate_zero_snapshot(
    ...
    _require(
        ...
        == {
            ...
            "v3_control_temporary_files", "postrun_audit_output_files_present",
            "network_requests", "audit_files_written", "labels_read",
            "arrays_2024_read", "arrays_2025_read", "models_fit",
            "submission_csv_created",
        },
        "V3 preaudit zero-mutation snapshot schema mismatch",
    )
    _require(
        ...
        and snapshot.get("v3_control_temporary_files")
        == {
            "checked_destination_relative_paths": [
                V3_AUTH_RELATIVE, V3_REVIEW_RELATIVE, V3_GO_RELATIVE
            ],
            "matching_temporary_files": 0,
        }
        ...
        "V3 preaudit zero-mutation snapshot values mismatch",
    )


def _v3_validate_test_evidence(
    evidence: Any,
    *,
    ...
    _require(
        isinstance(evidence, Mapping) and set(evidence) == V3_TEST_EVIDENCE_KEYS,
        "V3 immutable test-evidence schema mismatch",
    )
    evidence_created = _v3_created_utc(evidence.get("created_utc"), label="V3 test evidence")
    _require(
        evidence_created <= authorization_created
        and evidence.get("schema_version") == 1
        and evidence.get("artifact_type")
        == "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_V3_TEST_EVIDENCE"
        and evidence.get("status") == "PASS_FROZEN_V3_AUDITOR_AND_SEALER_TESTS"
        and evidence.get("pytest_isolation")
        == "ONE_TEST_FILE_PER_CLEAN_NETWORK_DENIED_SUBPROCESS"
        and evidence.get("required_test_names") == list(V3_REQUIRED_TEST_NAMES)
        and evidence.get("required_test_names_present") is True
        and evidence.get("all_exit_codes_zero") is True,
        "V3 immutable test-evidence header/result mismatch",
    )
    _require(
        evidence.get("bound_identities") == bound_identities,
        "V3 immutable test-evidence bound identities mismatch",
    )
    ...
    _require(
        ...
        == [
            bound_identities["v3_auditor"],
            bound_identities["v3_auditor_test"],
            bound_identities["v3_sealer"],
            bound_identities["v3_sealer_test"],
        ],
        "V3 exact-source compile evidence mismatch",
    )
    runs = evidence.get("test_runs")
    expected_commands = {
        "v3_auditor_tests": _v3_test_command(
            "tests/test_noaa_gfs_multiseason_raw_postrun_v3.py"
        ),
        "v3_sealer_tests": _v3_test_command(
            "tests/test_seal_noaa_gfs_multiseason_postrun_audit_v3.py"
        ),
        "frozen_v2_auditor_tests": _v3_test_command(
            "tests/test_noaa_gfs_multiseason_raw_postrun_v2.py"
        ),
    }
    _require(
        isinstance(runs, Mapping) and set(runs) == V3_TEST_RUN_KEYS,
        "V3 immutable pytest run map mismatch",
    )
    for role, expected_command in expected_commands.items():
        run = runs[role]
        _require(
            isinstance(run, Mapping)
            and set(run) == V3_TEST_RUN_RECORD_KEYS
            ...
            f"V3 immutable pytest run mismatch: {role}",
        )
    _require(
        ...
        "V3 immutable test-evidence safety/regression mismatch",
    )


def _v3_validate_policy(payload: Mapping[str, Any], *, label: str) -> None:
    _require(
...
def _v3_validate_predata_authority(
    root: Path, authorization_path: Path, go_path: Path
) -> dict[str, Any]:
    expected_authorization_path = path_under(root, V3_AUTH_RELATIVE)
    expected_go_path = path_under(root, V3_GO_RELATIVE)
    _require(
        Path(os.path.abspath(authorization_path)) == expected_authorization_path
        and Path(os.path.abspath(go_path)) == expected_go_path,
        "V3 authorization/GO paths are not canonical",
    )
    require_no_symlink_chain(expected_authorization_path, root, label="V3 authorization")
    require_no_symlink_chain(expected_go_path, root, label="V3 independent GO")
    _require(not _linklike(expected_authorization_path), "V3 authorization link forbidden")
    _require(not _linklike(expected_go_path), "V3 independent GO link forbidden")
    authorization = load_json(expected_authorization_path)
    go = load_json(expected_go_path)
    _require(set(authorization) == V3_AUTH_KEYS, "V3 authorization schema mismatch")
    _require(set(go) == V3_GO_KEYS, "V3 independent GO schema mismatch")
    authorization_created = _v3_created_utc(
        authorization.get("created_utc"), label="V3 authorization"
    )
    go_created = _v3_created_utc(go.get("created_utc"), label="V3 independent GO")
    ...
    _require(
        ...
        and re.fullmatch(r"postrun_audit_v3__[0-9]{8}T[0-9]{12}Z", attempt_id)
        is not None
        and authorization.get("recovery_attempt_id") == V3_RECOVERY_ATTEMPT_ID
        ...
        "V3 authorization header/binding mismatch",
    )
    _v3_validate_policy(authorization, label="V3 authorization")
    incident = _v3_validate_false_reject_incident(root, authorization.get("incident"))
    incident_identity = identity(
        path_under(root, V3_FALSE_REJECT_INCIDENT_RELATIVE), root
    )

    _v3_require_exact_declared_identity(
        authorization.get("superseded_v2_auditor"),
        V3_SUPERSEDED_V2_AUDITOR_IDENTITY,
        label="superseded V2 auditor",
    )
    _v3_require_exact_declared_identity(
        authorization.get("superseded_v2_auditor_test"),
        V3_SUPERSEDED_V2_AUDITOR_TEST_IDENTITY,
        label="superseded V2 auditor test",
    )
    _v3_verify_external_identity(
        authorization["superseded_v2_auditor"],
        SUPERSEDED_V2_AUDITOR,
        label="superseded V2 auditor",
        expected_size_sha256=(300_379, V3_SUPERSEDED_V2_AUDITOR_IDENTITY["sha256"]),
    )
    _v3_verify_external_identity(
        authorization["superseded_v2_auditor_test"],
        SUPERSEDED_V2_AUDITOR_TEST,
        label="superseded V2 auditor test",
        expected_size_sha256=(133_607, V3_SUPERSEDED_V2_AUDITOR_TEST_IDENTITY["sha256"]),
    )
    v3_auditor_actual = _v3_verify_external_identity(
        authorization.get("v3_auditor"), Path(__file__).resolve(), label="V3 auditor"
    )
    v3_auditor_test_actual = _v3_verify_external_identity(
        authorization.get("v3_auditor_test"), AUDITOR_TEST, label="V3 auditor test"
    )
    v3_sealer_actual = _v3_verify_external_identity(
        authorization.get("v3_sealer"), V3_SEALER, label="V3 sealer"
    )
    v3_sealer_test_actual = _v3_verify_external_identity(
        authorization.get("v3_sealer_test"), V3_SEALER_TEST, label="V3 sealer test"
    )

    controls = authorization.get("v2_recovery_controls")
    _require(
        isinstance(controls, Mapping)
        and set(controls) == set(V3_V2_CONTROL_IDENTITIES),
        "V3 V2-control binding schema mismatch",
    )
    for role, expected in V3_V2_CONTROL_IDENTITIES.items():
        _v3_verify_exact_root_identity(
            root, controls[role], expected, label=f"V3 bound V2 control {role}"
        )

    support = authorization.get("v2_recovery_support")
    _require(
        isinstance(support, Mapping) and set(support) == set(V3_V2_SUPPORT_PATHS),
        "V3 V2-support binding schema mismatch",
    )
    for role, expected_path in V3_V2_SUPPORT_PATHS.items():
        _v3_verify_external_identity(
            support[role],
            expected_path,
            label=f"V3 bound V2 support {role}",
            expected_size_sha256=V3_V2_SUPPORT_SIZE_SHA256[role],
        )

    original = authorization.get("original_v1_provenance")
    _require(
        isinstance(original, Mapping)
        and set(original) == set(V3_ORIGINAL_V1_PROVENANCE),
        "V3 original V1 provenance schema mismatch",
    )
    raw_authorization_record = original["original_raw_authorization_v1"]
    prelaunch_record = original["original_independent_prelaunch_audit_v1"]
    _v3_verify_exact_root_identity(
        root,
        raw_authorization_record,
        V3_ORIGINAL_V1_PROVENANCE["original_raw_authorization_v1"],
        label="V3 original raw authorization",
    )
    _v3_require_exact_declared_identity(
        prelaunch_record,
        V3_ORIGINAL_V1_PROVENANCE["original_independent_prelaunch_audit_v1"],
        label="V3 original independent prelaunch audit",
    )
    verify_identity(
        prelaunch_record,
    ...
    _require(
        ...
        "V3 original independent prelaunch status mismatch",
    )
    raw_authorization = load_json(
        path_under(root, V3_ORIGINAL_V1_PROVENANCE["original_raw_authorization_v1"]["path"])
    )
    _require(
        raw_authorization.get("independent_prelaunch_audit") == prelaunch_record,
        "V3 original raw authorization four-key prelaunch binding mismatch",
    )
    ...
    _require(
        ...
        "V3 original runtime identity schema mismatch",
    )
    for package, record in runtime_identity["packages"].items():
        _require(
            isinstance(record, Mapping)
            and set(record)
            == {"module_file", "module_file_sha256", "module_file_size_bytes", "version"},
            f"V3 original runtime package schema mismatch: {package}",
        )
    runtime_bytes = _v3_canonical_bytes(runtime_identity)
    _require(
        len(runtime_bytes) == 1_405
        and hashlib.sha256(runtime_bytes).hexdigest() == V3_RUNTIME_IDENTITY_SHA256
        and raw_authorization.get("runtime_identity_sha256")
        == V3_RUNTIME_IDENTITY_SHA256,
        "V3 original runtime identity canonical digest mismatch",
    )

    afterstate = _v3_recovered_afterstate_from_authorization(authorization)
    _v3_validate_zero_snapshot(
        authorization.get("preaudit_zero_mutation_snapshot"),
        incident_postfailure_sha256=V3_INCIDENT_POSTFAILURE_STATE_CANONICAL_SHA256,
    )
    _require(
        authorization.get("required_command")
        == _v3_required_command(root, expected_authorization_path, expected_go_path),
        "V3 authorization required command mismatch",
    )

    _require(
        isinstance(authorization.get("test_evidence"), Mapping),
        "V3 immutable test evidence is not an object",
    )
    bound_identities = {
        "v2_auditor": authorization["superseded_v2_auditor"],
        "v2_auditor_test": authorization["superseded_v2_auditor_test"],
        "v3_auditor": v3_auditor_actual,
        "v3_auditor_test": v3_auditor_test_actual,
        "v3_sealer": v3_sealer_actual,
        "v3_sealer_test": v3_sealer_test_actual,
    }
    _v3_validate_test_evidence(
        authorization.get("test_evidence"),
        authorization_created=authorization_created,
        bound_identities=bound_identities,
    )

    _require(go.get("schema_version") == 3, "V3 independent GO version mismatch")
    _require(
        go.get("artifact_type") == "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_GO_V3"
        and go.get("status") == "GO_V3_READ_ONLY_NETWORK_ZERO_FULL_OFFLINE_REPLAY"
        and go.get("audit_attempt_id") == attempt_id
        and go.get("recovery_attempt_id") == V3_RECOVERY_ATTEMPT_ID
        and go.get("recovery_rerun_authorized") is False
        and go.get("required_command") == authorization.get("required_command"),
        "V3 independent GO header/binding mismatch",
    )
    _v3_validate_policy(go, label="V3 independent GO")
    _require(go_created >= authorization_created, "V3 GO predates authorization")
    expected_review_path = path_under(root, V3_REVIEW_RELATIVE)
    _v3_require_exact_declared_identity(go.get("incident"), incident_identity, label="V3 GO incident")
    _require(
        isinstance(go.get("independent_review"), Mapping)
        and go["independent_review"].get("path") == V3_REVIEW_RELATIVE,
        "V3 GO review path mismatch",
    )
    review_actual = verify_identity(
        go["independent_review"], expected_review_path, root=root, label="V3 independent review"
    )
    review = load_json(expected_review_path)
    _require(set(review) == V3_REVIEW_KEYS, "V3 independent review schema mismatch")
    review_created = _v3_created_utc(review.get("created_utc"), label="V3 independent review")
    _require(
        ...
        "V3 independent review header/verdict mismatch",
    )
    _v3_validate_policy(review, label="V3 independent review")
    ...
    _require(
        ...
        and review.get("incident") == incident_identity
        and go.get("incident") == incident_identity
        and review.get("superseded_v2_auditor")
        == authorization["superseded_v2_auditor"]
        and review.get("superseded_v2_auditor_test")
        == authorization["superseded_v2_auditor_test"]
        and go.get("superseded_v2_auditor")
        == authorization["superseded_v2_auditor"]
        and go.get("superseded_v2_auditor_test")
        == authorization["superseded_v2_auditor_test"]
        and review.get("v3_auditor") == v3_auditor_actual
        and review.get("v3_auditor_test") == v3_auditor_test_actual
        and review.get("v3_sealer") == v3_sealer_actual
        and review.get("v3_sealer_test") == v3_sealer_test_actual
        and go.get("v3_auditor") == v3_auditor_actual
        and go.get("v3_auditor_test") == v3_auditor_test_actual
        and go.get("v3_sealer") == v3_sealer_actual
        and go.get("v3_sealer_test") == v3_sealer_test_actual
        ...
        "V3 review/GO cross-binding mismatch",
    )
    checks = review.get("independent_checks")
    recheck = review.get("test_evidence_recheck")
    _require(
        isinstance(checks, Mapping)
        and set(checks) == V3_REVIEW_CHECKS
        and all(checks.get(key) is True for key in V3_REVIEW_CHECKS),
        "V3 independent review check set/verdict mismatch",
    )
    _require(
        isinstance(recheck, Mapping)
        and set(recheck) == V3_REVIEW_TEST_RECHECK_KEYS
        ...
        and all(
            ...
            for key in V3_REVIEW_TEST_RECHECK_KEYS
            if key != "authorization_test_evidence_canonical_sha256"
        ),
        "V3 independent test-evidence recheck mismatch",
    )
    _require(_v3_control_temporary_files(root) == [], "V3 control temporary files present")
    return {
        "authorization": authorization,
        "authorization_identity": authorization_actual,
        "review": review,
        ...
        "v3_auditor_identity": v3_auditor_actual,
        "v3_auditor_test_identity": v3_auditor_test_actual,
    }


def _v3_validate_postauthority_afterstate(
    root: Path, authority: Mapping[str, Any]
) -> dict[str, Any]:
    authorization = authority["authorization"]
    afterstate = authority["recovered_afterstate"]
    _require(
        not path_under(root, "raw/RAW_LAUNCH_ACTIVE.lock").exists()
        and not path_under(root, "decoded/DECODE_RECOVERY_ACTIVE.lock").exists(),
        "V3 recovered afterstate has an active lock",
    )
    for role, record in authorization["canonical_outputs"].items():
        verify_identity(
            ...
            path_under(root, str(record["path"])),
            root=root,
            label=f"V3 canonical output {role}",
        )
    for role, record in authorization["recovery_transaction"].items():
        verify_identity(
            ...
            path_under(root, str(record["path"])),
            root=root,
            label=f"V3 recovery transaction {role}",
        )
    ...
    _require(
        ...
        == expected_history_paths,
        "V3 recovery-history exact filesystem closure mismatch",
    )

    transaction_root = path_under(root, "raw/output_transactions")
    _require(
        transaction_root.is_dir() and not _linklike(transaction_root),
        "V3 output-transaction root invalid",
    )
    recovery_authorization_v2 = load_json(
        path_under(root, V3_V2_CONTROL_RELATIVES["recovery_authorization_v2"])
    )
    remnants = recovery_authorization_v2.get("documented_preplan_remnants")
    _require(
        isinstance(remnants, list) and len(remnants) == 2,
        "V3 incident-bound preplan-remnant declaration mismatch",
    )
    ...
    for index, record in enumerate(remnants):
        _require(
            isinstance(record, Mapping)
            and set(record) == {"path", "size_bytes", "sha256"},
            f"V3 preplan remnant identity invalid: {index}",
        )
        verify_identity(
            record,
            path_under(root, str(record["path"])),
            root=root,
            label=f"V3 incident-bound preplan remnant {index}",
        )
    ...
    expected_transaction_dirs = {
        ...
        f"raw/output_transactions/{V3_RECOVERY_ATTEMPT_ID}",
        f"raw/output_transactions/{V3_RECOVERY_ATTEMPT_ID}/staged",
        f"raw/output_transactions/{V3_RECOVERY_ATTEMPT_ID}/staged/decoded",
        f"raw/output_transactions/{V3_RECOVERY_ATTEMPT_ID}/staged/raw",
    }
    actual_transaction_files: set[str] = set()
    actual_transaction_dirs: set[str] = set()
    for path in transaction_root.rglob("*"):
        _require(not _linklike(path), "V3 transaction tree contains a link/junction")
        ...
        if path.is_file():
            ...
            raise AuditFailure("V3 transaction tree contains a special object")
    _require(
        ...
        "V3 transaction recursive exact filesystem closure mismatch",
    )
    progress_dir = path_under(root, "decoded/recovery_progress")
    _require(progress_dir.is_dir() and not _linklike(progress_dir), "V3 progress directory invalid")
    ...
    _require(
        ...
        "V3 recovery progress filesystem closure mismatch",
    )
    ...
    _require(
        ...
        "V3 recovery progress inventory rehash mismatch",
    )
    encoded = _v3_canonical_bytes(actual_inventory)
    _require(
        len(encoded) == V3_PROGRESS_INVENTORY_CANONICAL_BYTES
        and hashlib.sha256(encoded).hexdigest()
        == V3_PROGRESS_INVENTORY_CANONICAL_SHA256,
        "V3 recovery progress actual canonical digest mismatch",
    )
    _require(
        canonical_payload_sha256(afterstate)
        == V3_RECOVERED_AFTERSTATE_CANONICAL_SHA256
        and _v3_control_temporary_files(root) == [],
        "V3 afterstate/temp recheck mismatch",
    )
    return {
        "recovered_afterstate": afterstate,
        "recovered_afterstate_canonical_sha256": V3_RECOVERED_AFTERSTATE_CANONICAL_SHA256,
        "canonical_output_count": 8,
        ...
        "v3_control_temporary_files_present": 0,
    }
...
def audit(
    ...
    *,
    ...
    if contract == PRODUCTION_CONTRACT:
        _require(
            _V3_CANONICAL_MAIN_ACTIVE,
            "V3 production audit is callable only from the canonical guarded main",
        )
    authority: dict[str, Any] | None = None
    v3_afterstate_start: dict[str, Any] | None = None
    v3_afterstate_end: dict[str, Any] | None = None
    if contract == PRODUCTION_CONTRACT or authorization_path is not None or independent_go_path is not None:
        _require(
            ...
            "V3 production audit requires authorization and independent GO",
        )
        authority = _v3_validate_predata_authority(
            root, authorization_path.resolve(), independent_go_path.resolve()
        )
        v3_afterstate_start = _v3_validate_postauthority_afterstate(root, authority)
    ...
    if authority is not None:
        v3_afterstate_end = _v3_validate_postauthority_afterstate(root, authority)
        _require(
            v3_afterstate_end == v3_afterstate_start,
            "V3 recovered afterstate changed during full audit",
        )
    return {
        "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V3",
        "schema_version": 3,
        "status": "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V3",
        ...
        "provenance": {
            ...
            "executing_v3_auditor": (
                authority["v3_auditor_identity"]
                if authority is not None
                else identity(Path(__file__).resolve())
            ),
            "executing_v3_auditor_test": (
                authority["v3_auditor_test_identity"]
                if authority is not None
                else identity(AUDITOR_TEST)
            ),
            "frozen_v2_auditor_baseline": V3_SUPERSEDED_V2_AUDITOR_IDENTITY,
            "frozen_v2_auditor_test_baseline": V3_SUPERSEDED_V2_AUDITOR_TEST_IDENTITY,
            "v3_audit_authorization": (
        ...
        "audit_models_fit": 0,
        "audit_files_written": 0,
        "v3_authorized_afterstate_recheck": {
            "start": v3_afterstate_start,
            "end": v3_afterstate_end,
        },
    }
...
def main(argv: Sequence[str] | None = None) -> int:
    global _V3_CANONICAL_MAIN_ACTIVE
    args = parse_args(argv)
    root = args.root.resolve()
    try:
        _require(
            args.authorization is not None and args.independent_go is not None,
            "V3 canonical command requires explicit --authorization and --independent-go",
        )
        authorization_path = args.authorization.resolve()
        independent_go_path = args.independent_go.resolve()
        _v3_require_canonical_runtime_entrypoint(
            root, authorization_path, independent_go_path
        )
        _V3_CANONICAL_MAIN_ACTIVE = True
        report = audit(
            root,
            ...
            json.dumps(
                {
                    "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V3",
                    "schema_version": 3,
                    "status": "CONDITIONAL_SKIP_V3_POSTRUN_AUDIT_ACTIVE",
                    "reason": str(exc),
            ...
            json.dumps(
                {
                    "artifact_type": "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V3",
                    "schema_version": 3,
                    "status": "FAIL_PRODUCER_POSTRUN_AUDIT_V3",
                    "reason": str(exc),
                    "audit_network_requests": 0,
                    "audit_files_written": 0,
                },
