...
CANONICAL_MODULE_ENTRYPOINT = (
    "scripts.seal_noaa_gfs_multiseason_decode_recovery_v2"
)
RAW_LOCK_RELATIVE = "prelaunch/DECODE_RECOVERY_RAW_CACHE_LOCK_V2.json"
PREFLIGHT_RELATIVE = "prelaunch/DECODE_RECOVERY_REAL_PROCESS_PREFLIGHT_V2.json"
AUTHORIZATION_RELATIVE = "prereg/decode_recovery_authorization_v2.json"
REVIEW_RELATIVE = (
    "independent_redteam/TRACK_A_DECODE_RECOVERY_INDEPENDENT_REVIEW_V2.json"
)
GO_RELATIVE = "independent_redteam/TRACK_A_DECODE_RECOVERY_GO_V2.json"
RECOVERY_RUNNER_SIZE_BYTES = 153_970
RECOVERY_RUNNER_SHA256 = (
    "892b9e0fb26cb0f9be19c77bcaa04c656e24d2404f42b2102596914e3d1c31d1"
)
...
def load_recovery_runner_exact(workspace_root: Path) -> Any:
    global recovery
    path = workspace_root / "scripts" / "run_noaa_gfs_multiseason_decode_recovery_v2.py"
    ...
    module_name = "scripts.run_noaa_gfs_multiseason_decode_recovery_v2"
...
def require_control_temp_siblings_absent(root: Path) -> dict[str, Any]:
    """Close temp-file residue for every immutable V1 and future V2 control."""

    checked: list[str] = []
    for relative in (*V1_CONTROL_RELATIVES, *V2_CONTROL_RELATIVES):
        destination = recovery.strict_under_root(root, relative)
        ...
        if (
            ...
            raise RecoverySealError(f"control parent absent/symlink: {parent}")
        prefix = f"{destination.name}.tmp.".casefold()
        for sibling in parent.iterdir():
            if sibling.name.casefold().startswith(prefix):
                raise RecoverySealError(
                    f"unexpected control publisher temporary sibling: {sibling}"
                )
        checked.append(relative)
    return {
        "checked_control_destinations": checked,
        "temporary_siblings": [],
    }


def validate_superseded_v1_chain_and_support(
    root: Path, workspace_root: Path
) -> tuple[dict[str, dict[str, Any]], dict[str, dict[str, Any]]]:
    """Rehash the authoritative V1 chain/support without selecting it to run."""

    chain = {
        role: dict(record) for role, record in recovery.V1_CHAIN_IDENTITIES.items()
    }
    if chain.get("recovery_runner") == recovery.REJECTED_TRANSIENT_V1_RUNNER_IDENTITY:
        raise RecoverySealError("rejected transient V1 runner selected as authority")
    for role, record in chain.items():
        if role == "recovery_runner":
            observed = external_identity(workspace_root / str(record["path"]))
            if (
                observed["size_bytes"] != record["size_bytes"]
                or observed["sha256"] != record["sha256"]
            ):
                raise RecoverySealError("authoritative V1 runner identity mismatch")
        else:
            recovery.require_root_identity(
                record, root, str(record["path"]), f"superseded V1 {role}"
            )

    support: dict[str, dict[str, Any]] = {}
    for role, expected in recovery.V1_SUPPORT_IDENTITIES.items():
        observed = external_identity(workspace_root / str(expected["path"]))
        if (
            observed["size_bytes"] != expected["size_bytes"]
            or observed["sha256"] != expected["sha256"]
        ):
            raise RecoverySealError(f"superseded V1 support {role} mismatch")
        support[role] = observed
    if support["recovery_runner"]["sha256"] == recovery.REJECTED_TRANSIENT_V1_RUNNER_IDENTITY["sha256"]:
        raise RecoverySealError("rejected transient V1 runner present in support")
    return chain, support


def validate_rejected_transient_wrapper(rejected: Any) -> None:
    if (
        ...
        raise RecoverySealError("identity correction rejected transient mismatch")


def validate_launch_incident_authority(
    root: Path, fixed: Mapping[str, Mapping[str, Any]]
) -> None:
    """Bind the flawed seal as history and the append-only correction as authority."""

    flawed_record = fixed["flawed_launch_incident"]
    correction_record = fixed["launch_identity_correction"]
    if flawed_record["sha256"] != recovery.LAUNCH_SUPERSESSION_INCIDENT_SHA256:
        raise RecoverySealError("flawed launch incident identity mismatch")
    if correction_record["sha256"] != recovery.LAUNCH_IDENTITY_CORRECTION_SHA256:
        raise RecoverySealError("launch identity correction identity mismatch")
    flawed = recovery.read_json_object(
        root / recovery.FIXED_ROOT_INPUTS["flawed_launch_incident"],
        "flawed V1 launch incident",
    )
    correction = recovery.read_json_object(
        root / recovery.FIXED_ROOT_INPUTS["launch_identity_correction"],
        "V1 launch identity correction",
    )
    recovery.validate_launch_supersession_payload(flawed)
    recovery.validate_launch_identity_correction_payload(correction)
    if correction["superseded_flawed_incident"] != flawed_record:
        raise RecoverySealError("identity correction does not bind flawed incident")
    if correction["authoritative_failed_v1_chain"] != recovery.V1_CHAIN_IDENTITIES:
        raise RecoverySealError("identity correction V1 chain authority mismatch")
    if correction["authoritative_v1_support"] != recovery.V1_SUPPORT_IDENTITIES:
        raise RecoverySealError("identity correction V1 support authority mismatch")
    if recovery.REJECTED_TRANSIENT_V1_RUNNER_IDENTITY != REJECTED_TRANSIENT_V1_RUNNER_IDENTITY:
        raise RecoverySealError("runner/sealer rejected transient constant mismatch")
    validate_rejected_transient_wrapper(correction["rejected_transient_identity"])
...
def code_identities(workspace_root: Path) -> dict[str, dict[str, Any]]:
    paths = {
        ...
        / "scripts"
        / "run_noaa_gfs_multiseason_decode_recovery_v2.py",
        ...
        / "scripts"
        / "audit_noaa_gfs_multiseason_raw_postrun_v2.py",
        "postrun_auditor_test": workspace_root
        / "tests"
        / "test_noaa_gfs_multiseason_raw_postrun_v2.py",
    }
...
def static_audit(root: Path, workspace_root: Path) -> dict[str, Any]:
    ...
    validate_launch_incident_authority(root, fixed)
    v1_chain, v1_support = validate_superseded_v1_chain_and_support(
        root, workspace_root
    )
    v1_zero_state = recovery.validate_failed_v1_recovery_zero_state(root)
    temp_closure = require_control_temp_siblings_absent(root)
    recovery.require_canonical_outputs_absent(root)
    return {
        "status": "PASS_STATIC_V2_DECODE_RECOVERY_SEALER_READY",
        "code_identities": identities,
        "sealer_identities": sealers,
        "flawed_launch_incident": fixed["flawed_launch_incident"],
        "launch_identity_correction": fixed["launch_identity_correction"],
        "superseded_v1_chain": v1_chain,
        "superseded_v1_support": v1_support,
        "failed_v1_recovery_zero_state": v1_zero_state,
        "control_temp_closure": temp_closure,
...
def spawn_importability_audit(root: Path, workspace_root: Path) -> dict[str, Any]:
    ...
    temp_before = require_control_temp_siblings_absent(root)
    recovery.require_canonical_outputs_absent(root)
    identities = code_identities(workspace_root)
    fixed = fixed_root_identities(root)
    validate_launch_incident_authority(root, fixed)
    ...
    if fixed_root_identities(root) != fixed:
        raise RecoverySealError("fixed root input changed during spawn audit")
    if validate_superseded_v1_chain_and_support(root, workspace_root) != (
        v1_chain,
        v1_support,
    ):
        raise RecoverySealError("superseded V1 chain/support changed during spawn audit")
    if recovery.validate_failed_v1_recovery_zero_state(root) != v1_zero_state:
        raise RecoverySealError("failed V1 recovery zero-state changed during spawn audit")
    if require_control_temp_siblings_absent(root) != temp_before:
        raise RecoverySealError("control temp closure changed during spawn audit")
    return {
        "status": "PASS_V2_CANONICAL_MODULE_ENTRYPOINT_TOP_LEVEL_BOOTSTRAP_7SPAWN",
...
def seal(root: Path, workspace_root: Path) -> dict[str, Any]:
    ...
    temp_closure_before = require_control_temp_siblings_absent(root)
    recovery.require_canonical_outputs_absent(root)
    ...
    fixed = fixed_root_identities(root)
    validate_launch_incident_authority(root, fixed)
    superseded_v1_chain, superseded_v1_support = (
        validate_superseded_v1_chain_and_support(root, workspace_root)
    )
    failed_v1_zero_state = recovery.validate_failed_v1_recovery_zero_state(root)
    ...
    raw_lock_payload = {
        ...
        "flawed_launch_incident": fixed["flawed_launch_incident"],
        "launch_identity_correction": fixed["launch_identity_correction"],
        "superseded_v1_chain": superseded_v1_chain,
    ...
    validate_launch_incident_authority(root, fixed_after)
    if validate_superseded_v1_chain_and_support(root, workspace_root) != (
        superseded_v1_chain,
        superseded_v1_support,
    ):
        raise RecoverySealError("superseded V1 chain/support changed during preflight")
    if recovery.validate_failed_v1_recovery_zero_state(root) != failed_v1_zero_state:
        raise RecoverySealError("failed V1 recovery zero-state changed during preflight")
    if require_control_temp_siblings_absent(root) != temp_closure_before:
        raise RecoverySealError("control temp closure changed during preflight")
    for record in remnants:
    ...
    attempt_id = f"decode_recovery_v2__{compact}"
    ...
    preflight_payload = {
        ...
        "run_prefix_regression": recovery.RUN_PREFIX_REGRESSION_EVIDENCE,
    ...
    # Seal completion is still pre-launch: only the three V2 producer controls
    # may now exist.  Historical V1 authority and the failed-launch zero-state
    # must remain byte-identical, with no review/GO or recovery target writes.
    if destinations["independent_review"].exists() or destinations["independent_go"].exists():
        raise RecoverySealError("independent V2 review/GO appeared during sealing")
    if validate_superseded_v1_chain_and_support(root, workspace_root) != (
        superseded_v1_chain,
        superseded_v1_support,
    ):
        raise RecoverySealError("superseded V1 chain/support changed after publication")
    if fixed_root_identities(root) != fixed:
        raise RecoverySealError("fixed root input changed after publication")
    if recovery.validate_failed_v1_recovery_zero_state(root) != failed_v1_zero_state:
        raise RecoverySealError("failed V1 recovery zero-state changed after publication")
    recovery.require_canonical_outputs_absent(root)
    require_control_temp_siblings_absent(root)
    return {
        "status": "PASS_V2_AUTHORIZATION_SEALED_PENDING_INDEPENDENT_REVIEW_AND_GO",
