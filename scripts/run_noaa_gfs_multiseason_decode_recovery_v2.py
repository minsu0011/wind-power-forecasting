"""Append-only V2 network-zero recovery of the completed Track-A raw cache.
...
FLAWED_V1_CHAIN_IDENTITIES = {
    **V1_CHAIN_IDENTITIES,
    "recovery_runner": REJECTED_TRANSIENT_V1_RUNNER_IDENTITY,
}
V1_SUPPORT_IDENTITIES = {
    "recovery_runner": {
        "path": "scripts/run_noaa_gfs_multiseason_decode_recovery_v1.py",
        "size_bytes": 128_749,
        "sha256": "af61a52044d68b4a798ba24395d6c17ae56b827ab291ae67bbf46fd3e2aa1dcd",
...
FIXED_ROOT_INPUTS = {
    ...
    "raw_cache_lock": "prelaunch/DECODE_RECOVERY_RAW_CACHE_LOCK_V2.json",
    "real_spawn_preflight": "prelaunch/DECODE_RECOVERY_REAL_PROCESS_PREFLIGHT_V2.json",
}
...
RUN_PREFIX_REGRESSION_EVIDENCE = {
...
def validate_required_test_names_evidence(payload: Any) -> None:
    """Validate the sealer's nested file-to-test-name evidence exactly enough.

    ``required_test_names`` is deliberately a mapping because every listed test
    is bound to the source file whose identity was compiled and hashed.  The V2
    run-prefix regression must therefore be found in the canonical runner-test
    value, not among the mapping's file-name keys.
    """

    if not isinstance(payload, Mapping) or V2_RUN_PREFIX_TEST_FILE not in payload:
        raise RecoveryContractError("required test-name mapping/key mismatch")
    for relative, names in payload.items():
        if type(relative) is not str or not isinstance(names, list):
            raise RecoveryContractError("required test-name mapping value mismatch")
        if any(type(name) is not str or not name for name in names):
            raise RecoveryContractError("required test name must be a nonempty string")
        if len(names) != len(set(names)):
            raise RecoveryContractError("required test names must be unique per file")
    if (
        RUN_PREFIX_REGRESSION_EVIDENCE["dedicated_test_name"]
        not in payload[V2_RUN_PREFIX_TEST_FILE]
    ):
        raise RecoveryContractError("required test V2 run-prefix regression name absent")


def validate_launch_supersession_payload(payload: Mapping[str, Any]) -> None:
    required_v2 = payload.get("required_v2_supersession")
    root_cause = payload.get("root_cause")
    postfailure = payload.get("postfailure_state_closure")
    v1_disposition = payload.get("v1_disposition")
    if (
        set(payload)
        != {
        ...
        or payload.get("failed_recovery_attempt_id")
        != "decode_recovery_v1__20260810T194716214040Z"
        or payload.get("failed_v1_chain") != FLAWED_V1_CHAIN_IDENTITIES
        or not isinstance(required_v2, Mapping)
        or set(required_v2)
        != {
        ...
        or any(
            required_v2[key] is not True
            for key in required_v2
            if key != "retry_performed_by_this_incident_seal"
        )
        or required_v2.get("retry_performed_by_this_incident_seal") is not False
        or not isinstance(root_cause, Mapping)
        or root_cause.get("classification") != "RUN_PATH_LOCAL_BINDING_OMISSION"
        or root_cause.get("frozen_runner_sha256")
        != V1_SUPPORT_IDENTITIES["recovery_runner"]["sha256"]
        or root_cause.get("recovery_claim_acquired") is not False
        or not isinstance(postfailure, Mapping)
        or postfailure.get("canonical_outputs_present") != 0
        or postfailure.get("new_recovery_transaction_entries_present") != 0
        or postfailure.get("decoded_directory_present") is not False
        or postfailure.get("raw_active_lock_present") is not False
        or postfailure.get("decoded_active_lock_present") is not False
        or postfailure.get("recovery_target_writes") != 0
        or not isinstance(v1_disposition, Mapping)
        or v1_disposition.get("v1_retry_allowed") is not False
        or v1_disposition.get("v1_runner_remains_byte_frozen") is not True
    ):
        raise RecoveryContractError("V1 launch supersession incident semantics mismatch")


def validate_launch_identity_correction_payload(payload: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or payload.get("authoritative_failed_v1_chain") != V1_CHAIN_IDENTITIES
        or payload.get("authoritative_v1_support") != V1_SUPPORT_IDENTITIES
        or not preserved_digest_pass
        or payload.get("superseded_flawed_incident")
        != {
            "path": FIXED_ROOT_INPUTS["flawed_launch_incident"],
            "size_bytes": 7_172,
            "sha256": LAUNCH_SUPERSESSION_INCIDENT_SHA256,
        }
        ...
        or policy.get("v2_execution_or_retry_authorized_by_this_document") is not False
        or not isinstance(rejected, Mapping)
        or rejected.get("identity") != REJECTED_TRANSIENT_V1_RUNNER_IDENTITY
        or rejected.get("historical_authority") is not False
        or rejected.get("executed_in_failed_launch") is not False
        or rejected.get("authorized_by_v1_controls") is not False
        or not isinstance(authority, Mapping)
        or authority.get("v1_authorization_review_and_go_bind_runner_sha256")
        != V1_CHAIN_IDENTITIES["recovery_runner"]["sha256"]
        or authority.get("restored_v1_runner_rehash_matches_authorized_identity")
        is not True
        or not isinstance(postrehash, Mapping)
        or postrehash.get("all_exact") is not True
        or postrehash.get("v1_chain") != V1_CHAIN_IDENTITIES
        or postrehash.get("v1_support") != V1_SUPPORT_IDENTITIES
        or not isinstance(unchanged, Mapping)
        or unchanged.get("canonical_outputs_present") != 0
        or unchanged.get("decoded_directory_present") is not False
        or unchanged.get("new_recovery_transaction_entries_present") != 0
        or unchanged.get("recovery_claim_acquired") is not False
        or unchanged.get("recovery_target_writes") != 0
        or unchanged.get("runner_guard_and_failure_report_network_requests") != 0
    ):
        raise RecoveryContractError("launch identity correction semantics mismatch")
...
def validate_authorization(
    ...
    if (
        ...
        or authorization["status"] != "AUTHORIZED_PENDING_INDEPENDENT_GO_V2"
    ...
    if not attempt_id.startswith("decode_recovery_v2__") or any(
    ...
    if (
        str(authorization["flawed_launch_incident"]["sha256"])
        != LAUNCH_SUPERSESSION_INCIDENT_SHA256
    ):
        raise RecoveryContractError("launch-supersession incident constant mismatch")
    if (
        str(authorization["launch_identity_correction"]["sha256"])
        != LAUNCH_IDENTITY_CORRECTION_SHA256
    ):
        raise RecoveryContractError("launch identity correction constant mismatch")
    if (
    ...
    launch_incident = read_json_object(
        strict_under_root(root, FIXED_ROOT_INPUTS["flawed_launch_incident"]),
        "V1 launch NameError supersession incident",
    )
    validate_launch_supersession_payload(launch_incident)
    correction = read_json_object(
        strict_under_root(root, FIXED_ROOT_INPUTS["launch_identity_correction"]),
        "V1 launch identity correction",
    )
    validate_launch_identity_correction_payload(correction)
    if correction["superseded_flawed_incident"] != authorization[
        "flawed_launch_incident"
    ]:
        raise RecoveryContractError("correction does not bind flawed incident identity")
    preserved = correction["preserved_failure_evidence"]
    preserved_digests = correction["preserved_failure_evidence_canonical_sha256"]
    for key, value in preserved.items():
        if launch_incident.get(key) != value:
            raise RecoveryContractError(f"correction changed preserved failure evidence {key}")
        if canonical_json_sha256(value) != preserved_digests.get(key):
            raise RecoveryContractError(f"correction preserved evidence digest mismatch {key}")
    if authorization["superseded_v1_chain"] != V1_CHAIN_IDENTITIES:
        raise RecoveryContractError("authorization V1 chain mismatch")
    for role, record in V1_CHAIN_IDENTITIES.items():
        if role == "recovery_runner":
            path = require_external_identity(
                {
                    "path": str((workspace_root / str(record["path"])).resolve()),
                    "size_bytes": record["size_bytes"],
                    "sha256": record["sha256"],
                },
                workspace_root / str(record["path"]),
                "authoritative failed V1 runner",
            )
            if sha256_file(path) == REJECTED_TRANSIENT_V1_RUNNER_IDENTITY["sha256"]:
                raise RecoveryContractError("rejected transient V1 runner selected")
        else:
            require_root_identity(record, root, str(record["path"]), f"V1 {role}")
    support = authorization["superseded_v1_support"]
    if not isinstance(support, Mapping) or set(support) != set(V1_SUPPORT_IDENTITIES):
        raise RecoveryContractError("authorization V1 support schema mismatch")
    for role, expected in V1_SUPPORT_IDENTITIES.items():
        observed_path = require_external_identity(
            support[role], workspace_root / str(expected["path"]), f"V1 support {role}"
        )
        if (
            observed_path.stat().st_size != expected["size_bytes"]
            or sha256_file(observed_path) != expected["sha256"]
        ):
            raise RecoveryContractError(f"V1 support {role} frozen identity mismatch")
    incident_evidence = {
        record["path"]: record
    ...
    recovery_runner_test_path = (
        workspace_root
        / "tests"
        / "test_noaa_gfs_multiseason_decode_recovery_runner_v2.py"
    )
    postrun_auditor_path = (
        workspace_root / "scripts" / "audit_noaa_gfs_multiseason_raw_postrun_v2.py"
    )
    postrun_auditor_test_path = (
        workspace_root / "tests" / "test_noaa_gfs_multiseason_raw_postrun_v2.py"
    )
    ...
    validate_required_test_names_evidence(
        test_evidence.get("required_test_names")
    ...
    if (
        ...
        or preflight_payload.get("schema_version") != 2
        ...
        or preflight_payload.get("flawed_launch_incident")
        != authorization["flawed_launch_incident"]
        or preflight_payload.get("launch_identity_correction")
        != authorization["launch_identity_correction"]
        or preflight_payload.get("superseded_v1_chain")
        != authorization["superseded_v1_chain"]
        or preflight_payload.get("superseded_v1_support")
        ...
        or preflight_payload.get("run_prefix_regression")
        != RUN_PREFIX_REGRESSION_EVIDENCE
    ...
    require_external_identity(
        ...
        workspace_root
        / "scripts"
        / "seal_noaa_gfs_multiseason_decode_recovery_v2.py",
        "recovery sealer",
    )
    require_external_identity(
        ...
        workspace_root
        / "tests"
        / "test_seal_noaa_gfs_multiseason_decode_recovery_v2.py",
...
def validate_independent_go(
    ...
    if (
        go["schema_version"] != 2
        or go["artifact_type"] != "TRACK_A_DECODE_RECOVERY_INDEPENDENT_GO_V2"
        or go["status"] != "GO_V2_NETWORK_ZERO_PROCESS_ISOLATED_DECODE_RECOVERY"
    ...
    if (
        go["flawed_launch_incident"]
        != authorization["flawed_launch_incident"]
        or go["launch_identity_correction"]
        != authorization["launch_identity_correction"]
        or go["superseded_v1_chain"] != authorization["superseded_v1_chain"]
        or go["superseded_v1_support"] != authorization["superseded_v1_support"]
    ):
        raise RecoveryContractError("GO V1 supersession chain mismatch")
...
def fixed_input_snapshot(
    ...
    for role, record in sorted(V1_CHAIN_IDENTITIES.items()):
        if role == "recovery_runner":
            continue
        path = require_root_identity(record, root, str(record["path"]), f"V1 {role}")
        root_records.append(root_identity(path, root))
    ...
    for role, record in sorted(authorization["superseded_v1_support"].items()):
        path = _require_exact_file(record, f"snapshot V1 support {role}")
        external_records.append(
            {
                "path": str(path),
                "size_bytes": path.stat().st_size,
...
def validate_raw_cache_lock(
    ...
    if (
        ...
        or lock["artifact_type"] != "TRACK_A_DECODE_RECOVERY_RAW_CACHE_LOCK_V2"
        or lock["status"] != "PASS_V2_IMMUTABLE_RAW_CACHE_AND_V1_ZERO_STATE"
        or lock["incident"] != authorization["incident"]
        or lock["flawed_launch_incident"]
        != authorization["flawed_launch_incident"]
        or lock["launch_identity_correction"]
        != authorization["launch_identity_correction"]
        or lock["superseded_v1_chain"] != authorization["superseded_v1_chain"]
        or lock["superseded_v1_support"] != authorization["superseded_v1_support"]
        or lock["inventories"] != inventories
        or lock["prerecovery_topology"] != prerecovery_topology
        or lock["failed_v1_recovery_zero_state"] != v1_zero_state
...
def write_recovery_transaction_plan(
    ...
    decode_failure_incident_record: Mapping[str, Any],
    flawed_launch_incident_record: Mapping[str, Any],
    launch_identity_correction_record: Mapping[str, Any],
    superseded_v1_chain: Mapping[str, Any],
    superseded_v1_support: Mapping[str, Any],
    frozen: Any,
) -> Path:
    ...
    publish_json_create_if_absent(
        plan,
        {
            ...
            "decode_failure_incident": dict(decode_failure_incident_record),
            "flawed_launch_incident": dict(flawed_launch_incident_record),
            "launch_identity_correction": dict(launch_identity_correction_record),
            "superseded_v1_chain": dict(superseded_v1_chain),
            "superseded_v1_support": dict(superseded_v1_support),
            "items": items,
...
def commit_recovery_transaction_fail_if_exists(
    ...
    *,
    ...
    expected_decode_failure_incident: Mapping[str, Any],
    expected_flawed_launch_incident: Mapping[str, Any],
    expected_launch_identity_correction: Mapping[str, Any],
    expected_superseded_v1_chain: Mapping[str, Any],
    expected_superseded_v1_support: Mapping[str, Any],
) -> Path:
    ...
    if (
        ...
        or plan["decode_failure_incident"]
        != dict(expected_decode_failure_incident)
        or plan["flawed_launch_incident"] != dict(expected_flawed_launch_incident)
        or plan["launch_identity_correction"]
        != dict(expected_launch_identity_correction)
        or plan["superseded_v1_chain"] != dict(expected_superseded_v1_chain)
        or plan["superseded_v1_support"] != dict(expected_superseded_v1_support)
    ):
...
def validate_failed_v1_recovery_zero_state(root: Path) -> dict[str, Any]:
    """Prove that the authorized-but-failed V1 launch never claimed or wrote."""

    require_canonical_outputs_absent(root)
    if (root / "decoded").exists():
        raise RecoveryContractError("V1 zero-state requires decoded tree absent")
    raw_root = root / "raw"
    checks = {
        "output_transactions": raw_root / "output_transactions",
        "progress": raw_root / "progress",
        "launch_history": raw_root / "launch_history",
    }
    forbidden: list[str] = []
    for role, directory in checks.items():
        if (
            directory.is_symlink()
            or (hasattr(directory, "is_junction") and directory.is_junction())
            or not directory.is_dir()
        ):
            raise RecoveryContractError(f"V1 zero-state {role} directory invalid")
        for path in directory.rglob("*"):
            if path.is_symlink() or (
                hasattr(path, "is_junction") and path.is_junction()
            ):
                raise RecoveryContractError("V1 zero-state contains symlink/junction")
            if "decode_recovery_v1__" in path.name:
                forbidden.append(path.relative_to(root).as_posix())
    if forbidden:
        raise RecoveryContractError(
            f"failed V1 recovery left forbidden afterstate: {sorted(forbidden)}"
        )
    return {
...
def prepare_nonmutating_preclaim(
    root: Path, authorization_path: Path, go_path: Path, workspace_root: Path
) -> dict[str, Any]:
    """Execute the same read-only launch prefix for static audit and real run."""
    ...
    (
        ...
        recovery_core,
        recovery_bootstrap,
    ) = validate_authorization(root, authorization_path, workspace_root)
    ...
    v1_zero_state = validate_failed_v1_recovery_zero_state(root)
    validate_raw_cache_lock(
        root, authorization, before_inventories, prerecovery_topology, v1_zero_state
    )
    ...
    if pilot_report.get("status") != "PASS_REAL_GRIB_STDLIB_BOOTSTRAP_PROCESS_ISOLATION":
    ...
    # Close the historical failed-launch zero state a second time after the
    # process pilot, immediately before the only mutating boundary.
    if validate_failed_v1_recovery_zero_state(root) != v1_zero_state:
        raise RecoveryContractError("failed V1 zero-state changed during pilot")
    return {
...
def static_audit(
    ...
    preclaim = prepare_nonmutating_preclaim(
        root, authorization_path, go_path, workspace_root
    )
    authorization = preclaim["authorization"]
    return {
        "status": "PASS_V2_STATIC_REAL_PRECLAIM_NETWORK_ZERO_RECOVERY_READY",
        "authorization_sha256": sha256_file(authorization_path),
        "independent_go_sha256": sha256_file(go_path),
        "attempt_id": authorization["recovery_attempt_id"],
        "runtime": preclaim["runtime_report"],
        "pilot_report": preclaim["pilot_report"],
        "v1_zero_state": preclaim["v1_zero_state"],
        "postpilot_alias_check_reached": True,
        "recovery_claim_acquired": False,
        "network_requests": 0,
        "writes": 0,
    }
...
def run(
    ...
    authorization = preclaim["authorization"]
    runtime_report = preclaim["runtime_report"]
    frozen = preclaim["frozen"]
    recovery_bootstrap = preclaim["recovery_bootstrap"]
    bootstrap_import_contract = preclaim["bootstrap_import_contract"]
    go = preclaim["go"]
    fixed_inputs_initial = preclaim["fixed_inputs_initial"]
    outputs = preclaim["outputs"]
    rows = preclaim["rows"]
    prerecovery_topology = preclaim["prerecovery_topology"]
    raw_manifest_rows = preclaim["raw_manifest_rows"]
    before_inventories = preclaim["before_inventories"]
    pilot_report = preclaim["pilot_report"]
    v1_zero_state = preclaim["v1_zero_state"]
    ...
    recovery_provenance = {
        "decode_failure_incident": authorization["incident"],
        "flawed_launch_incident": authorization["flawed_launch_incident"],
        "launch_identity_correction": authorization["launch_identity_correction"],
        "superseded_v1_chain": authorization["superseded_v1_chain"],
        "superseded_v1_support": authorization["superseded_v1_support"],
        "recovery_authorization_v2": root_identity(authorization_path, root),
        "independent_recovery_go_v2": root_identity(go_path, root),
        "independent_recovery_review_v2": go["independent_review"],
        ...
        "raw_cache_lock_v2": authorization["raw_cache_lock"],
        "real_spawn_preflight_v2": authorization["real_spawn_preflight"],
        "pilot_runtime_report": pilot_report,
        "failed_v1_recovery_zero_state": v1_zero_state,
        "fixed_inputs_initial": fixed_inputs_initial,
    ...
    decoded_lock = {
        ...
        "physical_family_ready_for_independent_postrun": pbl_pass or wind_pass,
    ...
    commit = commit_recovery_transaction_fail_if_exists(
        ...
        expected_decode_failure_incident=authorization["incident"],
        expected_flawed_launch_incident=authorization["flawed_launch_incident"],
        expected_launch_identity_correction=authorization[
            "launch_identity_correction"
        ],
        expected_superseded_v1_chain=authorization["superseded_v1_chain"],
        expected_superseded_v1_support=authorization["superseded_v1_support"],
    )
    ...
    return {
        "status": "PASS_V2_NETWORK_ZERO_PROCESS_ISOLATED_DECODE_RECOVERY",
...
def main() -> None:
    ...
    parser.add_argument(
        "--authorization",
        type=Path,
        default=ROOT_DEFAULT / "prereg" / "decode_recovery_authorization_v2.json",
    )
    parser.add_argument(
        "--independent-go",
        type=Path,
        ...
        / "independent_redteam"
        / "TRACK_A_DECODE_RECOVERY_GO_V2.json",
    )
    ...
    try:
        ...
        report = {
            "status": "FAIL_V2_NETWORK_ZERO_DECODE_RECOVERY",
