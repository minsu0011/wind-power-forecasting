"""Read-only, no-fit post-run auditor for target-free duplicate execution V5.
...
two physical aliases required by the published V5 amendment (the parquet
...
SCHEMA_VERSION = 3
ARTIFACT_TYPE = "TARGET_FREE_DUPLICATE_NO_FIT_POSTRUN_AUDIT_V5"
PASS_STATUS = "PASS_V5_NO_FIT_POSTRUN_AUDIT"
FAIL_STATUS = "FAIL_V5_NO_FIT_POSTRUN_AUDIT"
...
AMENDMENT_V5_REL = "prereg/target_free_multiseason_amendment_v5.json"
INCIDENT_REL = (
    "incidents/"
    "TARGET_FREE_MULTISEASON_V3_EXECUTION_OR_PUBLICATION_FAILURE_V1.json"
)
CODE_SEAL_REL = "prereg/target_free_duplicate_code_seal_v5.json"
AUTH_REL = "prereg/target_free_duplicate_execution_authorization_v5.json"
REVIEW_REL = (
    "independent_redteam/"
    "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_REVIEW_V5.json"
)
GO_REL = (
    "independent_redteam/"
    "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_GO_V5.json"
)
POSTRUN_REL = (
    "independent_redteam/"
    "TRACK_A_TARGET_FREE_DUPLICATE_POSTRUN_PASS_V5.json"
)
...
V4_AFFECTED_SCHEMA_SOURCE = {
    "TARGET_FREE_DUPLICATE_OOF_V5.parquet": "TARGET_FREE_DUPLICATE_OOF_V3.parquet",
...
V3_CONTROL_PATHS = {
    "code_seal": "prereg/target_free_duplicate_code_seal_v3.json",
    "authorization": "prereg/target_free_duplicate_execution_authorization_v3.json",
    "independent_review": (
        "independent_redteam/TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_REVIEW_V3.json"
    ),
    "independent_go": (
        "independent_redteam/TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_GO_V3.json"
    ),
}
V3_OUTPUT_ROOT_REL = "modeling/target_free_multiseason_v3"

OUTPUT_ROOT_REL = "modeling/target_free_multiseason_v5"
TRANSACTION_REL = OUTPUT_ROOT_REL + "/.transaction_v5"
ALIAS_SPECS = {
...
CODE_CONFIG_RUNTIME_KEYS = frozenset(
    ("amendment", "amendment_v4", "amendment_v5", "code_identities", "runtime_identity")
)
...
FINAL_OUTPUT_FORMATS = {
    ...
    "TARGET_FREE_DUPLICATE_OOF_V5.parquet": "PARQUET",
    "TARGET_FREE_FIT_LEDGER_V5.json": "JSON",
    "TARGET_FREE_DISTRIBUTION_STABILITY_V5.json": "JSON",
    "TARGET_FREE_DERIVED_WIND_DIAGNOSTICS_V5.parquet": "PARQUET",
...
ATTEMPT_ID_RE = re.compile(r"^target_free_duplicate_v5__[0-9]{8}T[0-9]{12}Z$")
...
class PredataContext:
    ...
    amendment_v5: dict[str, Any]
    amendment_v5_identity: dict[str, Any]
    failure_incident: dict[str, Any]
    failure_incident_identity: dict[str, Any]
...
def _validate_attempt_id(value: Any) -> str:
    ...
    suffix = value.removeprefix("target_free_duplicate_v5__")
    try:
...
def _validate_amendment_v4(
    ...
    _require(
        authority.get("authorized_code_and_test_paths") == list(V3_CODE_ROLE_PATHS.values()),
...
def _validate_failure_incident(
    root: Path,
    repo: Path,
    amendment_identity: Mapping[str, Any],
    amendment_v4_identity: Mapping[str, Any],
    *,
    validate_historical_identities: bool = True,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """Bind the immutable V3 failed-attempt incident without trusting temp records."""

    path = _artifact_path(root, INCIDENT_REL)
    size, digest = sha256_file(path)
    _require(size == INCIDENT_SIZE, "V3 incident physical size mismatch")
    _require(digest == INCIDENT_SHA256, "V3 incident physical sha256 mismatch")
    raw = path.read_bytes()
    incident = strict_json_bytes(raw, label=INCIDENT_REL)
    compact = canonical_json_bytes(incident)
    _require(len(compact) == INCIDENT_CANONICAL_SIZE, "V3 incident canonical size mismatch")
    _require(
        sha256_bytes(compact) == INCIDENT_CANONICAL_SHA256,
        "V3 incident canonical sha256 mismatch",
    )
    _require(
        pretty_json_bytes(incident, ensure_ascii=True) == raw,
        "V3 incident pretty serialization mismatch",
    )
    strict = incident.get("strict_schema")
    _require(isinstance(strict, dict), "V3 incident strict_schema missing")
    _exact_keys(incident, strict.get("top_level_keys_exact", ()), "V3 incident")
    _require(
        len(incident) == strict.get("top_level_key_count") == 20,
        "V3 incident top-level count mismatch",
    )
    for pointer, expected_count in strict.get("nested_object_key_counts_exact", {}).items():
        if "*" in pointer:
            continue
        cursor: Any = incident
        for token in pointer.strip("/").split("/"):
            cursor = cursor[token]
        _require(
            isinstance(cursor, dict) and len(cursor) == expected_count,
            f"V3 incident nested count mismatch: {pointer}",
        )
    _require(type(incident.get("schema_version")) is int and incident["schema_version"] == 1, "V3 incident schema mismatch")
    _require(
        incident.get("artifact_type")
        == "TARGET_FREE_DUPLICATE_V3_BOUND_INPUT_FOOTER_SCHEMA_FALSE_REJECT_INCIDENT_V1",
        "V3 incident artifact type mismatch",
    )
    _require(
        incident.get("status")
        == "SEALED_V3_BOUND_INPUT_FOOTER_SCHEMA_FALSE_REJECT_AFTER_ALIAS2_NO_VALUE_READ_NO_FIT_V5_REQUIRED",
        "V3 incident status mismatch",
    )
    _require(incident.get("canonical_path") == INCIDENT_REL, "V3 incident path mismatch")
    _parse_utc_microseconds(incident.get("created_utc"), "incident.created_utc")
    _require(
        _json_type_exact_equal(
            incident.get("amendments"),
            {"amendment": dict(amendment_identity), "amendment_v4": dict(amendment_v4_identity)},
        ),
        "V3 incident amendment identities mismatch",
    )

    failure = incident.get("failure_boundary")
    _require(isinstance(failure, dict) and len(failure) == 22, "V3 failure boundary not exact22")
    _require(
        failure.get("exception_class") == "IntegrityError"
        and failure.get("failing_bound_input_role") == "decoded_site_matrix"
        ...
        "V3 failure boundary drift",
    )
    disposition = incident.get("v3_disposition")
    _require(
        ...
        and disposition.get("v5_new_namespace_code_controls_and_single_attempt_required") is True,
        "V3 disposition drift",
    )
    supersession = incident.get("required_v5_supersession")
    _require(isinstance(supersession, dict) and len(supersession) == 20, "V5 supersession not exact20")
    _require(
        supersession.get("amendment_path") == AMENDMENT_V5_REL
        and supersession.get("authorized_code_and_test_paths") == list(CODE_ROLE_PATHS.values())
        and supersession.get("control_paths")
        == {
            "authorization": AUTH_REL,
            "code_seal": CODE_SEAL_REL,
            "independent_go": GO_REL,
            "independent_postrun": POSTRUN_REL,
            "independent_review": REVIEW_REL,
        }
        and supersession.get("future_control_binding_fields_exact")
        == ["amendment", "amendment_v4", "amendment_v5"]
        and supersession.get("v5_go_prior_v5_failure_incident") is None,
        "V3 incident V5 supersession drift",
    )
    if validate_historical_identities:
        for role, record in incident.get("code_identities", {}).items():
            _require(role in V3_CODE_ROLE_PATHS, f"unexpected V3 code role: {role}")
            verify_file_identity(
                record,
                repo / V3_CODE_ROLE_PATHS[role],
                label=f"incident.code_identities.{role}",
                expected_path=_path_text(repo / V3_CODE_ROLE_PATHS[role]),
            )
        for role, record in incident.get("control_identities", {}).items():
            _require(role in V3_CONTROL_PATHS, f"unexpected V3 control role: {role}")
            verify_file_identity(
                record,
                _artifact_path(root, V3_CONTROL_PATHS[role]),
                label=f"incident.control_identities.{role}",
                expected_path=V3_CONTROL_PATHS[role],
            )
    identity = {
        "path": INCIDENT_REL,
        "size_bytes": size,
...
def _validate_amendment_v5(
    ...
    """Validate the standalone V5 prevalue-footer supersession exactly."""

    path = _artifact_path(root, AMENDMENT_V5_REL)
    size, digest = sha256_file(path)
    _require(size == AMENDMENT_V5_SIZE, "V5 amendment physical size mismatch")
    _require(digest == AMENDMENT_V5_SHA256, "V5 amendment physical sha256 mismatch")
    raw = path.read_bytes()
    value = strict_json_bytes(raw, label=AMENDMENT_V5_REL)
    compact = canonical_json_bytes(value)
    _require(len(compact) == AMENDMENT_V5_CANONICAL_SIZE, "V5 amendment canonical size mismatch")
    _require(
        sha256_bytes(compact) == AMENDMENT_V5_CANONICAL_SHA256,
        "V5 amendment canonical sha256 mismatch",
    )
    _require(pretty_json_bytes(value, ensure_ascii=False) == raw, "V5 amendment pretty serialization mismatch")
    strict = value.get("strict_schema")
    _require(isinstance(strict, dict), "V5 amendment strict_schema missing")
    _exact_keys(value, strict.get("top_level_exact_keys", ()), "V5 amendment")
    _require(
        len(value) == strict.get("top_level_exact_key_count") == 24,
        "V5 amendment top-level count mismatch",
    )
    for key, expected_count in strict.get("top_level_object_key_counts", {}).items():
        _require(
            key in value and isinstance(value[key], dict) and len(value[key]) == expected_count,
            f"V5 amendment nested count mismatch: {key}",
        )
    _require(type(value.get("schema_version")) is int and value["schema_version"] == 5, "V5 amendment schema mismatch")
    _require(
        value.get("artifact_type")
        == "TARGET_FREE_MULTISEASON_PREREGISTRATION_APPEND_ONLY_EXECUTION_SUPERSESSION_V5",
        "V5 amendment artifact type mismatch",
    )
    _require(
        value.get("status")
        ...
        "V5 amendment status mismatch",
    )
    _require(value.get("created_utc") == AMENDMENT_V5_CREATED_UTC, "V5 created_utc drift")
    _parse_utc_microseconds(value["created_utc"], "amendment_v5.created_utc")
    _require(value.get("canonical_path") == AMENDMENT_V5_REL, "V5 canonical path mismatch")
    _require(_json_type_exact_equal(value.get("bound_v3_amendment"), amendment_identity), "V5 V3 identity mismatch")
    _require(_json_type_exact_equal(value.get("bound_v4_amendment"), amendment_v4_identity), "V5 V4 identity mismatch")
    _require(_json_type_exact_equal(value.get("bound_v3_failure_incident"), incident_identity), "V5 incident identity mismatch")

    inherited = value.get("inherited_effective_contract")
    _require(isinstance(inherited, dict) and len(inherited) == 15, "V5 inherited contract not exact15")
    _require(
        inherited.get("semantic_incorporation_and_authority_order_exact")
        == [
            "THIS_STANDALONE_V5_EXPLICIT_DELTAS",
            "BOUND_V4_EXACT2_SERIALIZATION_CORRECTION_BY_EXPLICIT_REFERENCE",
            "BOUND_V3_ALL_REMAINING_SEMANTICS_BY_EXPLICIT_REFERENCE",
        ]
        and inherited.get("v3_v4_parsed_semantic_mutation_forbidden") is True,
        "V5 standalone/no-merge closure mismatch",
    )
    correction = value.get("correction_scope")
    _require(
        isinstance(correction, dict)
        and len(correction) == 12
        and correction.get("parsed_v3_v4_pointer_replacement_count") == 0
        and correction.get("scope_expansion_forbidden") is True
        and correction.get("failed_role") == "decoded_site_matrix",
        "V5 correction scope mismatch",
    )
    _require(
        value.get("execution_budget") == EXECUTION_BUDGET,
        "V5 execution budget drift",
    )
    _require(
        value.get("code_and_test_paths_exact") == CODE_ROLE_PATHS,
        "V5 code/test paths mismatch",
    )
    controls = value.get("control_contract")
    _require(isinstance(controls, dict) and len(controls) == 26, "V5 control contract not exact26")
    _require(controls.get("control_schema_version_exact") == 5, "V5 control schema version mismatch")
    _require(
        controls.get("control_paths_exact")
        == {
            "authorization": AUTH_REL,
            "code_seal": CODE_SEAL_REL,
            "independent_go": GO_REL,
        ...
        "V5 control paths mismatch",
    )
    _require(
        controls.get("control_top_level_key_counts_exact")
        == {"AUTHORIZATION": 17, "CODE_SEAL": 13, "GO": 17, "POSTRUN": 18, "REVIEW": 16},
        "V5 control top counts mismatch",
    )
    _require(
        controls.get("amendment_identity_field_names_exact")
        == ["amendment", "amendment_v4", "amendment_v5"]
        and controls.get("output_namespace_exact") == OUTPUT_NAMESPACE
        and controls.get("single_attempt_exact") == SINGLE_ATTEMPT,
        "V5 control identity/namespace/single-attempt mismatch",
    )
    final_binding = value.get("final_lock_manifest_binding")
    _require(isinstance(final_binding, dict) and len(final_binding) == 9, "V5 final binding not exact9")
    _require(
        final_binding.get("superseding_exact_keys")
        == ["amendment", "amendment_v4", "amendment_v5", "code_identities", "runtime_identity"]
        and final_binding.get("superseding_exact_key_count") == 5
        and final_binding.get("lock_generation_literals", {}).get("status_by_decision_status_exact")
        == LOCK_STATUS_BY_DECISION_STATUS,
        "V5 final lock/manifest binding mismatch",
    )
    authority = value.get("authority_scope")
    _require(isinstance(authority, dict) and len(authority) == 22, "V5 authority not exact22")
    _require(
    ...
    _require(
        all(
            ...
            not in {
                "authorized_code_and_test_paths",
                "code_and_test_implementation_authorized",
                "documentary_amendment",
                "separate_code_seal_review_and_go_required",
            }
        ),
        "V5 amendment alone grants forbidden authority",
    )
    _require(
        value.get("runtime_and_serialization_preservation", {}).get("v3_runtime_identity_verbatim")
        == amendment.get("runtime_identity")
        and value.get("runtime_and_serialization_preservation", {}).get("v3_deterministic_execution_verbatim")
        == amendment.get("deterministic_execution"),
        "V5 inherited V3 runtime/determinism mismatch",
    )
    _require(
        value.get("runtime_and_serialization_preservation", {}).get("v4_parquet_exact12_kwargs_verbatim")
        == PARQUET_WRITE_KWARGS_EXACT,
        "V5 inherited V4 exact12 mismatch",
    )
    identity = {
        "path": AMENDMENT_V5_REL,
        "size_bytes": size,
        "sha256": digest,
        "canonical_size_bytes": len(compact),
        "canonical_sha256": sha256_bytes(compact),
...
def _expected_commands(root: Path, amendment: Mapping[str, Any]) -> dict[str, list[str]]:
    ...
    return {
        "runner": [python, "-B", "-m", "scripts.run_noaa_gfs_target_free_duplicate_v5", *base],
        "auditor": [python, "-B", "-m", "scripts.audit_noaa_gfs_target_free_duplicate_v5", *base],
        "sealer": [
            python,
            "-B",
            "-m",
            "scripts.seal_noaa_gfs_target_free_duplicate_v5",
            *base,
            "--postrun-pass",
...
def _validate_code_seal(
    ...
    amendment_identity: Mapping[str, Any],
    amendment_v4_identity: Mapping[str, Any],
    amendment_v5_identity: Mapping[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    _require(type(seal["schema_version"]) is int and seal["schema_version"] == 5, "code seal schema mismatch")
    _require(seal["artifact_type"] == "TARGET_FREE_DUPLICATE_CODE_SEAL_V5", "code seal artifact mismatch")
    _require(seal["status"] == "SEALED_V5_CODE_AND_TESTS_NO_EXECUTION_AUTHORITY", "code seal status mismatch")
    _require(seal["publisher"] == "V5_SEALER_CODE_SEAL_MODE_CREATE_IF_ABSENT_ONLY", "code seal publisher mismatch")
    ...
    _require(
        _json_type_exact_equal(seal["amendment_v5"], amendment_v5_identity),
        "code seal V5 amendment crosslink mismatch",
    )
    ...
    amendment_v5_created = _parse_utc_microseconds(AMENDMENT_V5_CREATED_UTC, "amendment_v5.created_utc")
    _require(
        amendment_v5_created < completed <= created,
        "test evidence chronology must be amendment V5 < completed <= code seal",
    )
...
def _validate_authorization(
    ...
    _require(type(auth["schema_version"]) is int and auth["schema_version"] == 5, "authorization schema mismatch")
    _require(auth["artifact_type"] == "TARGET_FREE_DUPLICATE_EXECUTION_AUTHORIZATION_V5", "authorization artifact mismatch")
    _require(auth["status"] == "AUTHORIZED_V5_SINGLE_ATTEMPT_PENDING_INDEPENDENT_REVIEW_AND_GO", "authorization status mismatch")
    ...
    _require(
        _json_type_exact_equal(auth["amendment_v5"], amendment_v5_identity),
        "authorization V5 amendment mismatch",
    )
    ...
    _require(
        _json_type_exact_equal(
            ...
            {"path": REVIEW_REL, "status_required": "PASS_V5_EXECUTION_REVIEW_PENDING_INDEPENDENT_GO"},
        ),
    ...
    _require(
        _json_type_exact_equal(
            auth["required_go"],
            {"path": GO_REL, "status_required": "GO_V5_SINGLE_TARGET_FREE_ATTEMPT"},
        ),
...
def _validate_review(
    ...
    _require(type(review["schema_version"]) is int and review["schema_version"] == 5, "review schema mismatch")
    _require(review["artifact_type"] == "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_REVIEW_V5", "review artifact mismatch")
    _require(review["status"] == "PASS_V5_EXECUTION_REVIEW_PENDING_INDEPENDENT_GO", "review status mismatch")
    ...
    _require(
        _json_type_exact_equal(review["amendment_v5"], amendment_v5_identity),
        "review V5 amendment mismatch",
    )
...
def _validate_go(
    ...
    _require(type(go["schema_version"]) is int and go["schema_version"] == 5, "GO schema mismatch")
    _require(go["artifact_type"] == "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_GO_V5", "GO artifact mismatch")
    _require(go["status"] == "GO_V5_SINGLE_TARGET_FREE_ATTEMPT", "GO status mismatch")
    ...
    _require(_json_type_exact_equal(go["amendment_v5"], amendment_v5_identity), "GO V5 amendment mismatch")
...
def validate_predata_authority(
    ...
    *,
    ...
    validate_historical_state: bool = True,
) -> PredataContext:
    ...
    failure_incident, failure_incident_identity = _validate_failure_incident(
        root,
        repo,
        amendment_identity,
        amendment_v4_identity,
        validate_historical_identities=validate_historical_state,
    )
    amendment_v5, amendment_v5_identity = _validate_amendment_v5(
        root,
        amendment,
        amendment_identity,
        amendment_v4,
    ...
    chronology = [
        ...
        _parse_utc_microseconds(failure_incident["created_utc"], "incident.created_utc"),
        _parse_utc_microseconds(amendment_v5["created_utc"], "amendment_v5.created_utc"),
    ...
    context = PredataContext(
        ...
        failure_incident_identity=failure_incident_identity,
        code_seal=code_seal,
    ...
    if validate_historical_state:
        _validate_historical_v3_root(context)
    return context


def _validate_historical_v3_root(context: PredataContext) -> None:
    """Require the failed V3 namespace to remain the immutable exact2 afterstate."""

    contract = context.amendment_v5.get("v3_failure_root_preservation")
    _require(isinstance(contract, dict) and len(contract) == 12, "V3 root preservation not exact12")
    _require(
        contract.get("root") == V3_OUTPUT_ROOT_REL + "/"
        and contract.get("entry_count_exact") == 2
        and contract.get("allowed_entry_names_exact")
        == ["FIELD_CENSUS_LOCK.json", "PROVENANCE_LEDGER.parquet"]
        and contract.get("single_v3_attempt_consumed") is True
        and contract.get("v3_retry_or_root_reuse_allowed") is False
        and contract.get("v3_aliases_may_source_v5_aliases") is False,
        "V3 root preservation literals mismatch",
    )
    v3_root = _artifact_path(context.root, V3_OUTPUT_ROOT_REL)
    _require(v3_root.is_dir() and not v3_root.is_symlink(), "V3 exact2 root missing or unsafe")
    entries = {item.name: item for item in v3_root.iterdir()}
    _require(
        list(sorted(entries)) == contract["allowed_entry_names_exact"],
        "V3 root is not exact2",
    )
    by_key = {
        "FIELD_CENSUS_LOCK": "FIELD_CENSUS_LOCK.json",
        "PROVENANCE_LEDGER": "PROVENANCE_LEDGER.parquet",
    }
    for key, basename in by_key.items():
        frozen = contract["entries_exact"][key]
        alias = entries[basename]
        source = _artifact_path(context.root, frozen["source_identity"]["path"])
        verify_file_identity(
            frozen["alias_identity"],
            alias,
            label=f"V3 root {basename}",
            expected_path=frozen["alias_identity"]["path"],
        )
        verify_file_identity(
            frozen["source_identity"],
            source,
            label=f"V3 source {basename}",
            expected_path=frozen["source_identity"]["path"],
        )
        _require(not os.path.samefile(source, alias), f"V3 alias shares source file: {basename}")
        source_stat = source.stat()
        alias_stat = alias.stat()
        if source_stat.st_ino and alias_stat.st_ino:
            _require(source_stat.st_ino != alias_stat.st_ino, f"V3 alias file id not distinct: {basename}")
        if source_stat.st_nlink:
            _require(source_stat.st_nlink == frozen["source_persistent_link_count_exact"], f"V3 source link drift: {basename}")
        if alias_stat.st_nlink:
            _require(alias_stat.st_nlink == frozen["alias_persistent_link_count_exact"], f"V3 alias link drift: {basename}")
    afterstate = context.failure_incident["postfailure_state"]
    for relative, present in afterstate["alias_stage_presence"].items():
        _require(present is False and not os.path.lexists(_artifact_path(context.root, relative)), f"V3 alias stage present: {relative}")
    for map_name in ("runner_staged_output_presence", "final_output_presence"):
        for relative, present in afterstate[map_name].items():
            _require(present is False and not os.path.lexists(_artifact_path(context.root, relative)), f"V3 forbidden residual present: {relative}")
    _require(
        not os.path.lexists(_artifact_path(context.root, V3_OUTPUT_ROOT_REL + "/.transaction_v3")),
        "V3 transaction root present",
    )
    aliases = context.amendment_v5["output_contract"]["aliases_exact"]
    _require(
        aliases["FIELD_CENSUS_LOCK.json"]["original_source_path"] == "manifest_census_v1.json"
        and aliases["PROVENANCE_LEDGER.parquet"]["original_source_path"]
        == "raw/RAW_RANGE_MANIFEST.parquet",
        "V5 aliases are not bound to original sources",
    )
...
def _validate_namespace_structure(
    ...
    _require(output_root.is_dir() and not output_root.is_symlink(), "V5 output root missing or unsafe")
    _require(transaction.is_dir() and not transaction.is_symlink(), "V5 transaction root missing or unsafe")

    root_entries = {entry.name: entry for entry in output_root.iterdir()}
    _require(
        frozenset(root_entries) == frozenset((*ALIAS_SPECS, ".transaction_v5")),
    ...
    field_stage = _reject_linklike_components(
        output_root / ".alias_stage_FIELD_CENSUS_LOCK_v5",
        label="field-census alias stage",
    )
    provenance_stage = _reject_linklike_components(
        output_root / ".alias_stage_PROVENANCE_LEDGER_v5",
...
def _validate_v4_affected_schema(
    ...
    source_basename = V4_AFFECTED_SCHEMA_SOURCE.get(basename, basename)
    _require(source_basename in contracts, f"V4 affected schema missing: {basename}")
    contract = contracts[source_basename]
    _require(
        contract
        == {
            "valid_time_utc": {"arrow_type": "timestamp[ns, tz=UTC]", "nullable": False},
...
def _validate_v5_bound_footer_metadata(context: PredataContext, pa: Any, pq: Any) -> None:
    """Independently replay the V5 exact4 metadata-only gate after GO."""

    section = context.amendment_v5.get("bound_parquet_footer_schemas_exact")
    _require(isinstance(section, dict) and len(section) == 8, "V5 exact4 footer section not exact8")
    roles = section.get("gate_roles_exact_order")
    _require(
        roles
        == [
            "decoded_site_matrix",
    ...
    canonical = section.get("structural_schema_canonicalization")
    _require(
        isinstance(canonical, dict)
        and canonical.get("constructor_type_literals_exact")
        == ["large_string", "int16", "int64", "float64"]
        and canonical.get("comparison_contract")
        == "RECONSTRUCT_EXPECTED_PYARROW_DATATYPES_AND_REQUIRE_STRUCTURAL_DATATYPE_EQUALITY",
        "V5 footer structural comparison contract mismatch",
    )
    common = section.get("common_footer_requirements")
    _require(
        isinstance(common, dict)
        and common.get("all_four_roles_must_pass") is True
        and common.get("source_logical_values_read_exact") == 0
        and common.get("value_handoff_must_remain_false") is True
        and common.get("allowed_api_exact") == ["pyarrow.parquet.ParquetFile"]
        and common.get("allowed_members_exact") == ["metadata.num_rows", "schema_arrow"],
        "V5 common footer gate mismatch",
    )
    for role in roles:
        record = section[role]
        _require(isinstance(record, dict) and len(record) == 6, f"V5 footer role not exact6: {role}")
        bound_key = record.get("bound_input_key")
        _require(bound_key == role, f"V5 footer bound-input key mismatch: {role}")
        source_identity = record.get("source_identity")
        _require(isinstance(source_identity, dict), f"V5 footer source identity missing: {role}")
        path = _resolve_bound_input(context.root, source_identity["path"])
        verify_file_identity(
            source_identity,
            path,
            label=f"V5 footer source {role}",
            expected_path=source_identity["path"],
        )
        try:
            parquet = pq.ParquetFile(_reject_linklike_components(path, label=f"V5 footer {role}"))
            rows = parquet.metadata.num_rows
            schema = parquet.schema_arrow
            columns = list(schema.names)
        except Exception as exc:
            raise AuditError(f"V5 footer open failed: {role}") from exc
        finally:
            if "parquet" in locals():
                del parquet
        _require(type(rows) is int and rows == record["row_count_exact"], f"V5 footer rows mismatch: {role}")
        _require(columns == record["column_names_exact"], f"V5 footer columns mismatch: {role}")
        expected_types = record["pyarrow_constructor_types_exact"]
        _require(len(schema) == len(expected_types), f"V5 footer field count mismatch: {role}")
        for index, (field, literal) in enumerate(zip(schema, expected_types)):
            _require(literal in constructors, f"V5 footer constructor literal unknown: {role}[{index}]")
            _require(field.type == constructors[literal], f"V5 footer structural type mismatch: {role}[{index}]")
        witness = {
            "columns": columns,
            "rows": rows,
        ...
        frozen = record["structural_schema_fingerprint"]
        _require(
            len(witness_bytes) == frozen["canonical_size_bytes"]
            and sha256_bytes(witness_bytes) == frozen["canonical_sha256"],
            f"V5 footer fingerprint mismatch: {role}",
        )
...
def _validate_decision(payload: Mapping[str, Any], amendment: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _require(type(payload["schema_version"]) is int and payload["schema_version"] == 5, "decision schema_version mismatch")
    _require(payload["artifact_type"] == "TARGET_FREE_FAMILY_DECISION_V5", "decision artifact_type mismatch")
...
def _validate_code_config_runtime_identities(
    ...
    *,
    ...
    _exact_keys(amendment_v5_identity, AMENDMENT_IDENTITY_KEYS, "amendment_v5 identity")
    ...
    expected = {
        ...
        "amendment_v5": dict(amendment_v5_identity),
...
def validate_family_lock_payload(
    ...
    *,
    ...
    _require(type(payload["schema_version"]) is int and payload["schema_version"] == 5, "family lock schema_version mismatch")
    ...
    _require(decision_payload["status"] in LOCK_STATUS_BY_DECISION_STATUS, "family lock decision status is unfrozen")
    _require(
        payload["status"] == LOCK_STATUS_BY_DECISION_STATUS[decision_payload["status"]],
        "family lock terminal status mismatch",
    )
...
def validate_run_manifest_payload(
    ...
    *,
    ...
    _require(type(payload["schema_version"]) is int and payload["schema_version"] == 5, "run manifest schema_version mismatch")
    ...
    _require(
        _parse_utc_microseconds(AMENDMENT_V5_CREATED_UTC, "amendment_v5.created_utc")
        < postrun_time
    ...
    _require(
        type(fit_ledger["schema_version"]) is int
        and fit_ledger["schema_version"] == 5
        and fit_ledger["artifact_type"] == "TARGET_FREE_FIT_LEDGER_V5"
...
def _validate_evidence_document_headers(
    ...
    _require(type(ledger["schema_version"]) is int and ledger["schema_version"] == 5, "fit ledger schema_version mismatch")
    _require(ledger["artifact_type"] == "TARGET_FREE_FIT_LEDGER_V5", "fit ledger artifact_type mismatch")
    _require(type(distribution["schema_version"]) is int and distribution["schema_version"] == 5, "distribution schema_version mismatch")
    _require(distribution["artifact_type"] == "TARGET_FREE_DISTRIBUTION_STABILITY_V5", "distribution artifact_type mismatch")
...
def validate_staged_outputs(
    ...
    *,
    ...
    _validate_v5_bound_footer_metadata(context, pa, pq)
    ...
    ledger_path, ledger_identity = raw_staged["TARGET_FREE_FIT_LEDGER_V5.json"]
    ledger_payload, ledger_raw = _read_strict_output_json(
        ledger_path,
        schemas["TARGET_FREE_FIT_LEDGER_V3_json"]["top_keys_exact"],
        label="TARGET_FREE_FIT_LEDGER_V5.json",
    )
    distribution_path, distribution_identity = raw_staged["TARGET_FREE_DISTRIBUTION_STABILITY_V5.json"]
    distribution_payload, distribution_raw = _read_strict_output_json(
        ...
        label="TARGET_FREE_DISTRIBUTION_STABILITY_V5.json",
    )
    oof_path, oof_identity = raw_staged["TARGET_FREE_DUPLICATE_OOF_V5.parquet"]
    oof_table, oof_logical = _read_staged_parquet(
        ...
        label="TARGET_FREE_DUPLICATE_OOF_V5.parquet",
    )
    ...
    derived_path, derived_identity = raw_staged["TARGET_FREE_DERIVED_WIND_DIAGNOSTICS_V5.parquet"]
    derived_table, derived_logical = _read_staged_parquet(
        ...
        label="TARGET_FREE_DERIVED_WIND_DIAGNOSTICS_V5.parquet",
    )
    _validate_v4_affected_schema(
        pa,
    ...
    staged_records = {
        ...
        "TARGET_FREE_DUPLICATE_OOF_V5.parquet": _staged_identity_record(oof_identity, "TARGET_FREE_DUPLICATE_OOF_V5.parquet", oof_table.num_rows, oof_logical),
        "TARGET_FREE_FIT_LEDGER_V5.json": _staged_identity_record(ledger_identity, "TARGET_FREE_FIT_LEDGER_V5.json", len(ledger_payload["unit_slots"]), sha256_bytes(ledger_raw)),
        "TARGET_FREE_DISTRIBUTION_STABILITY_V5.json": _staged_identity_record(distribution_identity, "TARGET_FREE_DISTRIBUTION_STABILITY_V5.json", len(distribution_payload["records"]), sha256_bytes(distribution_raw)),
        "TARGET_FREE_DERIVED_WIND_DIAGNOSTICS_V5.parquet": _staged_identity_record(derived_identity, "TARGET_FREE_DERIVED_WIND_DIAGNOSTICS_V5.parquet", derived_table.num_rows, derived_logical),
    }
...
def _validate_actual_auditor_command(
    ...
    _require(
        len(expected) == 10
        and expected[1:4]
        == ["-B", "-m", "scripts.audit_noaa_gfs_target_free_duplicate_v5"]
        and expected[4] == "--root"
...
def audit(
    ...
    *,
    ...
    payload = {
        ...
        "amendment_v5": context.amendment_v5_identity,
...
def install_readonly_runtime_guard() -> None:
    ...
    def guard(event: str, args: tuple[Any, ...]) -> None:
        if event.startswith("socket."):
            raise PermissionError("V5 auditor network access denied")
        if event in forbidden_events:
            raise PermissionError(f"V5 auditor mutation denied: {event}")
        if event == "open":
            mode = args[1] if len(args) > 1 else None
            flags = args[2] if len(args) > 2 else 0
            if isinstance(mode, str) and any(token in mode for token in ("w", "a", "x", "+")):
                raise PermissionError("V5 auditor write open denied")
            if isinstance(flags, int) and flags & write_flags:
                raise PermissionError("V5 auditor write flags denied")
