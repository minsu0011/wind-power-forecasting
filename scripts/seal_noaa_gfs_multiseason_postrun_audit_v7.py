#!/usr/bin/env python
"""Seal the append-only V7 postrun-audit authorization.
...
files by metadata only, compiles the four executing V7 sources, runs four test
...
AUDITOR_SOURCE_PATH = REPO / "scripts" / "audit_noaa_gfs_multiseason_raw_postrun_v7.py"
...
def _require_preimport_cache_runtime() -> None:
    if not (
        ...
        raise RuntimeError(
            "V7 sealer import requires -B, PYTHONDONTWRITEBYTECODE=1, "
...
def _require_auditor_bytecode_absent(path: Path) -> None:
    ...
    if matches:
        raise RuntimeError(
            "V7 auditor matching bytecode/cache competitor exists: "
...
def _require_auditor_source_competitors_absent(path: Path) -> None:
    ...
    if competitors:
        raise RuntimeError(
            "V7 auditor same-stem source-origin competitor exists: "
            + ", ".join(str(candidate) for candidate in competitors)
        )


def _preimport_auditor_source_guard(path: Path) -> None:
    """Reject top-level I/O/network routes before importing the moving V7 source."""
    ...
    if imported & forbidden_imports:
        raise RuntimeError("V7 auditor has a forbidden import-time route")
    allowed_top_level = {
    ...
    if any(type(node) not in allowed_top_level for node in tree.body):
        raise RuntimeError("V7 auditor has an unexpected top-level statement")
    for node in tree.body:
        if isinstance(node, ast.Expr) and not isinstance(node.value, ast.Constant):
            raise RuntimeError("V7 auditor has a top-level expression side effect")
        if isinstance(node, ast.If):
            if (
                ...
                raise RuntimeError("V7 auditor has an unexpected top-level branch")
        if isinstance(node, (ast.Assign, ast.AnnAssign)):
            for call in (item for item in ast.walk(node) if isinstance(item, ast.Call)):
                ...
                if not allowed:
                    raise RuntimeError(
                        f"V7 auditor has an unapproved import-time call: {function}"
                    )
...
if not (
    auditor.__name__ == "scripts.audit_noaa_gfs_multiseason_raw_postrun_v7"
    ...
    raise RuntimeError("V7 auditor import source origin mismatch")


ROOT_DEFAULT = auditor.ROOT_DEFAULT
CANONICAL_MODULE_ENTRYPOINT = (
    "scripts.seal_noaa_gfs_multiseason_postrun_audit_v7"
)
AUTH_RELATIVE = auditor.V7_AUTH_RELATIVE
REVIEW_RELATIVE = auditor.V7_REVIEW_RELATIVE
GO_RELATIVE = auditor.V7_GO_RELATIVE
SEALER_TEST = REPO / "tests" / "test_seal_noaa_gfs_multiseason_postrun_audit_v7.py"
FROZEN_V6_SOURCE_IDENTITIES = {
    "superseded_v6_auditor": {
        "path": str(
            (REPO / "scripts" / "audit_noaa_gfs_multiseason_raw_postrun_v6.py").resolve()
        ),
        "size_bytes": 556_811,
    ...
    "superseded_v6_auditor_test": {
        "path": str(
            (REPO / "tests" / "test_noaa_gfs_multiseason_raw_postrun_v6.py").resolve()
        ),
        "size_bytes": 278_146,
    ...
    "superseded_v6_sealer": {
        "path": str(
            (REPO / "scripts" / "seal_noaa_gfs_multiseason_postrun_audit_v6.py").resolve()
        ),
        "size_bytes": 95_744,
...
BOUND_SOURCE_PATHS = {
    "superseded_v6_auditor": Path(FROZEN_V6_SOURCE_IDENTITIES["superseded_v6_auditor"]["path"]),
    "superseded_v6_auditor_test": Path(FROZEN_V6_SOURCE_IDENTITIES["superseded_v6_auditor_test"]["path"]),
    "superseded_v6_sealer": Path(FROZEN_V6_SOURCE_IDENTITIES["superseded_v6_sealer"]["path"]),
    "superseded_v6_sealer_test": Path(FROZEN_V6_SOURCE_IDENTITIES["superseded_v6_sealer_test"]["path"]),
    "v7_auditor": Path(auditor.__file__).resolve(),
    "v7_auditor_test": auditor.AUDITOR_TEST.resolve(),
    "v7_sealer": Path(__file__).resolve(),
    "v7_sealer_test": SEALER_TEST.resolve(),
}
POSTRUN_REPORT_CANDIDATES = auditor.V7_POSTRUN_REPORT_CANDIDATES
...
SOURCE_ORIGIN_TRUST_MODEL = {
    ...
    "source_compile_exact_executing_v7_four": True,
    "adversarial_concurrent_swap_resistance_claimed": False,
}


class PostrunAuditSealError(RuntimeError):
    """The immutable V7 authorization could not be sealed safely."""
...
def require_canonical_module_entrypoint(root: Path) -> None:
    _require(
        ...
        "seal_noaa_gfs_multiseason_postrun_audit_v7` entrypoint",
    )
    _require(
        os.environ.get("PYTHONDONTWRITEBYTECODE") == "1"
        and sys.dont_write_bytecode is True
...
def _network_forbidden(*_args: Any, **_kwargs: Any) -> Any:
    raise _NetworkDenied("network access is forbidden during V7 authorization sealing")
...
def control_paths(root: Path) -> dict[str, Path]:
    return {
        "authorization": root_path(root, AUTH_RELATIVE, label="V7 authorization"),
        "independent_review": root_path(root, REVIEW_RELATIVE, label="V7 review"),
        "independent_go": root_path(root, GO_RELATIVE, label="V7 GO"),
    }
...
def control_temporary_files(root: Path) -> list[str]:
    ...
    for relative in CONTROL_TEMP_RELATIVES:
        path = root_path(root, relative, label=f"V7 control temp destination {relative}")
        parent = path.parent
...
def validate_control_namespace(
    ...
    for relative, prefix in prefixes.items():
        parent = root_path(root, relative, label=f"V7 namespace {relative}")
        _require(
            _lexists(parent) and parent.is_dir() and not _linklike(parent),
            f"V7 control namespace parent invalid: {relative}",
        )
        matched: list[str] = []
        for entry in parent.iterdir():
            ...
            _require(
                ...
                f"V7 control namespace contains non-regular/link entry: {entry}",
            )
...
def require_prepublication_control_state(root: Path) -> dict[str, Any]:
    ...
    _require(
        ...
        "V7 authorization/review/GO already exists",
    )
    _require(control_temporary_files(root) == [], "V7 control temporary sibling exists")
...
def require_postpublication_control_state(root: Path) -> dict[str, Any]:
    ...
    _require(
        ...
        "published V7 authorization is absent, non-regular or link-like",
    )
    _require(
        not _lexists(paths["independent_review"])
        and not _lexists(paths["independent_go"]),
        "V7 sealer observed an independent review/GO",
    )
    _require(control_temporary_files(root) == [], "V7 control temporary sibling exists")
...
def collect_code_state(workspace_root: Path) -> dict[str, Any]:
    ...
    _require(Path(auditor.V7_SEALER).resolve() == Path(__file__).resolve(), "auditor V7 sealer path mismatch")
    _require(Path(auditor.V7_SEALER_TEST).resolve() == SEALER_TEST.resolve(), "auditor V7 sealer-test path mismatch")
    ...
    for role, expected in FROZEN_V6_SOURCE_IDENTITIES.items():
        _require(bound[role] == expected, f"frozen V6 source identity mismatch: {role}")
    _require(
        bound["v7_auditor"] == EXPECTED_V7_AUDITOR_IDENTITY
        and bound["v7_auditor_test"] == EXPECTED_V7_AUDITOR_TEST_IDENTITY,
        "frozen V7 auditor/test identity mismatch",
    )
...
def build_zero_snapshot(root: Path) -> dict[str, Any]:
    ...
    _require(temporary == [], "V7 control temporary sibling exists")
    ...
    _require(
        set(snapshot) == auditor.V7_ZERO_SNAPSHOT_KEYS,
        "V7 zero-snapshot schema mismatch",
    )
    _require(
        snapshot["v7_control_temporary_files"]
        == {
            "checked_destination_relative_paths": list(CONTROL_TEMP_RELATIVES),
            "matching_temporary_files": [],
        },
        "V7 zero-snapshot temporary-destination declaration mismatch",
    )
    try:
        auditor._v7_validate_zero_snapshot(root, snapshot)
    except Exception as exc:
        raise PostrunAuditSealError(f"V7 zero-snapshot invalid: {exc}") from exc
    return snapshot
...
def collect_root_inputs(root: Path, code_state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    incident = dict(V7_TIMESTAMP_PRECISION_FALSE_REJECT_INCIDENT_IDENTITY)
    v5_false_reject_incident = dict(V6_HISTORICAL_FALSE_REJECT_INCIDENT_IDENTITY)
    historical_contract = expected_historical_failed_head_identity_contract()
    local_timestamp_incident = _load_and_validate_v7_timestamp_precision_incident(
        root, incident
    )
    local_history = _load_and_validate_v6_false_reject_incident(
        root, v5_false_reject_incident, historical_contract
    )
    try:
        auditor_timestamp_incident = auditor._v7_validate_timestamp_false_reject_incident(
            root, incident
        )
        auditor_history_incident = auditor._v6_validate_v5_false_reject_incident(
            root, v5_false_reject_incident
        )
        base_state = auditor._v6_load_and_validate_v5_authority_base(
            root, V5_AUTHORITY_BASE
        )
    except Exception as exc:
        raise PostrunAuditSealError(f"V7 frozen authority input invalid: {exc}") from exc
    _require(
        auditor_timestamp_incident.get("identity") == incident
        and local_timestamp_incident["identity"] == incident
        and auditor_history_incident.get("identity") == v5_false_reject_incident
        and local_history["incident"] == v5_false_reject_incident
        and base_state.get("base") == V5_AUTHORITY_BASE,
        "V7 incident/history/base independent validator result mismatch",
    )
    direct_payload = local_history["historical_incident_payload"]
    try:
        ...
        raise PostrunAuditSealError(
            f"V7 historical 9e74 contract invalid: {exc}"
        ) from exc
    _require(
        auditor_historical == historical_contract
        and local_history["failed_head_count"] == 7
        and local_history["absent_control_path_count"] == 5,
        "V7 local/auditor historical contract mismatch",
    )
    _require(
        local_timestamp_incident.get("historical_created_utc")
        == auditor_timestamp_incident.get("historical_created", {}).get("raw")
        == direct_payload.get("created_utc"),
        "V7 historical 9e74 raw timestamp cross-binding mismatch",
    )
    _require(
        local_timestamp_incident.get("created_utc")
        == auditor_timestamp_incident.get("created", {}).get("raw"),
        "V7 timestamp incident raw created_utc cross-binding mismatch",
    )
    ...
    return {
        "incident": incident,
        "incident_created_utc": local_timestamp_incident["created_utc"],
        "v5_false_reject_incident_created_utc": local_history[
            "incident_payload"
        ]["created_utc"],
...
def _evidence_bound_identities(
    ...
    for role, record in root_bound.items():
        _require(
            ...
            f"V7 evidence root identity mismatch: {role}",
        )
    ...
    _require(
        ...
        "V7 evidence exact-fourteen bound-identity schema mismatch",
    )
    return combined


def _compile_bound_sources(bound: Mapping[str, Mapping[str, Any]]) -> list[dict[str, Any]]:
    ordered_roles = ("v7_auditor", "v7_auditor_test", "v7_sealer", "v7_sealer_test")
...
def run_immutable_test_evidence(
    ...
    auditor_test_names = _source_function_names(Path(str(source_bound["v7_auditor_test"]["path"])))
    _require(set(auditor.V7_REQUIRED_TEST_NAMES).issubset(auditor_test_names), "required V7 regression test is absent")
    ...
    for role, relative in TEST_RELATIVES.items():
        ...
        _require_bound_bytecode_absent(source_bound)
    _require(set(runs) == auditor.V7_TEST_RUN_KEYS, "test-run role mismatch")
    ...
    _require(set(evidence) == auditor.V7_TEST_EVIDENCE_KEYS, "internal V7 test-evidence schema mismatch")
    return evidence


def _validate_v7_test_evidence_local(
    evidence: Any,
    *,
    ...
    _require(
        ...
        and set(evidence) == auditor.V7_TEST_EVIDENCE_KEYS,
        "V7 test-evidence schema mismatch",
    )
    evidence_created_ticks = _exact_utc_z_100ns_ticks(
        evidence.get("created_utc"), label="V7 test evidence created_utc"
    )
    expected_compile = [
    ...
    _require(
        ...
        == list(auditor.V7_REQUIRED_TEST_NAMES)
        and evidence.get("required_test_names_present") is True
        and evidence.get("all_exit_codes_zero") is True,
        "V7 test-evidence header/source binding mismatch",
    )
    runs = evidence.get("test_runs")
    _require(
        isinstance(runs, Mapping) and set(runs) == auditor.V7_TEST_RUN_KEYS,
        "V7 test-evidence run schema mismatch",
    )
    for role, relative in TEST_RELATIVES.items():
        ...
        _require(
            ...
            and set(record)
            == {
                "command", "exit_code", "summary", "stdout_sha256",
                "stderr_sha256", "network_guard_installed",
                "cacheprovider_disabled",
            }
            and record.get("command") == auditor._v7_test_command(relative)
            ...
            f"V7 test-evidence run invalid: {role}",
        )
    expected_shape_keys = {
        "actual_root_historical_predata_validated",
        "actual_root_collect_build_self_validator_validated",
        "actual_timestamp_incident_validated_predata",
        "compact_afterstate_commitment_validated", "decoded_reached",
        "direct_file_failure_incident_9e74_validated_predata",
        "frozen_v6_timestamp_false_reject_reproduced",
        "full_afterstate_reconstructed_in_memory",
        "historical_current_role_distinction_validated", "offline_replay_reached",
        "original_v1_four_key_identity_passed", "raw_reached",
        "rfc3339_100ns_parser_zero_through_seven_exact", "synthetic_fixture",
        "transaction_reached", "v5_authority_base_validated",
        "v5_false_reject_incident_validated",
        "v6_auth_sealer_timestamp_false_reject_incident_validated",
    ...
    _require(
        ...
        "V7 test-evidence production-shape regression mismatch",
    )
    ...
    _require(
        ...
        "V7 test-evidence real-seven regression mismatch",
    )
    _require(
        ...
        "V7 test-evidence safety mismatch",
    )
    try:
        auditor._v7_validate_test_evidence(
            evidence,
            authorization_created={"ticks_100ns": authorization_created_ticks},
            bound_identities=bound_identities,
        )
    except Exception as exc:
        raise PostrunAuditSealError(
            f"V7 independent auditor test-evidence rejection: {exc}"
        ) from exc
...
def build_authorization(
    ...
    *,
    ...
    audit_attempt_id = f"postrun_audit_v7__{compact}"
    ...
    payload = {
        ...
        "superseded_v6_auditor": bound["superseded_v6_auditor"],
        "superseded_v6_auditor_test": bound["superseded_v6_auditor_test"],
        "superseded_v6_sealer": bound["superseded_v6_sealer"],
        "superseded_v6_sealer_test": bound["superseded_v6_sealer_test"],
        "v7_auditor": bound["v7_auditor"],
        "v7_auditor_test": bound["v7_auditor_test"],
        "v7_sealer": bound["v7_sealer"],
        "v7_sealer_test": bound["v7_sealer_test"],
...
def validate_authorization_payload(
    ...
    _require(set(payload) == auditor.V7_AUTH_KEYS, "V7 authorization schema mismatch")
    created_raw = payload.get("created_utc")
    _require(
        ...
        is not None,
        "V7 authorization created_utc is not exact UTC RFC3339 microseconds",
    )
    raw_chronology = {
        "historical_incident_created_utc": inputs.get(
            "historical_incident_created_utc"
        ),
        "v5_authorization_created_utc": inputs.get(
            "v5_authorization_created_utc"
        ),
        "v5_review_created_utc": inputs.get("v5_review_created_utc"),
        "v5_go_created_utc": inputs.get("v5_go_created_utc"),
        "v5_false_reject_incident_created_utc": inputs.get(
            "v5_false_reject_incident_created_utc"
        ),
        "incident_created_utc": inputs.get("incident_created_utc"),
        "authorization_created_utc": created_raw,
    }
    chronology = {
        field: _exact_utc_z_100ns_ticks(value, label=field)
        for field, value in raw_chronology.items()
    }
    for field, value in raw_chronology.items():
        try:
            independent = auditor._v7_parse_rfc3339_utc_100ns(
                value, label=f"independent {field}"
            )
        except Exception as exc:
            raise PostrunAuditSealError(
                f"V7 independent timestamp parse failed: {field}: {exc}"
            ) from exc
        _require(
            independent.get("raw") == value
            and independent.get("ticks_100ns") == chronology[field],
            f"V7 local/auditor timestamp parse mismatch: {field}",
        )
    ordered_fields = tuple(raw_chronology)
    _require(
        all(
            chronology[left] < chronology[right]
            for left, right in zip(ordered_fields, ordered_fields[1:])
        ),
        "9e74/V5 AUTH-REVIEW-GO/09b9/722f/V7 authorization chronology mismatch",
    )
    expected_attempt_id = "postrun_audit_v7__" + created_raw.translate(
        str.maketrans("", "", "-:.")
    )
    _require(
        ...
        and re.fullmatch(
            r"postrun_audit_v7__[0-9]{8}T[0-9]{12}Z",
            str(payload.get("audit_attempt_id")),
        )
        is not None,
        "V7 authorization header/attempt mismatch",
    )
    try:
        auditor._v7_validate_policy(payload, label="V7 authorization sealer")
    except Exception as exc:
        raise PostrunAuditSealError(f"V7 authorization policy invalid: {exc}") from exc

    bound = code_state.get("bound_identities")
    _require(
        isinstance(bound, Mapping) and set(bound) == BOUND_SOURCE_ROLES,
        "V7 authorization bound-source role schema mismatch",
    )
    for role, expected in FROZEN_V6_SOURCE_IDENTITIES.items():
        _require(bound[role] == expected, f"frozen V6 source changed: {role}")
    _require(
        bound["v7_auditor"] == EXPECTED_V7_AUDITOR_IDENTITY
        and bound["v7_auditor_test"] == EXPECTED_V7_AUDITOR_TEST_IDENTITY,
        "frozen V7 auditor/test identity mismatch",
    ...
    _require(
        ...
        "V7 authorization source identity binding mismatch",
    )
    _require(
        ...
        "V7 authorization immutable execution binding mismatch",
    )
    _require(
        ...
        "V7 incident/base/historical contract binding mismatch",
    )
    local_timestamp = _load_and_validate_v7_timestamp_precision_incident(
    ...
    try:
        auditor_timestamp = auditor._v7_validate_timestamp_false_reject_incident(
            root, payload["incident"]
        )
        auditor_history_incident = auditor._v6_validate_v5_false_reject_incident(
            root, V6_HISTORICAL_FALSE_REJECT_INCIDENT_IDENTITY
        )
        base_state = auditor._v6_load_and_validate_v5_authority_base(
            root, payload["v5_authority_base"]
        )
        auditor_historical = auditor._v6_validate_historical_9e74_payload(
            local_history["historical_incident_payload"],
            HISTORICAL_DIRECT_FILE_FAILURE_INCIDENT_IDENTITY,
        )
        auditor._v6_validate_historical_contract(
            payload["historical_failed_head_identity_contract"],
        ...
        raise PostrunAuditSealError(
            f"V7 authority incident/history/base revalidation failed: {exc}"
        ) from exc
    _require(
        local_timestamp.get("identity") == payload["incident"]
        and auditor_timestamp.get("identity") == payload["incident"]
        and local_history.get("incident")
        == V6_HISTORICAL_FALSE_REJECT_INCIDENT_IDENTITY
        and auditor_history_incident.get("identity")
        == V6_HISTORICAL_FALSE_REJECT_INCIDENT_IDENTITY
        and base_state.get("base") == payload["v5_authority_base"]
        and auditor_historical
        == payload["historical_failed_head_identity_contract"],
        "V7 independent local/auditor authority input mismatch",
    )
    _require(
        local_history["historical_incident_payload"].get("created_utc")
        == local_timestamp.get("historical_created_utc")
        == auditor_timestamp.get("historical_created", {}).get("raw")
        == raw_chronology["historical_incident_created_utc"]
        and base_state["authorization"].get("created_utc")
        == raw_chronology["v5_authorization_created_utc"]
        and base_state["review"].get("created_utc")
        == raw_chronology["v5_review_created_utc"]
        and base_state["go"].get("created_utc")
        == raw_chronology["v5_go_created_utc"]
        and local_history["incident_payload"].get("created_utc")
        == auditor_history_incident.get("payload", {}).get("created_utc")
        == raw_chronology["v5_false_reject_incident_created_utc"]
        and local_timestamp.get("created_utc")
        == auditor_timestamp.get("created", {}).get("raw")
        == raw_chronology["incident_created_utc"],
    ...
    _require(
        ...
        == auditor.V5_RECOVERED_AFTERSTATE_COMMITMENT,
        "V7 compact recovered-afterstate commitment binding mismatch",
    )
    _require(
        ...
        == auditor.V7_ZERO_SNAPSHOT_KEYS,
        "V7 preaudit zero-snapshot binding mismatch",
    )
    ...
    expected_evidence_bound = {
        ...
        "direct_file_failure_incident": HISTORICAL_DIRECT_FILE_FAILURE_INCIDENT_IDENTITY,
        "v6_auth_sealer_timestamp_precision_false_reject_incident": inputs[
            "incident"
        ],
    ...
    _require(
        ...
        "V7 authorization exact-fourteen evidence binding mismatch",
    )
    _validate_v7_test_evidence_local(
        evidence,
        authorization_created_ticks=chronology["authorization_created_utc"],
        bound_identities=expected_evidence_bound,
    )
    try:
        auditor._v7_validate_zero_snapshot(
            root, payload["preaudit_zero_mutation_snapshot"]
        )
        auditor._v7_assert_compact_control(payload, label="V7 authorization sealer")
    except Exception as exc:
        raise PostrunAuditSealError(f"V7 authorization evidence invalid: {exc}") from exc
...
def static_audit(root: Path, workspace_root: Path = REPO) -> dict[str, Any]:
    ...
    return {
        "status": "PASS_V7_AUTHORIZATION_PRESEAL_STATIC_AUDIT",
        "artifact_type": "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDIT_V7_AUTH_SEALER_STATIC_AUDIT",
...
def seal(root: Path, workspace_root: Path = REPO) -> dict[str, Any]:
    ...
    try:
        observed_identity = strict_root_identity(
            root, AUTH_RELATIVE, label="published V7 authorization"
        )
        _require(
        ...
        _require(
            load_json_control(destination, label="published V7 authorization")
            == authorization,
            "published authorization payload mismatch",
        )
        try:
            strict_published = auditor._v5_load_strict_json(
                destination, label="published V7 authorization"
            )
        except Exception as exc:
            raise PostrunAuditSealError(
                f"published V7 authorization strict JSON rejection: {exc}"
            ) from exc
        _require(
    ...
    return {
        "status": "PASS_V7_AUTHORIZATION_SEALED_PENDING_INDEPENDENT_REVIEW_AND_GO",
        "artifact_type": "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDIT_V7_AUTH_SEAL_REPORT",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        report = {
            "status": "FAIL_V7_POSTRUN_AUDIT_AUTHORIZATION_POSTPUBLICATION",
        ...
        report = {
            "status": "FAIL_V7_POSTRUN_AUDIT_AUTHORIZATION_SEAL",
            "error_type": type(exc).__name__,
            "error": str(exc),
            "authorization_files_published": 0,
            "independent_review_files_published": 0,
