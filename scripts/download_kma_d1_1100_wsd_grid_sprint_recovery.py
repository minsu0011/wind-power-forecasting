...
EXECUTION_CONFIG = PROJECT / "configs" / "kma_d1_1100_wsd_sprint_execution_v2_recovery.json"
NX = 149
NY = 253
TOKEN_COUNT = NX * NY
CELLS = ((94, 121), (94, 122))
...
RECOVERY_UNPERSISTED_TRANSFER_COUNT = 2
RECOVERY_UNPERSISTED_RESPONSE_BYTES_KNOWN = 682_594
RECOVERY_SEQUENCE = 4_861
RECOVERY_RESPONSE_BYTES = 341_297
RECOVERY_RESPONSE_SHA256 = "2ff9e1ab475105d61e135c518a90a6f1493c94c8f83abcb297c0c8eb64f8aea9"
...
def parse_grid(payload: bytes) -> tuple[np.ndarray, dict[str, Any]]:
    ...
    selected_valid = np.isfinite(selected) & (selected >= 0.0) & (selected <= 75.0)
    facts = {
        ...
        "selected_cells_valid_0_75": [bool(value) for value in selected_valid],
        "selected_cells_all_valid_0_75": bool(selected_valid.all()),
    }
    return selected, facts


def verify_frozen_recovery_response(spec: RequestSpec, payload: bytes, facts: dict[str, Any]) -> None:
    if spec.sequence != RECOVERY_SEQUENCE:
        return
    if len(payload) != RECOVERY_RESPONSE_BYTES or sha256_bytes(payload) != RECOVERY_RESPONSE_SHA256:
        raise RuntimeError("sequence 4861 response drifted from frozen diagnostic identity")
    expected = {
        "numeric_token_count": 37_697,
        "sentinel_le_minus_99_count": 37_697,
        "wsd_cell_94_121": -99.0,
        "wsd_cell_94_122": -99.0,
        "selected_cells_valid_0_75": [False, False],
        "selected_cells_all_valid_0_75": False,
    }
    for key, value in expected.items():
        if facts.get(key) != value:
            raise RuntimeError(f"sequence 4861 frozen diagnostic mismatch: {key}")
...
def read_existing(spec: RequestSpec) -> dict[str, Any] | None:
    ...
    verify_frozen_recovery_response(spec, payload, facts)
    ...
    for key in ("selected_cells_valid_0_75", "selected_cells_all_valid_0_75"):
        if key in metadata and facts[key] != metadata[key]:
            raise RuntimeError(f"existing selected-cell validity mismatch {spec.sequence}: {key}")
        if spec.sequence >= RECOVERY_SEQUENCE and key not in metadata:
            raise RuntimeError(f"recovery metadata missing selected-cell validity {spec.sequence}: {key}")
...
def verify_execution_config() -> dict[str, Any]:
    ...
    if payload.get("schema_version") != 3:
        raise RuntimeError("recovery execution config schema mismatch")
    script = Path(__file__).resolve()
    if payload["script_identity"] != {
        "path": "scripts/download_kma_d1_1100_wsd_grid_sprint_recovery.py",
        "bytes": script.stat().st_size,
        "sha256": sha256_file(script),
    }:
        raise RuntimeError("execution config does not bind current downloader")
    for declared in [payload.get("parent_execution_config"), *payload.get("preregistration_chain", [])]:
        if not isinstance(declared, dict):
            raise RuntimeError("recovery execution config identity declaration missing")
        path = PROJECT / str(declared.get("path"))
        actual = {"path": str(declared.get("path")), "bytes": path.stat().st_size, "sha256": sha256_file(path)}
        if actual != declared:
            raise RuntimeError(f"recovery execution input identity mismatch: {path.name}")
    ...
    if payload.get("unpersisted_transfer_count_before_recovery") != RECOVERY_UNPERSISTED_TRANSFER_COUNT:
        raise RuntimeError("execution config recovery-attempt count mismatch")
    return payload
...
def run_batch(specs: list[RequestSpec], *, workers: int) -> list[dict[str, Any]]:
    ...
    for start in range(0, len(specs), workers):
        ...
        cumulative = sum(int(item["response_bytes"]) for item in results) + RECOVERY_UNPERSISTED_RESPONSE_BYTES_KNOWN
...
def summarize(results: list[dict[str, Any]], *, mode: str) -> dict[str, Any]:
    ...
    invalid_sequences = [
        int(item["sequence"])
        for item in results
        if not bool(item.get("selected_cells_all_valid_0_75", True))
    ]
    recovery_adjusted_projection = max(sizes) * (GRAND_CALLS + RECOVERY_UNPERSISTED_TRANSFER_COUNT)
    return {
        "schema_version": 1,
        "mode": mode,
        "records": len(results),
        ...
        "byte_gate_pass": recovery_adjusted_projection <= BYTE_STOP,
        "recovery_unpersisted_transfer_count": RECOVERY_UNPERSISTED_TRANSFER_COUNT,
        "recovery_unpersisted_response_bytes_known": RECOVERY_UNPERSISTED_RESPONSE_BYTES_KNOWN,
        "projected_total_transfer_calls_including_recovery": GRAND_CALLS + RECOVERY_UNPERSISTED_TRANSFER_COUNT,
        "recovery_adjusted_projected_bytes_by_max": recovery_adjusted_projection,
        "quota_response_bytes_accounted_current_mode": sum(sizes) + RECOVERY_UNPERSISTED_RESPONSE_BYTES_KNOWN,
        ...
        "invalid_selected_anchor_count": len(invalid_sequences),
        "invalid_selected_anchor_sequences": invalid_sequences,
