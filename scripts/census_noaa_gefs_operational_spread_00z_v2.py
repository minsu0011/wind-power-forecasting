#!/usr/bin/env python3
"""Read-only 2022-2024 NOAA operational GEFS 00Z object/index census.
...
PROTOCOL_PATH = PROJECT_ROOT / "configs" / "noaa_gefs_operational_spread_00z_census_protocol_v2.json"
OUTPUT_ROOT = PROJECT_ROOT / "artifacts" / "external" / "noaa_gefs_operational_spread_00z_census_v2"
HOST = "noaa-gefs-pds.s3.amazonaws.com"
USER_AGENT = "baram2026-noaa-gefs-readonly-census-00z-v2/2.0"
...
MAX_UNAVAILABLE_DAYS = 5
MAX_FAILED_OBJECTS = 180
...
PRODUCTS = (
    Product("surface_0p25_ens_mean", "pgrb2sp25", "geavg.t00z.pgrb2s.0p25", "10 m above ground", "ens mean"),
    Product("surface_0p25_ens_spread", "pgrb2sp25", "gespr.t00z.pgrb2s.0p25", "10 m above ground", "ens std dev"),
    Product("pressure_0p50_ens_mean", "pgrb2ap5", "geavg.t00z.pgrb2a.0p50", "850 mb", "ens mean"),
    Product("pressure_0p50_ens_spread", "pgrb2ap5", "gespr.t00z.pgrb2a.0p50", "850 mb", "ens std dev"),
)
...
OFFICIAL_SOURCES = (
    ...
    (
        "aws_registry_dynamical_gefs",
        "https://registry.opendata.aws/dynamical-noaa-gefs/",
        ("dynamical-noaa-gefs", "CC BY 4.0", "Managed By"),
    ),
    (
        "dynamical_gefs_catalog",
        "https://dynamical.org/catalog/noaa-gefs-forecast-35-day/",
        ("2020-10-01", "wind_u_10m", "processed by dynamical.org"),
    ),
    (
        "dacon_rules",
...
def data_key(source_day: date, product: Product, lead: int) -> str:
    return (
        f"/gefs.{source_day:%Y%m%d}/00/atmos/{product.directory}/"
...
def parse_index(
    ...
    expected_init = f"d={source_day:%Y%m%d}00"
...
def main() -> int:
    ...
    if sha256_file(PROTOCOL_PATH) != "4860050db45c512d9f5417b5cbcb35aa3153420f03aa2356692e4a48de386bd6":
    ...
    for row in results:
        rows_by_day.setdefault(row["operating_date"], []).append(row)
    availability_days: list[dict[str, Any]] = []
    for day in days:
        day_text = day.isoformat()
        subset = rows_by_day.get(day_text, [])
        available = len(subset) == len(PRODUCTS) * len(LEADS) and all(
            bool(row.get("overall_pass")) for row in subset
        )
        availability_days.append(
            {
                "operating_date": day_text,
                "available": available,
                "missing_gefs_00z": 0 if available else 1,
                "observed_objects": len(subset),
                "failed_objects": sum(not bool(row.get("overall_pass")) for row in subset),
                "later_paired_increment_rule": "model_candidate" if available else "exact_zero_identity",
            }
        )
    unavailable_days = [item for item in availability_days if not item["available"]]
    all_rows_accounted = (
        len(results) == EXPECTED_OBJECT_COUNT
        and len(rows_by_day) == EXPECTED_DAY_COUNT
        and all(item["observed_objects"] == len(PRODUCTS) * len(LEADS) for item in availability_days)
    )
    source_gate_pass = bool(
        all_rows_accounted
        and len(unavailable_days) <= MAX_UNAVAILABLE_DAYS
        and len(failures) <= MAX_FAILED_OBJECTS
    )
    global_pass = source_gate_pass and provenance_pass

    availability_path = OUTPUT_ROOT / "operating_day_availability.json"
    availability_payload = {
        "schema_version": 1,
        "mask_id": "noaa_gefs_operational_spread_00z_census_v2_operating_day_availability",
        "created_utc": retrieved_utc,
        "protocol": file_identity(PROTOCOL_PATH),
        "contract": {
            "objects_per_day": len(PRODUCTS) * len(LEADS),
            "available_if": "all frozen product/lead objects pass status, semantics, data cutoff, and index cutoff",
            "unavailable_effect": "missing_gefs_00z=1 and later paired increment exact zero for all 24 rows",
            "maximum_unavailable_days": MAX_UNAVAILABLE_DAYS,
            "maximum_failed_objects": MAX_FAILED_OBJECTS,
        },
        "counts": {
            "operating_days": len(availability_days),
            "available_days": len(availability_days) - len(unavailable_days),
    ...
    availability_path.write_text(
        json.dumps(availability_payload, ensure_ascii=False, indent=2, sort_keys=True) + "\n",
        encoding="utf-8",
    )
    summary = {
        "schema_version": 1,
        "census_id": "noaa_gefs_operational_spread_00z_census_v2",
        "created_utc": retrieved_utc,
        "status": "PASS_SOURCE_GATE_WITH_PREDECLARED_IDENTITY_FALLBACK" if global_pass else "NO_GO_SOURCE_GATE_FAILED",
        "protocol": file_identity(PROTOCOL_PATH),
        "scope": {
        ...
        "coverage": {
            ...
            "failed_objects": len(failures),
            "maximum_failed_objects": MAX_FAILED_OBJECTS,
            "available_operating_days": len(availability_days) - len(unavailable_days),
            "unavailable_operating_days": len(unavailable_days),
            "maximum_unavailable_operating_days": MAX_UNAVAILABLE_DAYS,
            "unavailable_day_fraction": len(unavailable_days) / len(availability_days),
            "availability_mask": file_identity(availability_path),
            "unavailable_day_records": unavailable_days,
        ...
        "official_provenance": {
            ...
            "member_documentation_caveat": "NCEI/AWS landing prose contains a stale legacy 21-member description; EMC operational GEFSv12 and NCO raw-product semantics govern 2022-2024. This census consumes only geavg/gespr aggregate products and does not reconstruct members.",
            "transformed_archive_caveat": "Dynamical's 2026-created transformed archive is not treated as historical publication evidence. NOAA NODD object/index metadata govern availability; 80 m and 100 m transformed winds are excluded because aggregate pgrb2b provenance is unavailable."
        },
        "decision": {
            "global_pass": global_pass,
            "model_preregister_allowed": global_pass,
            "fit_or_score_allowed_by_this_census": False,
            "if_fail": "NO-GO; do not raise the missing-day/object thresholds or switch cycle, lead, product, mirror, or fill.",
            "if_pass": "Freeze one separate paired-increment preregistration before any label read, fit, prediction, or score. The year-generic unavailable-day rule remains exact-zero identity."
        },
    }
    ...
    manifest = {
        "schema_version": 1,
        "manifest_id": "noaa_gefs_operational_spread_00z_census_v2_manifest",
        "created_utc": retrieved_utc,
        ...
        "artifacts": [
            ...
            file_identity(availability_path),
