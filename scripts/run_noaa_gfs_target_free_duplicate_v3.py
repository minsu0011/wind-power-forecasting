"""Target-free duplicate diagnostic V3 runner.

This module implements the append-only V3 contract together with the exact
two-pointer documentary correction in
prereg/target_free_multiseason_amendment_v4.json. Importing the module is
side-effect free. In particular, import never opens Parquet data, fits a
...
OUTPUT_ROOT_RELATIVE: Final[str] = "modeling/target_free_multiseason_v3"
TRANSACTION_DIRNAME: Final[str] = ".transaction_v3"
...
class ContractError(RuntimeError):
    """Base class for a deterministic V3 contract violation."""
...
def strict_json_dumps(value: Any, *, pretty: bool) -> bytes:
    """Serialize finite JSON using the V3 output serializer."""
...
CODE_SEAL_RELATIVE_PATH: Final[str] = (
    "prereg/target_free_duplicate_code_seal_v3.json"
)
EXECUTION_AUTHORIZATION_RELATIVE_PATH: Final[str] = (
    "prereg/target_free_duplicate_execution_authorization_v3.json"
)
...
POSTRUN_PASS_RELATIVE_PATH: Final[str] = (
    "independent_redteam/TRACK_A_TARGET_FREE_DUPLICATE_POSTRUN_PASS_V3.json"
)
...
CONTROL_OUTPUT_NAMESPACE: Final[Mapping[str, Any]] = {
    "root": "modeling/target_free_multiseason_v3/",
    "runner_transaction_root": (
        "modeling/target_free_multiseason_v3/.transaction_v3/"
    ),
...
TEST_FILES: Final[Mapping[str, str]] = {
    "runner": "tests/test_noaa_gfs_target_free_duplicate_v3.py",
    "auditor": "tests/test_audit_noaa_gfs_target_free_duplicate_v3.py",
    "sealer": "tests/test_seal_noaa_gfs_target_free_duplicate_v3.py",
}
...
def validate_family_decision_document(document: Mapping[str, Any]) -> None:
    ...
    if record["artifact_type"] != "TARGET_FREE_FAMILY_DECISION_V3":
...
def required_control_commands(artifact_root: Path) -> dict[str, list[str]]:
    ...
    modules = {
        "runner": "scripts.run_noaa_gfs_target_free_duplicate_v3",
        "auditor": "scripts.audit_noaa_gfs_target_free_duplicate_v3",
        "sealer": "scripts.seal_noaa_gfs_target_free_duplicate_v3",
    }
...
def validate_actual_runner_command(
    ...
    if not (
        ...
        == ["-B", "-m", "scripts.run_noaa_gfs_target_free_duplicate_v3"]
...
def _validate_control_envelope(role: str, record: Mapping[str, Any]) -> None:
    ...
    _require_exact_tree(record["schema_version"], 3, f"{role} schema_version")
...
def validate_control_chronology(
    ...
    if (
        not (v3_ticks < v4_ticks < completed_ticks <= seal_ticks)
        or any(
            left >= right
...
def validate_execution_control_chain(
    ...
    *,
    ...
    validate_control_chronology(amendment, amendment_v4, records)
...
def _validate_parquet_names_types_rows(
    ...
    *,
    ...
    try:
        ...
        observed_types = tuple(str(field.type) for field in schema)
        if observed_columns != tuple(expected_columns):
            raise IntegrityError(f"Parquet column order differs: {path}")
        if observed_types != tuple(expected_type_names):
            raise IntegrityError(
                f"Parquet type vector differs: {path}: {observed_types!r}"
            )
...
def prepare_execution(
    ...
    *,
    ...
    validation = dataclasses.replace(
        ...
        bound_input_identity_and_schema_pass=True,
    )
    authorization = PredataAuthorization(
        validation,
        artifact_root=root,
    ...
    create_v3_namespace_exclusive(root)
...
def validate_fit_ledger_document(document: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or record["artifact_type"] != "TARGET_FREE_FIT_LEDGER_V3"
...
def stage_serialized_runner_artifacts(
    ...
    *,
    ...
    if _linklike(output_root) or not output_root.is_dir():
        raise OutputPublicationError("V3 output root is absent")
...
ALIAS_SPECS: Final[tuple[AliasSpec, ...]] = (
    AliasSpec(
        ...
        stage_relative_path=(
            ...
            ".alias_stage_FIELD_CENSUS_LOCK_v3"
        ),
        final_relative_path=(
    ...
    AliasSpec(
        ...
        stage_relative_path=(
            "modeling/target_free_multiseason_v3/"
            ".alias_stage_PROVENANCE_LEDGER_v3"
        ),
        final_relative_path=(
            "modeling/target_free_multiseason_v3/PROVENANCE_LEDGER.parquet"
        ),
...
def assert_v3_namespace_absent(artifact_root: Path) -> Path:
    root = artifact_root.resolve(strict=True)
    output_root = root / OUTPUT_ROOT_RELATIVE
    _assert_output_path_no_linklike(
        output_root.parent, "V3 output namespace parent"
    )
    if _lexists(output_root):
        raise OutputPublicationError("V3 output namespace must be absent")
    return output_root


def create_v3_namespace_exclusive(artifact_root: Path) -> Path:
    output_root = assert_v3_namespace_absent(artifact_root)
    try:
        output_root.mkdir(parents=False, exist_ok=False)
    except OSError as exc:
        raise OutputPublicationError("exclusive V3 namespace creation failed") from exc
    if _linklike(output_root):
        raise OutputPublicationError("created V3 namespace is link-like")
    return output_root
...
def publish_exact_byte_alias(
    ...
    *,
    ...
    try:
        ...
        raise OutputPublicationError(
            "alias publication failed; incident and V4 are required"
        ) from exc
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the sealed target-free duplicate V3 diagnostic"
    )
