#!/usr/bin/env python3
"""Fresh-namespace v10 wrapper around the frozen GEFS Stage1 model runner.
...
authorized v10 source/model/tombstone/quarantine namespaces.
"""
...
V7_FAILURE_SEAL_SHA = "797eb8188f4f8726f1941efdfd194877a77c0018c1bd5a8ee22f958f60bdc7ba"
...
V10_AUTHORIZATION_SHA = "0ef28e4d641b6a11291d974cbcd329a7d25ba3be0a47c46737e7a855b0aa0d68"
FRESH_SOURCE = PROJECT_ROOT / "artifacts/external/noaa_gefs_operational_spread_00z_original_v10/stage1_source_through_operating_2023"
FRESH_STAGE2_SOURCE = PROJECT_ROOT / "artifacts/external/noaa_gefs_operational_spread_00z_original_v10/stage2_source_operating_2024"
FRESH_SOURCE_LOCK = PROJECT_ROOT / "artifacts/audits/noaa_gefs_operational_spread_00z_original_v10_stage1_launch_lock.json"
FRESH_MODEL_OUTPUT = PROJECT_ROOT / "artifacts/postgate/noaa_gefs_operational_spread_00z_paired_increment_strict_v10"
FRESH_MODEL_TOMBSTONE = PROJECT_ROOT / "artifacts/audits/noaa_gefs_operational_spread_00z_paired_increment_v10_stage1_single_attempt.json"
FRESH_EXTRACTOR = PROJECT_ROOT / "scripts/download_noaa_gefs_operational_spread_00z_original_v10.py"
FRESH_FOCUSED_TEST = PROJECT_ROOT / "tests/test_noaa_gefs_operational_spread_00z_paired_increment_v10.py"
FRESH_POSTRUN_TEST = PROJECT_ROOT / "tests/test_noaa_gefs_operational_spread_00z_paired_increment_v10_postrun.py"
...
def _load_frozen_core() -> types.ModuleType:
    ...
    substitutions = (
        (
            ...
            '"noaa_gefs_operational_spread_00z_paired_increment_strict_v10"',
            2,
        ),
        (
            '"noaa_gefs_operational_spread_00z_paired_increment_v5_stage1_single_attempt"',
            '"noaa_gefs_operational_spread_00z_paired_increment_v10_stage1_single_attempt"',
            1,
        ),
        (
            '"noaa_gefs_operational_spread_00z_paired_increment_v5_partial_*"',
            '"noaa_gefs_operational_spread_00z_paired_increment_v10_partial_*"',
            1,
        ),
        (
            'f"noaa_gefs_operational_spread_00z_paired_increment_v5_partial_{os.getpid()}_{uuid.uuid4().hex}"',
            'f"noaa_gefs_operational_spread_00z_paired_increment_v10_partial_{os.getpid()}_{uuid.uuid4().hex}"',
            1,
        ),
        (
            'f"noaa_gefs_operational_spread_00z_paired_increment_v5_stage1_failure_{attempt_token}.json"',
            'f"noaa_gefs_operational_spread_00z_paired_increment_v10_stage1_failure_{attempt_token}.json"',
            1,
        ),
        (
            ...
            "for source in (BASE_PREREG, V2_PREREG, V3_PREREG, V4_PREREG, V5_PREREG, V6_PREREG, V7_PREREG, V8_PREREG, V9_PREREG, V10_PREREG, V2_FAILURE_SEAL, V7_SYNTHETIC_INCIDENT, V7_FAILURE_SEAL, V10_AUTHORIZATION, EXECUTION_PROTOCOL, INCIDENT, SOURCE_LAUNCH_LOCK, SOURCE_LAUNCH_LOCK_SHA):",
            1,
        ),
    )
    ...
    module = types.ModuleType("_noaa_gefs_v10_frozen_core")
...
core.V7_FAILURE_SEAL = V7_FAILURE_SEAL
core.V10_AUTHORIZATION = V10_AUTHORIZATION
...
def verify_effective_preregister() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if _sha256_file(V6_PREREG) != V6_SHA or _sha256_file(V2_FAILURE_SEAL) != V2_FAILURE_SEAL_SHA:
        raise AssertionError("v10 authorization/failure lineage differs")
    if _sha256_file(V10_PREREG) != V10_SHA or _sha256_file(V7_FAILURE_SEAL) != V7_FAILURE_SEAL_SHA or _sha256_file(V10_AUTHORIZATION) != V10_AUTHORIZATION_SHA:
        raise AssertionError("v10 source-boundary lineage differs")
    ...
    if v10.get("supersedes", {}).get("sha256") != _sha256_file(V9_PREREG):
        raise AssertionError("v10-v9 lineage differs")
    science = v10.get("immutable_scientific_inheritance", {})
    if science.get("candidate_id") != "gefs00z_0p50_10m_850_mean_spread_w025" or science.get("blend_weight") != 0.25:
        raise AssertionError("v10 scientific candidate differs")
    if v10.get("fresh_non_scientific_namespace", {}).get("model_output") != FRESH_MODEL_OUTPUT.relative_to(PROJECT_ROOT).as_posix():
        raise AssertionError("v10 model output path differs")
    return result
...
def verify_source_launch_lock() -> dict[str, Any]:
    ...
    if not path.is_file() or not sidecar.is_file():
        raise FileNotFoundError("fresh v10 source launch lock/sidecar absent")
    expected = f"{core.sha256_file(path)}  {path.name}\n"
    if sidecar.read_text(encoding="ascii") != expected:
        raise AssertionError("fresh v10 source launch-lock sidecar differs")
    lock = json.loads(path.read_text(encoding="utf-8"))
    if lock.get("status") != "FROZEN_V10_AFTER_OFFLINE_REGRESSION_AND_INDEPENDENT_REVIEW_BEFORE_FULL_STAGE1_RANGE_GET":
        raise AssertionError("fresh v10 source launch-lock status differs")
    if lock.get("scope", {}).get("operating_2024_values") != 0 or lock.get("preflight_payload_reuse") != 0:
        raise AssertionError("fresh v10 source scope/reuse differs")
    ...
    if len(runner_records) != 1 or runner_records[0]["sha256"] != core.sha256_file(Path(__file__)):
        raise AssertionError("fresh v10 model runner was not pre-network bound")
    return lock
...
def verify_source_canonical() -> dict[str, Any]:
    if core.STAGE2_SOURCE.exists():
        raise AssertionError("fresh v10 conditional Stage2 source exists")
    manifest_path = core.SOURCE_ROOT / "manifest.json"
    if not manifest_path.is_file():
        raise FileNotFoundError("fresh v10 Stage1 source canonical absent")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("status") != "PASS_V10_STAGE1_SOURCE_EXTRACTION":
        raise AssertionError("fresh v10 source did not pass")
    core._assert_records_current(manifest["source_closure"])
    core._assert_records_current(manifest["artifacts"])
    expected_nonmutation = {"stage2_namespace_created": False, "label_model_metric_csv_writes": 0, "old_v2_partial_reused": 0, "terminal_v7_partial_reused": 0}
    if manifest.get("nonmutation") != expected_nonmutation:
        raise AssertionError("fresh v10 source nonmutation differs")
    return manifest
...
def static_audit() -> dict[str, Any]:
    ...
    result.update({
        "status": "PASS_ACTUAL_STAGE1_RUNNER_STATIC_CONTRACT_V10_FRESH_NAMESPACE",
        "config_v6_sha256": _sha256_file(V6_PREREG),
        "config_v7_sha256": _sha256_file(V7_PREREG),
        "config_v10_sha256": _sha256_file(V10_PREREG),
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.raw_dir.resolve() != Path(r"data/local/open").resolve():
        raise AssertionError("fresh v10 raw input root differs")
    if args.out_dir.resolve() != FRESH_MODEL_OUTPUT.resolve():
        raise AssertionError("fresh v10 model output differs")
