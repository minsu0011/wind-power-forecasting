"""Seal the V3 target-free duplicate experiment without interpreting model data.

This module has two deliberately separate roles:

* code-seal mode validates the frozen V3 amendment, the exact six code/test
  identities, and externally supplied isolated-test evidence, then publishes
  only the code-seal control;
...
CODE_SEAL_RELATIVE = "prereg/target_free_duplicate_code_seal_v3.json"
AUTHORIZATION_RELATIVE = "prereg/target_free_duplicate_execution_authorization_v3.json"
REVIEW_RELATIVE = (
    "independent_redteam/TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_REVIEW_V3.json"
)
GO_RELATIVE = "independent_redteam/TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_GO_V3.json"
POSTRUN_RELATIVE = "independent_redteam/TRACK_A_TARGET_FREE_DUPLICATE_POSTRUN_PASS_V3.json"

OUTPUT_ROOT_RELATIVE = "modeling/target_free_multiseason_v3/"
TRANSACTION_ROOT_RELATIVE = "modeling/target_free_multiseason_v3/.transaction_v3/"
...
CONTROL_SPECS = {
    "code_seal": {
        "keys": frozenset(
            {
                "schema_version", "artifact_type", "status", "created_utc",
                "amendment", "amendment_v4", "code_identities", "test_evidence", "output_namespace",
                "authority_scope", "required_next_controls", "publisher",
            }
        ),
        "artifact_type": "TARGET_FREE_DUPLICATE_CODE_SEAL_V3",
        "status": "SEALED_V3_CODE_AND_TESTS_NO_EXECUTION_AUTHORITY",
        "publisher": "V3_SEALER_CODE_SEAL_MODE_CREATE_IF_ABSENT_ONLY",
    ...
    "authorization": {
        "keys": frozenset(
            {
                "schema_version", "artifact_type", "status", "created_utc",
                "attempt_id", "amendment", "amendment_v4", "code_seal", "code_identities",
                "output_namespace", "execution_budget", "required_commands",
                "required_review", "required_go", "authority_scope", "publisher",
            }
        ),
        "artifact_type": "TARGET_FREE_DUPLICATE_EXECUTION_AUTHORIZATION_V3",
        "status": "AUTHORIZED_V3_SINGLE_ATTEMPT_PENDING_INDEPENDENT_REVIEW_AND_GO",
    ...
    "review": {
        "keys": frozenset(
            {
                "schema_version", "artifact_type", "status", "created_utc",
                "attempt_id", "amendment", "amendment_v4", "code_seal", "authorization",
                "code_identities", "output_namespace", "independent_checks",
                "authority_scope", "verdict", "publisher",
            }
        ),
        "artifact_type": "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_REVIEW_V3",
        "status": "PASS_V3_EXECUTION_REVIEW_PENDING_INDEPENDENT_GO",
    ...
    "go": {
        ...
        "artifact_type": "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_GO_V3",
        "status": "GO_V3_SINGLE_TARGET_FREE_ATTEMPT",
    ...
    "postrun": {
        ...
        "artifact_type": "TRACK_A_TARGET_FREE_DUPLICATE_POSTRUN_PASS_V3",
        "status": "PASS_V3_NO_FIT_POSTRUN_AUDIT_PENDING_FINAL_SEAL",
...
REQUIRED_REVIEW = {
    ...
    "status_required": "PASS_V3_EXECUTION_REVIEW_PENDING_INDEPENDENT_GO",
}
REQUIRED_GO = {
    "path": GO_RELATIVE,
    "status_required": "GO_V3_SINGLE_TARGET_FREE_ATTEMPT",
}
...
AUDITOR_STDOUT_KEYS = frozenset(
    {
        "schema_version", "artifact_type", "status", "attempt_id", "amendment", "amendment_v4",
...
ATTEMPT_ID = re.compile(
    r"^target_free_duplicate_v3__[0-9]{8}T[0-9]{12}Z$"
)
...
class TargetFreeSealError(RuntimeError):
    """A fail-closed V3 sealer contract check failed."""
...
class PartialPublicationError(TargetFreeSealError):
    """A V3 publication became partial and therefore requires V4 supersession."""
...
def require_attempt_id(value: Any, *, label: str) -> str:
    ...
    timestamp = value.removeprefix("target_free_duplicate_v3__")
    try:
...
def _validate_v3_amendment(artifact_root: Path) -> dict[str, Any]:
    ...
    _require(authority.get("authorized_code_and_test_paths") == list(CODE_ROLE_PATHS.values()), "V3 code/test path order mismatch")

    output = payload.get("output_contract")
    _require(isinstance(output, Mapping), "V3 output contract absent")
    _require(output.get("output_root") == OUTPUT_ROOT_RELATIVE, "V3 output root mismatch")
    _require(output.get("positive_complete_paths_exact") == [f"{OUTPUT_ROOT_RELATIVE}{name}" for name in EXACT11_SCHEMA_ORDER], "V3 positive exact11 mismatch")
    _require(output.get("completed_negative_complete_paths_exact") == [f"{OUTPUT_ROOT_RELATIVE}{name}" for name in EXACT11_SCHEMA_ORDER], "V3 negative exact11 mismatch")
    _require(output.get("final_publication_order_positive_exact") == list(FINAL_PUBLICATION_ORDER), "V3 positive publication order mismatch")
    _require(output.get("final_publication_order_negative_exact") == list(FINAL_PUBLICATION_ORDER), "V3 negative publication order mismatch")
    _require(output.get("runner_staged_paths_exact") == RUNNER_STAGED_RELATIVES, "V3 runner staged paths mismatch")
    _require(output.get("sealer_staged_paths_exact") == SEALER_STAGED_RELATIVES, "V3 sealer staged paths mismatch")
    ...
    for name, frozen in ALIAS_SPECS.items():
    ...
    _require(gate.get("future_code_seal_path") == CODE_SEAL_RELATIVE, "V3 code-seal path mismatch")
    _require(gate.get("future_execution_authorization_path") == AUTHORIZATION_RELATIVE, "V3 authorization path mismatch")
    _require(gate.get("future_independent_review_path") == REVIEW_RELATIVE, "V3 review path mismatch")
    _require(gate.get("future_independent_go_path") == GO_RELATIVE, "V3 GO path mismatch")
    _require(gate.get("future_postrun_independent_pass_path") == POSTRUN_RELATIVE, "V3 postrun path mismatch")
    _require(gate.get("required_sealer") == CODE_ROLE_PATHS["sealer"], "V3 sealer path mismatch")
    _require(gate.get("required_sealer_test") == CODE_ROLE_PATHS["sealer_test"], "V3 sealer-test path mismatch")
...
def validate_amendment(artifact_root: Path) -> dict[str, Any]:
    ...
    validate_amendment_v4(artifact_root, v3)
...
def expected_required_commands(
    ...
    return {
        "runner": [python, "-B", "-m", "scripts.run_noaa_gfs_target_free_duplicate_v3", *common],
        "auditor": [python, "-B", "-m", "scripts.audit_noaa_gfs_target_free_duplicate_v3", *common],
        "sealer": [python, "-B", "-m", "scripts.seal_noaa_gfs_target_free_duplicate_v3", *common, "--postrun-pass", postrun],
    }
...
def validate_control_payload(
    ...
    *,
    ...
    _require(
        ...
        f"{kind} was created before the V4 documentary correction",
    )
    ...
    if kind == "code_seal":
        ...
        _require(
            ...
            < _utc_order_value(
                ...
                label="V3 test evidence",
            )
            <= _utc_order_value(payload["created_utc"], label="V3 code seal"),
            "V3 code-seal/test-evidence V4 chronology mismatch",
        )
        _require_exact_tree(
...
def validate_auditor_stdout(
    ...
    *,
    ...
    _require(payload.get("artifact_type") == "TARGET_FREE_DUPLICATE_NO_FIT_POSTRUN_AUDIT_V3", "auditor stdout artifact type mismatch")
    _require(payload.get("status") == "PASS_V3_NO_FIT_POSTRUN_AUDIT", "auditor stdout status mismatch")
...
def validate_runner_stdout(
    ...
    *,
    ...
    _require(
        ...
        == "TARGET_FREE_DUPLICATE_RUNNER_STAGED_RESULT_V3",
...
def validate_postrun_namespace_state(
    ...
    root = path_under(
        ...
        label="V3 postrun output root",
    )
    transaction = path_under(
        artifact_root,
        TRANSACTION_ROOT_RELATIVE.rstrip("/"),
        label="V3 postrun transaction root",
    )
    _require(root.is_dir() and not _linklike(root), "V3 postrun output root absent/link-like")
    _require(
        transaction.is_dir() and not _linklike(transaction),
        "V3 postrun transaction root absent/link-like",
    )
    root_entries = {path.name: path for path in root.iterdir()}
    _require(
        set(root_entries) == {".transaction_v3", *ALIAS_ORDER},
        "V3 postrun root is not aliases2 plus transaction directory",
    )
    _require(root_entries[".transaction_v3"].is_dir(), "V3 transaction entry is not a directory")
    ...
    _require(
        ...
        "V3 postrun transaction is not exact staged7",
    )
    _require(
        all(path.is_file() and not _linklike(path) for path in transaction_entries.values()),
        "V3 postrun transaction contains a non-regular staged entry",
    )
...
def validate_authority_chain(
    ...
    *,
    ...
    _require(
        ...
        "V3 control attempt_id chain mismatch",
    )
    ...
    _require(
        ...
        "V3 code identity chain mismatch",
    )

    chronology = [
        _utc_order_value(amendment["created_utc"], label="V3 amendment"),
        _utc_order_value(AMENDMENT_V4_CREATED_UTC, label="V4 amendment"),
        _utc_order_value(code_seal["created_utc"], label="V3 code seal"),
        _utc_order_value(authorization["created_utc"], label="V3 authorization"),
        _utc_order_value(review["created_utc"], label="V3 independent review"),
        _utc_order_value(independent_go["created_utc"], label="V3 independent GO"),
        _utc_order_value(postrun["created_utc"], label="V3 independent postrun pass"),
    ]
    _require(
        all(left < right for left, right in zip(chronology, chronology[1:])),
        "V3 control chronology is not strictly increasing",
    )
    _require(
        _utc_order_value(code_seal["test_evidence"]["completed_utc"], label="V3 test evidence")
        <= chronology[2],
        "V3 test evidence completed after code seal",
    )
    _require(
        chronology[1]
        < _utc_order_value(
        ...
        "V3 test evidence completed before the V4 documentary correction",
    )
    ...
    if validate_staged_content:
        ...
        _require(
            ...
            "V3 auditor decision does not match staged decision",
        )
    ...
    _require(
        ...
        "V3 no-fit sealer/auditor evidence mismatch",
    )
    return {
        "amendment": amendment,
        "controls": controls,
...
def build_code_seal_payload(
    *,
    ...
    evidence_completed = _utc_order_value(
        test_evidence.get("completed_utc"), label="V3 test evidence"
    )
    seal_created = _utc_order_value(created_utc, label="V3 code seal")
    _require(
        amendment_created < amendment_v4_created < evidence_completed <= seal_created,
        "V3 code-seal chronology mismatch",
    )
    payload = {
        "schema_version": 3,
...
def _expected_family_lock_payload(
    *,
    ...
    return {
        ...
        "status": amendment["output_contract"][
            "family_lock_status_literals_exact"
        ][terminal_key],
...
def validate_run_manifest_payload(
    ...
    *,
    ...
    _require(
        ...
        "run-manifest chronology does not follow the V4 documentary correction",
    )
    _require(
        isinstance(controls, Mapping)
    ...
    try:
        ...
        if published or physically_published:
            raise PartialPublicationError(
                "partial alias publication is terminal; append-only V4 supersession required",
...
def publish_final_sequence(
    ...
    *,
    ...
    try:
        ...
        if published or physically_published:
            raise PartialPublicationError(
                "partial V3 final publication is terminal; incident and V4 required",
                physically_published or published,
            ) from exc
        raise
...
def validate_exact11_completion(
    ...
    *,
    ...
    root = path_under(artifact_root, OUTPUT_ROOT_RELATIVE.rstrip("/"), label="V3 output root")
    _require(root.is_dir() and not _linklike(root), "V3 output root absent/link-like")
    expected = set(EXACT11_BASENAMES)
    entries = tuple(root.iterdir())
    observed = {path.name for path in entries if path.is_file()}
    _require(observed == expected, f"V3 exact11 file set mismatch: {sorted(observed ^ expected)}")
    _require(
        all(not _linklike(root / name) for name in expected),
        "V3 exact11 contains a link-like path",
    )
    directories = {path.name for path in entries if path.is_dir()}
    transaction = root / ".transaction_v3"
    if require_transaction_absent:
        _require(not directories, "V3 completed namespace retains a directory")
        _require(not _lexists(transaction), "V3 completed transaction path still exists")
    else:
        _require(directories == {".transaction_v3"}, "V3 precleanup directory set mismatch")
        _require(not any(transaction.iterdir()), "V3 final transaction directory is not empty")
    contract = amendment or validate_amendment(artifact_root)
    validate_final_alias_schema_closure(artifact_root, contract)
    decision = strict_json_load(root / "TARGET_FREE_FAMILY_DECISION.json", label="V3 final decision")
    lock = strict_json_load(root / "TARGET_FREE_FAMILY_LOCK.json", label="V3 final family lock")
    _require(
        (root / "TARGET_FREE_FAMILY_LOCK.json").read_bytes()
        == pretty_json_bytes(lock),
        "V3 final family lock serialization mismatch",
    )
    validate_decision_payload(decision, contract)
    _require(set(lock) == LOCK_TOP_KEYS, "V3 final family-lock schema mismatch")
    decision_statuses = contract["output_contract"]["decision_status_literals_exact"]
    lock_statuses = contract["output_contract"]["family_lock_status_literals_exact"]
    if decision["status"] == decision_statuses["positive"]:
        ...
        _require(
            lock["status"] == lock_statuses["positive"]
        ...
        _require(
            ...
            and lock["status"] == lock_statuses["global_ambiguity"]
        ...
        _require(
            ...
            and decision["global_ambiguity"] is False
            and lock["status"] == lock_statuses["clear_no_family"]
            and lock["selected_family"] is None,
            "V3 clear-no-family tombstone mismatch",
        )
    manifest = root / "TRACK_A_TARGET_FREE_RUN_MANIFEST.json"
    payload = strict_json_load(manifest, label="V3 run manifest")
    _require(
        manifest.read_bytes() == pretty_json_bytes(payload),
        "V3 final run-manifest serialization mismatch",
    )
    _require(set(payload) == MANIFEST_TOP_KEYS, "V3 run manifest schema mismatch")
    _require(payload.get("manifest_is_last_commit_marker") is True, "V3 manifest-last marker false")
...
def finalize_validated_authority_chain(
    ...
    *,
    ...
    transaction = path_under(
        root,
        TRANSACTION_ROOT_RELATIVE.rstrip("/"),
        label="completed V3 transaction cleanup",
    )
    _require(transaction.is_dir() and not any(transaction.iterdir()), "completed V3 transaction is not empty before cleanup")
    transaction.rmdir()
    ...
    return {
        "status": "PASS_V3_EXACT11_COMMITTED_MANIFEST_LAST",
        "terminal_state": decision["status"],
...
def validate_decision_payload(
    ...
    _require(
        payload.get("artifact_type") == "TARGET_FREE_FAMILY_DECISION_V3",
...
def validate_fit_ledger_payload(
    ...
    *,
    ...
    _require(
        ...
        and ledger["schema_version"] == 3,
        "fit ledger schema_version mismatch",
    )
    _require(
        ledger.get("artifact_type") == "TARGET_FREE_FIT_LEDGER_V3",
...
def validate_distribution_payload(
    ...
    *,
    ...
    _require(
        ...
        and distribution["schema_version"] == 3,
        "distribution schema_version mismatch",
    )
    _require(
        distribution.get("artifact_type")
        == "TARGET_FREE_DISTRIBUTION_STABILITY_V3",
...
def validate_staged_json_outputs(
    ...
    ledger = strict_json_load(
        path_under(artifact_root, RUNNER_STAGED_RELATIVES["TARGET_FREE_FIT_LEDGER_V3.json"], label="staged fit ledger"),
    ...
    distribution = strict_json_load(
        path_under(artifact_root, RUNNER_STAGED_RELATIVES["TARGET_FREE_DISTRIBUTION_STABILITY_V3.json"], label="staged distribution"),
...
def _network_forbidden(*_args: Any, **_kwargs: Any) -> Any:
    raise TargetFreeSealError("network access is forbidden in the V3 sealer")
...
def static_self_validate(source_path: Path | None = None) -> dict[str, Any]:
    ...
    return {
        "status": "PASS_V3_SEALER_STATIC_SELF_VALIDATION",
...
def publish_code_seal(
    ...
    *,
    ...
    destination = path_under(artifact_root, CODE_SEAL_RELATIVE, label="V3 code seal")
    _require(not _lexists(destination), "V3 code seal already exists")
...
def code_seal_mode(
    ...
    validate_sealer_runtime_identity(amendment)
    _require(not path_under(artifact_root, OUTPUT_ROOT_RELATIVE.rstrip("/"), label="V3 output root").exists(), "code-seal mode requires output root absent")
    code = collect_code_identities(repo_root)
    evidence = strict_json_load(test_evidence_path, label="V3 isolated test evidence")
    ...
    return {
        "status": "PASS_V3_CODE_SEAL_PUBLISHED_NO_EXECUTION_AUTHORITY",
        "code_seal": published,
        "models_fit": 0,
        "parquet_logical_value_reads": 0,
        "network_requests": 0,
