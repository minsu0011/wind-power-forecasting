...
from scripts import run_noaa_gfs_target_free_duplicate_v3 as subject
...
def synthetic_output_prepared(
    ...
    return (
        subject.PreparedExecution(
            ...
            attempt_id="target_free_duplicate_v3__20260811T123456123456Z",
            amendment={},
            amendment_v4={},
            bound_paths={},
...
def test_v4_common_control_top_keysets_and_identity_are_mandatory() -> None:
    ...
    wrong["amendment_v4"]["sha256"] = "1" * 64
    with pytest.raises(subject.PredataAuthorizationError, match="amendment_v4"):
...
def test_parquet_footer_resources_close_on_success_and_validation_failure(
    ...
    class SyntheticSchema:
        ...
        def __iter__(self):
            return iter((type("Field", (), {"type": "double"})(),))
...
def test_manual_ridge_prediction_matches_synthetic_fitted_pipeline() -> None:
    rng = np.random.default_rng(260810)
    x = rng.normal(size=(96, 35)).astype(np.float64)
    y = x[:, 0] * 2.0 - x[:, 1] + 0.5
    pipeline = subject.make_ridge_pipeline().fit(x, y)
    expected = pipeline.predict(x[:8])
...
@pytest.mark.parametrize(
    "attempt_id",
    [
        "target_free_duplicate_v3__20260811T123456123456Z",
        "target_free_duplicate_v3__20260229T123456123456Z",
        "target_free_duplicate_v3__20260811T12345612345Z",
    ],
)
def test_attempt_id_regex_and_calendar_validation(attempt_id: str) -> None:
    if attempt_id == "target_free_duplicate_v3__20260811T123456123456Z":
...
def test_control_timestamps_require_exact_microseconds_and_strict_order() -> None:
    ...
    controls = {
        "CODE_SEAL": {
            "created_utc": "2026-08-11T12:00:00.000004Z",
            "test_evidence": {
                "completed_utc": "2026-08-11T12:00:00.000003Z"
            },
        },
        "AUTHORIZATION": {"created_utc": "2026-08-11T12:00:00.000005Z"},
        "REVIEW": {"created_utc": "2026-08-11T12:00:00.000006Z"},
        "GO": {"created_utc": "2026-08-11T12:00:00.000007Z"},
    }
    subject.validate_control_chronology(amendment, amendment_v4, controls)
    ...
    subject.validate_control_chronology(
        amendment, amendment_v4, equal_completion_and_seal
    )
    for seal_time in (
        amendment_v4["created_utc"],
        amendment["created_utc"],
    ):
        ...
        with pytest.raises(subject.PredataAuthorizationError, match="not strict"):
            subject.validate_control_chronology(
                amendment, amendment_v4, broken
            )
    for v4_time, completed_time in (
        (amendment["created_utc"], controls["CODE_SEAL"]["test_evidence"]["completed_utc"]),
        (amendment_v4["created_utc"], amendment_v4["created_utc"]),
    ):
        broken_v4 = {"created_utc": v4_time}
        broken = copy.deepcopy(controls)
        broken["CODE_SEAL"]["test_evidence"]["completed_utc"] = completed_time
        with pytest.raises(subject.PredataAuthorizationError, match="not strict"):
            subject.validate_control_chronology(amendment, broken_v4, broken)
...
def test_actual_runner_command_is_type_and_position_exact() -> None:
    expected = [
        ...
        "-B",
        "-m",
        "scripts.run_noaa_gfs_target_free_duplicate_v3",
        "--root",
...
def test_actual_runner_command_guard_is_before_runtime_alias_and_value_handoff() -> None:
    ...
    assert chain_index < preparation_source.index("create_v3_namespace_exclusive(")
...
def test_production_run_path_completes_synthetic_negative_without_any_fit(
    ...
    ledger = subject.strict_json_loads(
        (transaction / "TARGET_FREE_FIT_LEDGER_V3.json").read_bytes()
    )
...
def test_partial_staging_is_preserved_and_v3_retry_is_rejected(
    tmp_path: subject.Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    raw = constant_raw_evaluations()
    derived = empty_derived_evaluation(raw)
