...
from scripts import seal_noaa_gfs_multiseason_decode_recovery_v2 as sealer
...
def test_sealer_source_has_no_network_or_recovery_launch_path() -> None:
    source_path = ROOT / "scripts" / "seal_noaa_gfs_multiseason_decode_recovery_v2.py"
...
def test_frozen_supporting_schema_sets_are_exact() -> None:
    assert len(sealer.RAW_LOCK_KEYS) == 19
    assert len(sealer.PREFLIGHT_KEYS) == 49
...
def test_seal_publishes_exactly_three_v2_controls() -> None:
    source = (
        ROOT / "scripts" / "seal_noaa_gfs_multiseason_decode_recovery_v2.py"
    ...
    for node in ast.walk(seal_node):
        if (
            ...
            published.append(str(node.args[0].slice.value))
    assert published == ["raw_cache_lock", "real_spawn_preflight", "authorization"]
    assert "independent_review" not in published
    assert "independent_go" not in published
...
def test_v1_and_v2_control_temporary_siblings_fail_closed(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, relative: str
) -> None:
    for control in (*sealer.V1_CONTROL_RELATIVES, *sealer.V2_CONTROL_RELATIVES):
        (tmp_path / control).parent.mkdir(parents=True, exist_ok=True)
    monkeypatch.setattr(
        sealer,
        "recovery",
        types.SimpleNamespace(
            strict_under_root=lambda root, item: (root / item).resolve()
        ),
    )
    destination = tmp_path / relative
    temporary = destination.with_name(f"{destination.name}.TMP.synthetic")
    temporary.write_bytes(b"partial")
    with pytest.raises(sealer.RecoverySealError, match="temporary sibling"):
        sealer.require_control_temp_siblings_absent(tmp_path)
...
def _rejected_transient_wrapper() -> dict[str, object]:
    path = (
        ROOT
        / "artifacts"
        / "baram2026_ncei_scada_longrun_20260810_v2"
        / sealer.FIXED_ROOT_INPUTS["launch_identity_correction"]
    )
    payload = json.loads(path.read_text(encoding="utf-8"))
    return payload["rejected_transient_identity"]


def test_identity_correction_rejected_transient_wrapper_passes_exactly() -> None:
    wrapper = _rejected_transient_wrapper()
    sealer.validate_rejected_transient_wrapper(wrapper)
    assert wrapper["identity"] == sealer.REJECTED_TRANSIENT_V1_RUNNER_IDENTITY


@pytest.mark.parametrize(
    "mutation",
    ("extra", "missing", "identity", "disposition", "historical", "executed", "authorized"),
)
def test_identity_correction_rejected_transient_wrapper_tamper_fails(
    mutation: str,
) -> None:
    wrapper = json.loads(json.dumps(_rejected_transient_wrapper()))
    if mutation == "extra":
        wrapper["extra"] = False
    elif mutation == "missing":
        wrapper.pop("identity")
    elif mutation == "identity":
        wrapper["identity"]["sha256"] = "0" * 64
    elif mutation == "disposition":
        wrapper["disposition"] = "AUTHORIZED"
    elif mutation == "historical":
        wrapper["historical_authority"] = True
    elif mutation == "executed":
        wrapper["executed_in_failed_launch"] = True
    else:
        wrapper["authorized_by_v1_controls"] = True
    with pytest.raises(sealer.RecoverySealError, match="rejected transient"):
        sealer.validate_rejected_transient_wrapper(wrapper)
...
def _control_state(root: Path) -> dict[str, object]:
    state: dict[str, object] = {}
    for relative in (*sealer.V1_CONTROL_RELATIVES, *sealer.V2_CONTROL_RELATIVES):
        path = root / relative
        state[relative] = (
            (path.stat().st_size, sealer.sha256_file(path))
...
def test_direct_file_write_mode_fails_before_data_or_publication(tmp_path: Path) -> None:
    command = [
        sys.executable,
        "-B",
        str(ROOT / "scripts" / "seal_noaa_gfs_multiseason_decode_recovery_v2.py"),
        "--root",
        str(tmp_path),
    ]
...
def test_direct_file_static_audit_remains_read_only_and_allowed() -> None:
    root = ROOT / "artifacts" / "baram2026_ncei_scada_longrun_20260810_v2"
    before = _control_state(root)
    completed = subprocess.run(
        [
            sys.executable,
            "-B",
    ...
    report = json.loads(completed.stdout)
    assert report["status"] == "PASS_STATIC_V2_DECODE_RECOVERY_SEALER_READY"
    assert report["writes"] == 0
    assert report["network_requests"] == 0
    assert _control_state(root) == before
...
def test_canonical_module_entrypoint_real_seven_spawn_is_read_only() -> None:
    ...
    assert report["status"] == (
        "PASS_V2_CANONICAL_MODULE_ENTRYPOINT_TOP_LEVEL_BOOTSTRAP_7SPAWN"
    )
...
def test_static_audit_writes_nothing() -> None:
    root = ROOT / "artifacts" / "baram2026_ncei_scada_longrun_20260810_v2"
    before = _control_state(root)
    report = sealer.static_audit(root, ROOT)
    after = _control_state(root)
    assert report["status"] == "PASS_STATIC_V2_DECODE_RECOVERY_SEALER_READY"
    assert report["network_requests"] == 0
    assert report["writes"] == 0
    assert before == after
