...
from scripts import audit_noaa_gfs_multiseason_raw_postrun_v6 as subject
...
def test_complete_synthetic_producer_passes_and_audit_is_read_only(tmp_path: Path) -> None:
    ...
    assert report["status"] == "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V6"
...
def test_cli_normalizes_malformed_artifacts_to_structured_fail(
    ...
    assert report["status"] == "FAIL_PRODUCER_POSTRUN_AUDIT_V6"
...
def test_inherited_core_auditor_is_stdout_only_and_writes_zero_files(tmp_path: Path) -> None:
    ...
    before = snapshot(tmp_path)
    report = audit_fixture(root, runner, contract)
    assert report["artifact_type"] == "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V6"
    assert report["audit_files_written"] == 0
    assert report["audit_network_requests"] == 0
    assert snapshot(tmp_path) == before
...
def build_valid_v6_authority(
    ...
    source_identities = {
        **subject.FROZEN_V5_SOURCE_IDENTITIES,
        "v6_auditor": artifact(Path(subject.__file__).resolve()),
        "v6_auditor_test": artifact(subject.AUDITOR_TEST),
        "v6_sealer": artifact(subject.V6_SEALER),
        "v6_sealer_test": artifact(subject.V6_SEALER_TEST),
    }
    bound = {
...
def test_historical_predata_gate_precedes_schema_parquet_data_and_replay(
    ...
    for name in (
        ...
        monkeypatch.setattr(subject, name, forbidden)
    monkeypatch.setattr(pd, "read_parquet", forbidden)
    monkeypatch.setattr(subject, "_V6_CANONICAL_MAIN_ACTIVE", True)
    runner = tmp_path / "runner.py"
    runner.write_text("# metadata-only ordering sentinel\n", encoding="utf-8")
    with pytest.raises(subject.AuditFailure, match="historical failed-head identity contract"):
        subject.audit(
            root,
            contract=subject.PRODUCTION_CONTRACT,
            runner_path=runner,
...
def test_late_reuse_revalidates_historical_chain(
    ...
    before = snapshot(root)
    original_poststate = subject._v6_validate_postauthority_afterstate
    ...
    monkeypatch.setattr(subject, "_v6_validate_postauthority_afterstate", counted_poststate)
    monkeypatch.setattr(subject, "_v6_validate_predata_authority", lambda *_args: authority)
    monkeypatch.setattr(subject, "_V6_CANONICAL_MAIN_ACTIVE", True)
    monkeypatch.setattr(subject, "require_no_symlink_chain", lambda *_args, **_kwargs: None)
    ...
    monkeypatch.setattr(subject, "preflight_zero_target_parquet_schemas", lambda *_args: {})
    monkeypatch.setattr(
        subject,
        "_audit_provenance",
        lambda *_args: {
    ...
    monkeypatch.setattr(
        subject, "_audit_raw_ranges",
        lambda *_args: {"raw_info": [], "raw_rows": 0, "raw_bytes": 0},
    )
    monkeypatch.setattr(
        subject, "_audit_request_events",
        lambda *_args: {
    ...
    monkeypatch.setattr(subject, "_audit_full_offline_redecode", lambda *_args, **_kwargs: {})
    ...
    monkeypatch.setattr(subject, "_audit_canonical_metadata", mutate_in_memory_between_poststate_calls)
    with pytest.raises(
        subject.AuditFailure, match="historical failed-head identity contract mismatch"
    ):
        subject.audit(
            root,
            contract=subject.PRODUCTION_CONTRACT,
    ...
    assert snapshot(root) == before
    audit_node = next(
        node for node in ast.parse(Path(subject.__file__).read_text(encoding="utf-8")).body
...
def test_v6_auditor_is_stdout_only_and_writes_zero_files(
    ...
    monkeypatch.setattr(subject, "_v6_require_canonical_runtime_entrypoint", lambda *_args: None)
    rc = subject.main(
        [
    ...
    assert snapshot(root) == before
    assert subject._v6_postrun_report_candidates(root) == []
...
def test_v6_authority_and_historical_gate_precede_any_parquet_or_data_read(
    ...
    with pytest.raises(subject.AuditFailure, match="absent|duplicate JSON key|historical"):
        subject.audit(
            root,
            contract=subject.PRODUCTION_CONTRACT,
            runner_path=runner,
