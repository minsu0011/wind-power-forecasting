...
from scripts import audit_noaa_gfs_target_free_duplicate_v3 as subject
...
def test_module_import_does_not_import_numpy_or_pyarrow() -> None:
    code = (
        "import sys; "
        "import scripts.audit_noaa_gfs_target_free_duplicate_v3; "
...
def test_amendment_bound_input_count_and_release_paths(amendment: dict) -> None:
    assert len(amendment["bound_inputs"]) == 31
    release = amendment["implementation_release_gate"]
    assert release["required_auditor"] == subject.CODE_ROLE_PATHS["auditor"]
    assert release["required_auditor_test"] == subject.CODE_ROLE_PATHS["auditor_test"]
    assert release["future_code_seal_path"] == subject.CODE_SEAL_REL
    assert release["future_execution_authorization_path"] == subject.AUTH_REL
    assert release["future_independent_review_path"] == subject.REVIEW_REL
    assert release["future_independent_go_path"] == subject.GO_REL
    assert release["future_postrun_independent_pass_path"] == subject.POSTRUN_REL
...
def test_exact11_positive_and_negative_sets_match(amendment: dict) -> None:
    output = amendment["output_contract"]
    assert output["positive_complete_paths_exact"] == output["completed_negative_complete_paths_exact"]
    assert len(output["positive_complete_paths_exact"]) == 11
    assert output["positive_complete_paths_exact"][-1].endswith("TRACK_A_TARGET_FREE_RUN_MANIFEST.json")
    assert all(name in output["positive_complete_paths_exact"] for name in output["diagnostic_csvs_allowed_exact"])
...
def test_future_lock_and_manifest_exact_top_contracts_are_frozen() -> None:
    ...
    assert subject.LOCK_ARTIFACT_TYPE == "TARGET_FREE_FAMILY_LOCK_V3"
    assert subject.MANIFEST_ARTIFACT_TYPE == "TRACK_A_TARGET_FREE_RUN_MANIFEST_V3"
    assert subject.MANIFEST_STATUS == "COMMITTED_V3_TARGET_FREE_DECISION"
...
def test_attempt_id_requires_compact_calendar_valid_microsecond_utc() -> None:
    valid = "target_free_duplicate_v3__20260811T123456123456Z"
    assert subject._validate_attempt_id(valid) == valid
    for invalid in (
        "target_free_duplicate_v3__20260230T123456123456Z",
        "target_free_duplicate_v3__20260811T246000123456Z",
        "target_free_duplicate_v3__20260811T12345612345Z",
...
def test_control_top_key_sets_and_scopes_are_frozen() -> None:
    assert len(subject.CODE_SEAL_TOP_KEYS) == 12
    assert len(subject.AUTH_TOP_KEYS) == 16
    assert len(subject.REVIEW_TOP_KEYS) == 15
    assert len(subject.GO_TOP_KEYS) == 16
    assert len(subject.POSTRUN_TOP_KEYS) == 17
    assert len(subject.AUDITOR_STDOUT_TOP_KEYS) == 17
    assert "amendment_v4" in subject.AUDITOR_STDOUT_TOP_KEYS
...
def test_common_control_constants_match_runner_and_sealer() -> None:
    from scripts import run_noaa_gfs_target_free_duplicate_v3 as runner
    from scripts import seal_noaa_gfs_target_free_duplicate_v3 as sealer
    ...
    assert len(sealer.AUDITOR_STDOUT_KEYS) == 17
    assert len(sealer.RUNNER_STDOUT_KEYS) == len(runner.RUNNER_STDOUT_KEYS) == 11
    assert "amendment_v4" in sealer.AUDITOR_STDOUT_KEYS
...
def test_required_control_paths_and_single_attempt_are_frozen() -> None:
    ...
    assert subject.SINGLE_ATTEMPT["prior_v3_failure_incident"] is None
...
def test_required_command_shapes(amendment: dict) -> None:
    ...
    assert commands["runner"][1:4] == ["-B", "-m", "scripts.run_noaa_gfs_target_free_duplicate_v3"]
    assert commands["auditor"][1:4] == ["-B", "-m", "scripts.audit_noaa_gfs_target_free_duplicate_v3"]
    assert commands["sealer"][1:4] == ["-B", "-m", "scripts.seal_noaa_gfs_target_free_duplicate_v3"]
...
def _build_control_fixture(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    seal = {
        ...
        "created_utc": "2026-08-11T16:01:00.000000Z",
    ...
    attempt = "target_free_duplicate_v3__20260811T160200000000Z"
    auth = {
        "schema_version": 3,
        ...
        "created_utc": "2026-08-11T16:02:00.000000Z",
        "attempt_id": attempt,
        "amendment": amendment_identity,
        "amendment_v4": amendment_v4_identity,
        ...
        "required_review": {"path": subject.REVIEW_REL, "status_required": "PASS_V3_EXECUTION_REVIEW_PENDING_INDEPENDENT_GO"},
        "required_go": {"path": subject.GO_REL, "status_required": "GO_V3_SINGLE_TARGET_FREE_ATTEMPT"},
    ...
    review = {
        ...
        "created_utc": "2026-08-11T16:03:00.000000Z",
        "attempt_id": attempt,
        "amendment": amendment_identity,
        "amendment_v4": amendment_v4_identity,
        "code_seal": seal_identity,
    ...
    go = {
        ...
        "created_utc": "2026-08-11T16:04:00.000000Z",
        "attempt_id": attempt,
        "amendment": amendment_identity,
        "amendment_v4": amendment_v4_identity,
        "code_seal": seal_identity,
...
def test_full_synthetic_control_chain_passes_before_arrow(tmp_path: Path) -> None:
    ...
    assert context.attempt_id == "target_free_duplicate_v3__20260811T160200000000Z"
...
def test_actual_auditor_command_rejects_unauthorized_argv(
    ...
    if variant == "extra":
        ...
        actual[3] = "scripts.run_noaa_gfs_target_free_duplicate_v3"
...
def test_control_chain_rejects_authorization_extra_key(tmp_path: Path) -> None:
    ...
    with pytest.raises(subject.AuditError, match="exact keys"):
        subject.validate_predata_authority(root, auth, go, repo=repo, validate_bound_inputs=False, validate_runtime=False)
...
@pytest.mark.parametrize(
    ...
    (
        ("code_seal", ("schema_version",), 3.0, "code seal schema"),
        ...
        ("authorization", ("schema_version",), 3.0, "authorization schema"),
        (
            "authorization",
        ...
        ("review", ("schema_version",), 3.0, "review schema"),
        (
            "review",
        ...
        ("go", ("schema_version",), 3.0, "GO schema"),
        (
            "go",
...
@pytest.mark.parametrize(
    "completed_utc",
    (
        subject.AMENDMENT_V4_CREATED_UTC,
        "2026-08-11T15:00:30.853999Z",
    ),
    ids=("equal_to_v4", "earlier_than_v4"),
)
def test_control_chain_rejects_test_evidence_not_strictly_after_v4(
...
def test_control_chain_rejects_attempt_calendar_drift(tmp_path: Path) -> None:
    root, repo, auth, go = _build_control_fixture(tmp_path)
    payload = subject.strict_json_file(auth)
    payload["attempt_id"] = "target_free_duplicate_v3__20260230T120200000000Z"
    _write_json(auth, payload)
...
def _context_for_namespace(tmp_path: Path, amendment: dict) -> subject.PredataContext:
    ...
    (tmp_path / "raw").mkdir()
    (tmp_path / subject.OUTPUT_ROOT_REL / ".transaction_v3").mkdir(parents=True)
    ...
    for name in subject.RUNNER_OUTPUTS:
        (output / ".transaction_v3" / name).write_bytes(b"placeholder\n")
...
def test_actual_bound_source_parquet_metadata_only(amendment: dict) -> None:
    _, _, pq = subject._import_arrow_after_predata()
    ...
    subject._validate_source_parquet_metadata(context, pq, validate_provenance_alias=False)
...
def _actual_v4_runtime_context(amendment: dict) -> subject.PredataContext:
    ...
    return subject.PredataContext(
        ...
        independent_go={},
        independent_go_identity={},
        attempt_id="target_free_duplicate_v3__20260811T160200000000Z",
    )
...
def test_v4_affected_parquet_schema_requires_nonnullable_ns_utc_and_date32(
    ...
    context = _actual_v4_runtime_context(amendment)
    basename = "TARGET_FREE_DUPLICATE_OOF_V3.parquet"
    valid = pa.schema(
        [
...
def test_evidence_headers_bind_terminal_decision_status(amendment: dict) -> None:
    ...
    ledger = {"schema_version": 3, "artifact_type": "TARGET_FREE_FIT_LEDGER_V3", "status": status}
    distribution = {"schema_version": 3, "artifact_type": "TARGET_FREE_DISTRIBUTION_STABILITY_V3", "status": status}
...
def _lock_payload(amendment: dict, kind: str) -> tuple[dict, dict, dict, dict]:
    ...
    payload = {
        ...
        "status": amendment["output_contract"]["family_lock_status_literals_exact"][kind],
    ...
    return payload, decision, code_identities, postrun_identity
...
def test_future_family_lock_validator_accepts_all_terminal_states(
    ...
    payload, decision, code_identities, postrun_identity = _lock_payload(
        amendment, kind
    )
    _, amendment_identity = subject._validate_amendment(ROOT)
    _, amendment_v4_identity = subject._validate_amendment_v4(
...
def _manifest_payload(amendment: dict) -> tuple[dict, dict]:
    ...
    payload = {
        ...
        "created_utc": "2026-08-11T18:00:00.000000Z",
        "code_config_runtime_identities": {
            "amendment": amendment_identity,
            "amendment_v4": amendment_v4_identity,
            "code_identities": code_identities,
    ...
    dependencies = {
        ...
        "postrun_created_utc": "2026-08-11T17:00:00.000000Z",
    }
...
def test_alias_schema_closure_precedes_every_staged_logical_read() -> None:
    ...
    assert first["_validate_namespace_structure"] < first["_import_arrow_after_predata"]
    assert first["_import_arrow_after_predata"] < first["_validate_source_parquet_metadata"]
...
def test_artifact_path_component_walk_rejects_junction_signal(
    ...
    with pytest.raises(subject.AuditError, match="junction component"):
        subject._artifact_path(root, ".transaction_v3/payload.json")
...
def test_readonly_guard_denies_network_and_writes_in_child_process(tmp_path: Path) -> None:
    code = (
        ...
        "from scripts.audit_noaa_gfs_target_free_duplicate_v3 import install_readonly_runtime_guard; "
...
def test_main_failure_is_stdout_only_and_zero_mutation(tmp_path: Path) -> None:
    ...
    command = [
        ...
        "-B",
        "-m",
        "scripts.audit_noaa_gfs_target_free_duplicate_v3",
        "--root",
        str(root),
        "--authorization",
