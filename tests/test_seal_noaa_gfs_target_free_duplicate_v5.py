...
from scripts import seal_noaa_gfs_target_free_duplicate_v5 as seal
...
def _test_evidence(repo: Path, amendment: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "completed_utc": "2026-08-11T19:00:00.000000Z",
    }
...
def _auditor_stdout(
    ...
    *,
    ...
    attempt_id: str = "target_free_duplicate_v5__20260811T000000000000Z",
    ...
    payload = {
        ...
        "status": "PASS_V5_NO_FIT_POSTRUN_AUDIT",
        "attempt_id": attempt_id,
        "amendment": seal.amendment_identity(),
        "amendment_v4": seal.amendment_v4_identity(),
        "amendment_v5": seal.amendment_v5_identity(),
...
def _lock_payload(amendment: Mapping[str, Any], status_key: str) -> dict[str, Any]:
    ...
    return {
        "schema_version": 5,
        "artifact_type": "TARGET_FREE_FAMILY_LOCK_V5",
...
def _manifest_payload() -> dict[str, Any]:
    return {
        "schema_version": 5,
        "artifact_type": "TRACK_A_TARGET_FREE_RUN_MANIFEST_V5",
        "status": "COMMITTED_V5_TARGET_FREE_DECISION",
        "terminal_state": "FIXTURE",
        "created_utc": "2026-08-11T19:00:09.000000Z",
        "code_config_runtime_identities": {},
        "control_identities": {},
        "alias_identities": {},
        "output_identities": {},
...
def _final_stage_fixture(
    ...
    payloads: dict[str, bytes] = {
        ...
        "TARGET_FREE_FIT_LEDGER_V5.json": b"{}\n",
        "TARGET_FREE_DISTRIBUTION_STABILITY_V5.json": b"{}\n",
    }
...
def _finalizer_validation_fixture(
    ...
    json_payloads: dict[str, Any] = {
        ...
        "TARGET_FREE_FIT_LEDGER_V5.json": fit_ledger,
    ...
    controls = {
        ...
        "review": {},
        "go": {},
        "postrun": {"created_utc": "2026-08-11T19:00:08.000000Z"},
    }
    return {
        "amendment": amendment,
        "controls": controls,
...
def test_normative_amendment_f9d0_ae5c_and_exact_orders(amendment: Mapping[str, Any]) -> None:
    ...
    v5 = seal.validate_amendment_v5(
        seal.ARTIFACT_ROOT_DEFAULT,
        seal._validate_v3_amendment(seal.ARTIFACT_ROOT_DEFAULT),
        seal.validate_amendment_v4(
            seal.ARTIFACT_ROOT_DEFAULT,
            seal._validate_v3_amendment(seal.ARTIFACT_ROOT_DEFAULT),
        ),
        seal.validate_v3_failure_incident(seal.ARTIFACT_ROOT_DEFAULT),
    )
    output = v5["output_contract"]
    complete = [f"{seal.OUTPUT_ROOT_RELATIVE}{name}" for name in seal.EXACT11_BASENAMES]
    assert output["completed_positive_paths_exact"] == complete
    assert output["completed_negative_or_ambiguity_paths_exact"] == complete
...
def test_normative_v4_exact2_overlay_identity_signature_and_control_delta(
    ...
    assert len(seal.AUDITOR_STDOUT_KEYS) == 18
    assert len(seal.RUNNER_STDOUT_KEYS) == 11


def test_v5_incident_and_amendment_identity_control_output_and_serialization_closure() -> None:
    root = seal.ARTIFACT_ROOT_DEFAULT
    v3 = seal._validate_v3_amendment(root)
    v4 = seal.validate_amendment_v4(root, v3)
    incident = seal.validate_v3_failure_incident(root)
    v5 = seal.validate_amendment_v5(root, v3, v4, incident)
    assert seal.file_identity(root / seal.INCIDENT_RELATIVE, relative_to=root) == {
        key: value
        for key, value in seal.incident_identity().items()
        if key in seal.FILE_IDENTITY_KEYS
    }
    assert seal.file_identity(root / seal.AMENDMENT_V5_RELATIVE, relative_to=root) == {
        key: value
        for key, value in seal.amendment_v5_identity().items()
        if key in seal.FILE_IDENTITY_KEYS
    }
    assert v5["bound_v3_failure_incident"] == seal.incident_identity()
    assert v5["code_and_test_paths_exact"] == seal.CODE_ROLE_PATHS
    assert v5["control_contract"]["control_top_level_key_counts_exact"] == {
        "AUTHORIZATION": 17,
        "CODE_SEAL": 13,
        "GO": 17,
        "POSTRUN": 18,
    ...
    assert v5["control_contract"]["auditor_stdout_contract"]["top_key_count"] == 18
    assert v5["control_contract"]["runner_stdout_contract"]["top_key_count"] == 11
    assert v5["output_contract"]["runner_staged_paths_exact"] == seal.RUNNER_STAGED_RELATIVES
    assert v5["runtime_and_serialization_preservation"][
        "v4_parquet_exact12_kwargs_verbatim"
    ] == seal.V4_EXPLICIT_WRITE_TABLE_KWARGS
    assert v5["final_lock_manifest_binding"]["superseding_exact_keys"] == list(
        seal.CODE_CONFIG_RUNTIME_KEYS
    )
    assert v5["final_lock_manifest_binding"]["lock_generation_literals"][
        "status_by_decision_status_exact"
    ] == seal.LOCK_STATUS_BY_DECISION_STATUS
...
def test_attempt_id_has_strict_prefix_shape_and_calendar_valid_microseconds() -> None:
    valid = "target_free_duplicate_v5__20260811T235959123456Z"
    assert seal.require_attempt_id(valid, label="fixture") == valid
    for invalid in (
        "target_free_duplicate_v5__fixture",
        "target_free_duplicate_v5__20260230T000000000000Z",
        "target_free_duplicate_v5__20260811T235960000000Z",
...
def test_historical_v3_failure_root_requires_exact_immutable_alias2_in_temp(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    root = tmp_path / "artifact"
    root.mkdir()
    failed_root = root / seal.V3_OUTPUT_ROOT_RELATIVE
    failed_root.mkdir(parents=True)
    synthetic_specs: dict[str, dict[str, Any]] = {}
    incident_aliases: dict[str, Any] = {}
    incident_names = {
        "FIELD_CENSUS_LOCK.json": "FIELD_CENSUS_LOCK",
        "PROVENANCE_LEDGER.parquet": "PROVENANCE_LEDGER",
    }
    for ordinal, name in enumerate(seal.ALIAS_ORDER, start=1):
        source_relative = f"synthetic/source_{ordinal}.bin"
        source = root / source_relative
        source.parent.mkdir(parents=True, exist_ok=True)
        data = f"historical-alias-{ordinal}\n".encode("ascii")
        source.write_bytes(data)
        final_relative = f"{seal.V3_OUTPUT_ROOT_RELATIVE}{name}"
        final = root / final_relative
        final.write_bytes(data)
        digest = _sha(data)
        synthetic_specs[name] = {
            "source": source_relative,
            "stage": f"{seal.V3_OUTPUT_ROOT_RELATIVE}.stage_{ordinal}",
            "final": final_relative,
            "size_bytes": len(data),
            "sha256": digest,
            "bundle": (),
        }
        incident_aliases[incident_names[name]] = {
            "alias_identity": seal.file_identity(final, relative_to=root)
        }

    envelopes = {
        "code_seal": (
            "prereg/v3_code_seal.json",
            "TARGET_FREE_DUPLICATE_CODE_SEAL_V3",
            "SEALED_V3_CODE_AND_TESTS_NO_EXECUTION_AUTHORITY",
            "2026-08-11T16:00:00.000000Z",
        ),
        "authorization": (
            "prereg/v3_authorization.json",
            "TARGET_FREE_DUPLICATE_EXECUTION_AUTHORIZATION_V3",
            "AUTHORIZED_V3_SINGLE_ATTEMPT_PENDING_INDEPENDENT_REVIEW_AND_GO",
            "2026-08-11T16:10:00.000000Z",
        ),
        "independent_review": (
            "independent_redteam/v3_review.json",
            "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_REVIEW_V3",
            "PASS_V3_EXECUTION_REVIEW_PENDING_INDEPENDENT_GO",
            "2026-08-11T16:20:00.000000Z",
        ),
        "independent_go": (
            "independent_redteam/v3_go.json",
            "TRACK_A_TARGET_FREE_DUPLICATE_EXECUTION_GO_V3",
            "GO_V3_SINGLE_TARGET_FREE_ATTEMPT",
            "2026-08-11T16:30:00.000000Z",
        ),
    }
    control_identities: dict[str, dict[str, Any]] = {}
    for role, (relative, artifact_type, status, created_utc) in envelopes.items():
        path = root / relative
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_bytes(
            seal.pretty_json_bytes(
                {
                    "schema_version": 3,
        ...
        control_identities[role] = seal.file_identity(path, relative_to=root)
    incident = {
        "control_identities": control_identities,
        "postfailure_state": {"aliases": incident_aliases},
    }
    monkeypatch.setattr(seal, "V3_ALIAS_SPECS", synthetic_specs)
    monkeypatch.setattr(
        seal, "validate_v3_failure_incident", lambda _root: incident
    )
    monkeypatch.setattr(
        seal,
        "_validate_v3_amendment",
        lambda _root: {"created_utc": "2026-08-11T14:00:00.000000Z"},
    )
    result = seal.validate_historical_v3_failure_state(root)
    assert result["v3_output_root_entries"] == list(seal.ALIAS_ORDER)
    assert set(result["aliases"]) == set(seal.ALIAS_ORDER)
    extra = failed_root / "unexpected.txt"
    extra.write_text("do not delete\n", encoding="ascii")
    with pytest.raises(seal.TargetFreeSealError, match="exact immutable aliases2"):
        seal.validate_historical_v3_failure_state(root)
    assert extra.read_text(encoding="ascii") == "do not delete\n"
...
def test_family_lock_builder_binds_frozen_nested_contract_and_terminal_nullability(
    ...
    assert tuple(lock["code_config_runtime_identities"]) == seal.CODE_CONFIG_RUNTIME_KEYS
...
def test_manifest_builder_binds_exact8_fit5_oof4_and_map_order_is_not_authority(
    ...
    assert tuple(manifest["code_config_runtime_identities"]) == seal.CODE_CONFIG_RUNTIME_KEYS
...
def test_finalizer_tmp_exact11_manifest_last_and_transaction_removed_only_after_pass(
    ...
    assert report["status"] == "PASS_V5_EXACT11_COMMITTED_MANIFEST_LAST"
...
def test_failure_after_lock_publication_preserves_orphan_lock_and_manifest_stage(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    amendment: Mapping[str, Any],
) -> None:
    root = tmp_path / "orphan-lock"
    root.mkdir()
    validation = _finalizer_validation_fixture(root, amendment)
    monkeypatch.setattr(
        seal, "load_bound_parent_plan", lambda *_args, **_kwargs: _parent_plan_fixture()
    ...
    def fail_manifest(stage: Path, destination: Path, **kwargs: Any) -> dict[str, Any]:
        nonlocal calls
        calls += 1
        if destination.name == "TRACK_A_TARGET_FREE_RUN_MANIFEST.json":
            raise seal.TargetFreeSealError("injected pre-manifest failure")
        return real_publish(stage, destination, **kwargs)

    monkeypatch.setattr(seal, "publish_staged_create_if_absent", fail_manifest)
    with pytest.raises(seal.PartialPublicationError) as caught:
        seal.finalize_validated_authority_chain(
            root,
            validation,
    ...
    output = root / seal.OUTPUT_ROOT_RELATIVE
    transaction = root / seal.TRANSACTION_ROOT_RELATIVE
    assert calls == len(seal.FINAL_PUBLICATION_ORDER)
    assert caught.value.published == seal.FINAL_PUBLICATION_ORDER[:-1]
    assert (output / "TARGET_FREE_FAMILY_LOCK.json").is_file()
    assert not (output / "TRACK_A_TARGET_FREE_RUN_MANIFEST.json").exists()
    assert transaction.is_dir()
    assert (transaction / "TRACK_A_TARGET_FREE_RUN_MANIFEST.json").is_file()
    assert not (transaction / "TARGET_FREE_FAMILY_LOCK.json").exists()
...
def test_staged_output_identity_exact2_exact7_and_logical_hash_rules() -> None:
    ...
    bad = copy.deepcopy(staged)
    bad["runner_staged_outputs"]["TARGET_FREE_DUPLICATE_OOF_V5.parquet"]["row_count"] += 1
    with pytest.raises(seal.TargetFreeSealError, match="row count"):
        seal.validate_staged_output_identities(bad)
...
def test_runner_stdout_full_pretty_capture_and_ledger_derived_threadpool_chain(
    ...
    staged = _staged_identities()
    attempt = "target_free_duplicate_v5__20260811T000000000000Z"
...
@pytest.mark.parametrize("status_key", ["positive", "clear_no_family", "global_ambiguity"])
def test_auditor_stdout_exact18_no_fit_and_dynamic_decision_contract(
    amendment: Mapping[str, Any], status_key: str
) -> None:
    ...
    assert seal.validate_auditor_stdout(
        ...
        amendment_v5_record=seal.amendment_v5_identity(),
...
def test_audit_result_binds_full_stdout_bytes_command_empty_stderr_and_zero_fit(
    ...
    bad["runner_result"]["stdout_payload"]["attempt_id"] = (
        "target_free_duplicate_v5__20260811T000000000001Z"
    )
...
def test_code_seal_exact13_triple_bound_payload_and_tmp_only_publication(
    tmp_path: Path, amendment: Mapping[str, Any]
) -> None:
    repo = tmp_path / "repo"
    repo.mkdir()
    ...
    payload = seal.build_code_seal_payload(
        ...
        created_utc="2026-08-11T19:00:01.000000Z",
        repo_root=repo,
    )
    ...
    assert payload["amendment_v5"] == seal.amendment_v5_identity()
    ...
    with pytest.raises(seal.TargetFreeSealError, match="V5 documentary amendment"):
        seal.validate_control_payload(
            "code_seal", bad, amendment=amendment, repo_root=repo
        )

    bad = copy.deepcopy(payload)
    bad.pop("amendment_v5")
    with pytest.raises(seal.TargetFreeSealError, match="top-level schema"):
        seal.validate_control_payload(
            "code_seal", bad, amendment=amendment, repo_root=repo
        )
...
def test_code_seal_mode_is_synthetic_only_and_never_enters_footer_or_final_state(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    amendment: Mapping[str, Any],
) -> None:
    ...
    evidence = _test_evidence(repo, amendment)
    evidence_path = tmp_path / "synthetic_test_evidence.json"
    evidence_path.write_bytes(seal.pretty_json_bytes(evidence))
    monkeypatch.setattr(seal, "validate_amendment", lambda _root: amendment)
    monkeypatch.setattr(
        seal, "validate_sealer_runtime_identity", lambda _amendment: {}
    )
    monkeypatch.setattr(seal, "install_network_guard", lambda: None)
    monkeypatch.setattr(
        seal, "utc_now_exact", lambda: "2026-08-11T20:00:00.000000Z"
    )

    def forbidden(*_args: Any, **_kwargs: Any) -> Any:
        raise AssertionError("code-seal entered a post-GO footer/output validator")

    monkeypatch.setattr(seal, "validate_final_alias_schema_closure", forbidden)
    monkeypatch.setattr(seal, "validate_historical_v3_failure_state", forbidden)
    report = seal.code_seal_mode(root, repo, evidence_path)
    assert report["status"] == "PASS_V5_CODE_SEAL_PUBLISHED_NO_EXECUTION_AUTHORITY"
    assert report["parquet_logical_value_reads"] == 0
    assert (root / seal.CODE_SEAL_RELATIVE).is_file()
    assert not (root / seal.OUTPUT_ROOT_RELATIVE).exists()
    for relative in (
        seal.AUTHORIZATION_RELATIVE,
        seal.REVIEW_RELATIVE,
        seal.GO_RELATIVE,
        seal.POSTRUN_RELATIVE,
    ):
...
def test_authorization_and_go_contracts_use_frozen_exact14_budget_and_windows_argv(
    ...
    common = {
        "schema_version": 5,
        "created_utc": "2026-08-11T19:00:02.000000Z",
        "attempt_id": attempt,
        "amendment": seal.amendment_identity(),
        "amendment_v4": seal.amendment_v4_identity(),
        "amendment_v5": seal.amendment_v5_identity(),
...
def test_static_self_validator_proves_single_publisher_no_fit_no_network_no_parquet_values() -> None:
    ...
    assert report["status"] == "PASS_V5_SEALER_STATIC_SELF_VALIDATION"
    ...
    assert source_text.count("fail-closed V3 sealer") == 0
    assert source_text.count("forbidden in the V3 sealer") == 0
    assert source_text.count("fail-closed V5 sealer") == 1
    assert source_text.count("forbidden in the V5 sealer") == 1
...
def test_only_authorized_pair_is_touched_by_this_change() -> None:
    assert Path(seal.__file__).name == "seal_noaa_gfs_target_free_duplicate_v5.py"
    assert Path(__file__).name == "test_seal_noaa_gfs_target_free_duplicate_v5.py"
