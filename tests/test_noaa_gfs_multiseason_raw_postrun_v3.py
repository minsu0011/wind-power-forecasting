...
from scripts import audit_noaa_gfs_multiseason_raw_postrun_v3 as subject
...
def copy_v3_predata_controls(root: Path) -> None:
    relatives = [
        subject.V3_FALSE_REJECT_INCIDENT_RELATIVE,
        *subject.V3_V2_CONTROL_RELATIVES.values(),
        subject.V3_ORIGINAL_V1_PROVENANCE["original_raw_authorization_v1"]["path"],
        subject.V3_ORIGINAL_V1_PROVENANCE[
...
def synthetic_v3_test_evidence(
    created_utc: str,
    *,
    v3_auditor: dict[str, object],
    v3_auditor_test: dict[str, object],
    v3_sealer: dict[str, object],
    v3_sealer_test: dict[str, object],
) -> dict[str, object]:
    bound = {
        "v2_auditor": subject.V3_SUPERSEDED_V2_AUDITOR_IDENTITY,
        "v2_auditor_test": subject.V3_SUPERSEDED_V2_AUDITOR_TEST_IDENTITY,
        "v3_auditor": v3_auditor,
        "v3_auditor_test": v3_auditor_test,
        "v3_sealer": v3_sealer,
        "v3_sealer_test": v3_sealer_test,
    }
    ...
    for role, relative in {
        ...
        runs[role] = {
            "command": subject._v3_test_command(relative),
            "exit_code": 0,
            "summary": "1 passed",
            "stdout_sha256": "0" * 64,
    ...
    return {
        ...
        "source_compile": {
            ...
            "result": "PASS",
            "files": [v3_auditor, v3_auditor_test, v3_sealer, v3_sealer_test],
        },
        "pytest_isolation": "ONE_TEST_FILE_PER_CLEAN_NETWORK_DENIED_SUBPROCESS",
        "test_runs": runs,
        "required_test_names": list(subject.V3_REQUIRED_TEST_NAMES),
...
def build_valid_v3_authorization_with_unavailable_review(
    root: Path,
    *,
    sealer: Path,
    sealer_test: Path,
) -> tuple[Path, Path]:
    copy_v3_predata_controls(root)
    created_utc = "2026-08-10T22:00:00.000000Z"
    audit_attempt_id = "postrun_audit_v3__20260810T220000000000Z"
    progress_inventory = [
    ...
    progress = {
        ...
        "inventory_canonical_sha256": subject.V3_PROGRESS_INVENTORY_CANONICAL_SHA256,
        "final_checkpoint": subject.V3_FINAL_PROGRESS_IDENTITY,
    }
    v3_auditor = artifact(Path(subject.__file__).resolve())
    v3_auditor_test = artifact(subject.AUDITOR_TEST)
    v3_sealer = artifact(sealer)
    v3_sealer_test = artifact(sealer_test)
    evidence = synthetic_v3_test_evidence(
        created_utc,
        v3_auditor=v3_auditor,
        v3_auditor_test=v3_auditor_test,
        v3_sealer=v3_sealer,
    ...
    authorization_path = root / subject.V3_AUTH_RELATIVE
    go_path = root / subject.V3_GO_RELATIVE
    authorization = {
        "schema_version": 3,
        ...
        "incident": artifact(root / subject.V3_FALSE_REJECT_INCIDENT_RELATIVE, root),
        "superseded_v2_auditor": subject.V3_SUPERSEDED_V2_AUDITOR_IDENTITY,
        "superseded_v2_auditor_test": subject.V3_SUPERSEDED_V2_AUDITOR_TEST_IDENTITY,
        "v3_auditor": v3_auditor,
        "v3_auditor_test": v3_auditor_test,
        "v3_sealer": v3_sealer,
        "v3_sealer_test": v3_sealer_test,
        "v2_recovery_controls": {
            ...
            for role, relative in subject.V3_V2_CONTROL_RELATIVES.items()
        },
        "v2_recovery_support": {
            role: artifact(path)
            for role, path in subject.V3_V2_SUPPORT_PATHS.items()
        },
        "recovery_attempt_id": subject.V3_RECOVERY_ATTEMPT_ID,
        "recovery_transaction": subject.V3_TRANSACTION_IDENTITIES,
        "recovery_history": {
            "file_count": 3,
            "completion_locks": subject.V3_COMPLETION_LOCK_IDENTITIES,
            "postcommit_input_audit": subject.V3_POSTCOMMIT_INPUT_AUDIT_IDENTITY,
        },
        "recovery_progress": progress,
        "canonical_outputs": subject.V3_CANONICAL_OUTPUT_IDENTITIES,
        "original_v1_provenance": subject.V3_ORIGINAL_V1_PROVENANCE,
        "preaudit_zero_mutation_snapshot": {
            "incident_postfailure_state_canonical_sha256": (
                subject.V3_INCIDENT_POSTFAILURE_STATE_CANONICAL_SHA256
            ),
            "recovered_afterstate_canonical_sha256": (
                subject.V3_RECOVERED_AFTERSTATE_CANONICAL_SHA256
            ),
            "active_locks": {
            ...
            "v3_control_temporary_files": {
                "checked_destination_relative_paths": [
                    subject.V3_AUTH_RELATIVE,
                    subject.V3_REVIEW_RELATIVE,
                    subject.V3_GO_RELATIVE,
                ],
        ...
        "runtime_identity_sha256": subject.V3_RUNTIME_IDENTITY_SHA256,
        "test_evidence": evidence,
        "required_command": subject._v3_required_command(root, authorization_path, go_path),
    ...
    write_json(authorization_path, authorization)
    afterstate = subject._v3_recovered_afterstate_from_authorization(authorization)
    missing_review = root / subject.V3_REVIEW_RELATIVE
    go = {
        "schema_version": 3,
        "artifact_type": "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_GO_V3",
        "status": "GO_V3_READ_ONLY_NETWORK_ZERO_FULL_OFFLINE_REPLAY",
...
def complete_v3_review_and_go(
    root: Path, authorization_path: Path, go_path: Path
) -> Path:
    authorization = json.loads(authorization_path.read_text(encoding="utf-8"))
    afterstate = subject._v3_recovered_afterstate_from_authorization(authorization)
    review_path = root / subject.V3_REVIEW_RELATIVE
    recheck = {
    ...
    review = {
        "schema_version": 3,
        "artifact_type": "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_REVIEW_V3",
        "status": "PASS_PENDING_INDEPENDENT_GO_V3",
        ...
        "superseded_v2_auditor": authorization["superseded_v2_auditor"],
        "superseded_v2_auditor_test": authorization["superseded_v2_auditor_test"],
        "v3_auditor": authorization["v3_auditor"],
        "v3_auditor_test": authorization["v3_auditor_test"],
        "v3_sealer": authorization["v3_sealer"],
        "v3_sealer_test": authorization["v3_sealer_test"],
        "v2_recovery_controls": authorization["v2_recovery_controls"],
        "recovered_afterstate": afterstate,
        "test_evidence_recheck": recheck,
        "independent_checks": {key: True for key in subject.V3_REVIEW_CHECKS},
        "network_requests_allowed": 0,
        "audit_files_written_allowed": 0,
        "max_spawn_processes": 7,
        "labels_read_allowed": False,
...
def test_complete_synthetic_producer_passes_and_audit_is_read_only(tmp_path: Path) -> None:
    ...
    assert report["status"] == "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V3"
...
def test_v3_worker_is_importable_in_exactly_seven_spawn_processes(
    tmp_path: Path,
) -> None:
    pilot = (
        subject.REPO
    ...
    with concurrent.futures.ProcessPoolExecutor(
        ...
        mp_context=context,
        initializer=subject.v3_spawn_probe_barrier_initializer,
        initargs=(barrier,),
    ) as pool:
        futures = [
...
def test_cli_normalizes_malformed_artifacts_to_structured_fail(
    ...
    assert report["status"] == "FAIL_PRODUCER_POSTRUN_AUDIT_V3"
...
def test_real_immutable_four_key_independent_prelaunch_identity_passes(
    ...
    record = authorization["independent_prelaunch_audit"]
    assert record == subject.V3_ORIGINAL_V1_PROVENANCE[
        "original_independent_prelaunch_audit_v1"
    ]
...
def test_frozen_v2_false_reject_incident_and_source_identities_are_exact() -> None:
    root = subject.ROOT_DEFAULT
    incident_path = root / subject.V3_FALSE_REJECT_INCIDENT_RELATIVE
    assert incident_path.stat().st_size == subject.V3_FALSE_REJECT_INCIDENT_SIZE_BYTES
    assert subject.sha256_file(incident_path) == subject.V3_FALSE_REJECT_INCIDENT_SHA256
    incident = json.loads(incident_path.read_text(encoding="utf-8"))
    ...
    assert artifact(subject.SUPERSEDED_V2_AUDITOR) == subject.V3_SUPERSEDED_V2_AUDITOR_IDENTITY
    assert artifact(subject.SUPERSEDED_V2_AUDITOR_TEST) == subject.V3_SUPERSEDED_V2_AUDITOR_TEST_IDENTITY


def test_v3_auditor_is_stdout_only_and_writes_zero_files(tmp_path: Path) -> None:
    root, runner, contract = build_complete_fixture(tmp_path)
    before = snapshot(tmp_path)
    report = audit_fixture(root, runner, contract)
    assert report["audit_files_written"] == 0
...
def test_v3_authority_and_go_precede_any_parquet_or_data_read(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    authority_failure: str,
) -> None:
    root = tmp_path / "authority-first"
    sealer = tmp_path / "seal_v3.py"
    sealer_test = tmp_path / "test_seal_v3.py"
    sealer.write_text("# synthetic frozen V3 sealer\n", encoding="utf-8")
    sealer_test.write_text("# synthetic frozen V3 sealer test\n", encoding="utf-8")
    monkeypatch.setattr(subject, "V3_SEALER", sealer)
    monkeypatch.setattr(subject, "V3_SEALER_TEST", sealer_test)
    authorization_path, go_path = build_valid_v3_authorization_with_unavailable_review(
        root, sealer=sealer, sealer_test=sealer_test
    )
    expected_reason = "independent review file is absent"
    if authority_failure == "missing_go":
        go_path.unlink()
        expected_reason = "required JSON is absent"
    ...
    def forbidden(*_args: object, **_kwargs: object) -> object:
        raise AssertionError("data/parquet/decoder route was reached before V3 authority")
    ...
    def guarded_open(path: Path, *args: object, **kwargs: object) -> object:
        ...
        if lowered.endswith((".parquet", ".grib2", ".csv")):
            raise AssertionError("data file opened before V3 authority")
        return original_open(path, *args, **kwargs)
...
@pytest.mark.parametrize(
    ...
    (
        ("missing_v3_sealer", "authorization schema mismatch"),
        ("extra_v3_sealer_alias", "authorization schema mismatch"),
        ("support_alias", "V2-support binding schema mismatch"),
    ),
)
def test_v3_authorization_rejects_sealer_schema_and_support_aliases_before_data(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
    expected_reason: str,
    ...
    authorization = json.loads(authorization_path.read_text(encoding="utf-8"))
    if mutation == "missing_v3_sealer":
        authorization.pop("v3_sealer")
    elif mutation == "extra_v3_sealer_alias":
        authorization["sealer"] = authorization["v3_sealer"]
    else:
    ...
    def forbidden(*_args: object, **_kwargs: object) -> object:
        raise AssertionError("data route was reached before V3 authority rejection")
...
def test_v3_predata_authority_accepts_complete_production_shaped_control_chain(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    review_path = complete_v3_review_and_go(root, authorization_path, go_path)
    authority = subject._v3_validate_predata_authority(
...
def test_v3_review_and_go_schema_or_verdict_tamper_fails_closed(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    control: str,
    mutation: str,
    ...
    if control == "review":
        ...
        write_json(go_path, go)
    with pytest.raises(subject.AuditFailure, match=expected_reason):
        subject._v3_validate_predata_authority(root, authorization_path, go_path)


def build_v3_postauthority_fixture(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> tuple[Path, dict[str, object]]:
    root = tmp_path / "postauthority"
    for relative in (subject.V3_AUTH_RELATIVE, subject.V3_REVIEW_RELATIVE, subject.V3_GO_RELATIVE):
    ...
    afterstate = subject._v3_recovered_afterstate_from_authorization(authorization)
    authority = {"authorization": authorization, "recovered_afterstate": afterstate}

    for record in subject.V3_CANONICAL_OUTPUT_IDENTITIES.values():
        path = root / str(record["path"])
        path.parent.mkdir(parents=True, exist_ok=True)
        path.touch()
    for record in subject.V3_TRANSACTION_IDENTITIES.values():
        path = root / str(record["path"])
        path.parent.mkdir(parents=True, exist_ok=True)
        path.touch()
    for record in subject.V3_COMPLETION_LOCK_IDENTITIES.values():
        path = root / str(record["path"])
        path.parent.mkdir(parents=True, exist_ok=True)
        path.touch()
    postcommit = root / str(subject.V3_POSTCOMMIT_INPUT_AUDIT_IDENTITY["path"])
    ...
    source_v2_authorization = json.loads(
        (
            ...
            / subject.V3_V2_CONTROL_RELATIVES["recovery_authorization_v2"]
        ).read_text(encoding="utf-8")
    )
    remnants = source_v2_authorization["documented_preplan_remnants"]
    v2_authorization_path = (
        root / subject.V3_V2_CONTROL_RELATIVES["recovery_authorization_v2"]
    )
    ...
    for record in remnants:
        ...
        path.touch()
    for relative in (
        f"raw/output_transactions/{subject.V3_RECOVERY_ATTEMPT_ID}/staged/decoded",
        f"raw/output_transactions/{subject.V3_RECOVERY_ATTEMPT_ID}/staged/raw",
    ):
...
def test_v3_postauthority_recursive_closure_rejects_junk(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, mutation: str
) -> None:
    root, authority = build_v3_postauthority_fixture(tmp_path, monkeypatch)
    if mutation == "extra_staged":
        junk = (
            root
            / "raw"
            / "output_transactions"
            / subject.V3_RECOVERY_ATTEMPT_ID
            / "staged"
            / "decoded"
            / "junk.bin"
        )
    ...
    with pytest.raises(subject.AuditFailure, match=expected):
        subject._v3_validate_postauthority_afterstate(root, authority)


def test_v3_postauthority_exact_closure_reports_bound_counts_and_digest(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root, authority = build_v3_postauthority_fixture(tmp_path, monkeypatch)
    summary = subject._v3_validate_postauthority_afterstate(root, authority)
    assert summary == {
...
def test_v3_postauthority_recursive_closure_rejects_symlink(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root, authority = build_v3_postauthority_fixture(tmp_path, monkeypatch)
    target = root / str(subject.V3_TRANSACTION_IDENTITIES["plan"]["path"])
...
def _run_v3_cli(
    ...
    *,
    ...
    if include_dash_b:
        command.append("-B")
    if module:
        command += ["-m", "scripts.audit_noaa_gfs_multiseason_raw_postrun_v3"]
    else:
        command += [str(Path(subject.__file__).resolve())]
    command += ["--root", str(root.resolve())]
    if include_controls:
        command += [
            "--authorization", str((root / subject.V3_AUTH_RELATIVE).resolve()),
            "--independent-go", str((root / subject.V3_GO_RELATIVE).resolve()),
        ]
...
def test_direct_file_entrypoint_is_rejected_before_authority_or_data(
    ...
    result = _run_v3_cli(tmp_path, module=False)
    assert result.returncode == 1
    report = json.loads(result.stdout)
...
def test_missing_dash_b_is_rejected_before_authority_or_data(
    ...
    result = _run_v3_cli(tmp_path, include_dash_b=False)
    assert result.returncode == 1
    report = json.loads(result.stdout)
...
def test_omitted_explicit_control_args_are_rejected_before_data(tmp_path: Path) -> None:
    result = _run_v3_cli(tmp_path, include_controls=False)
    assert result.returncode == 1
    report = json.loads(result.stdout)
...
def test_recovered_afterstate_completion_lock_key_names_are_digest_bound() -> None:
    ...
    authorization = {
        ...
        "recovery_progress": {
            ...
            "inventory_canonical_sha256": (
                subject.V3_PROGRESS_INVENTORY_CANONICAL_SHA256
            ),
            "final_checkpoint": subject.V3_FINAL_PROGRESS_IDENTITY,
        },
        "canonical_outputs": subject.V3_CANONICAL_OUTPUT_IDENTITIES,
    ...
    afterstate = subject._v3_recovered_afterstate_from_authorization(authorization)
    assert len(subject._v3_canonical_bytes(afterstate)) == 24_234
    ...
    with pytest.raises(subject.AuditFailure, match="recovery-history declaration"):
        subject._v3_recovered_afterstate_from_authorization(tampered)
