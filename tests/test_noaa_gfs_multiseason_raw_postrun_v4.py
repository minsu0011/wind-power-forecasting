...
from scripts import audit_noaa_gfs_multiseason_raw_postrun_v4 as subject
...
def test_complete_synthetic_producer_passes_and_audit_is_read_only(tmp_path: Path) -> None:
    ...
    assert report["status"] == "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V4"
...
def test_cli_normalizes_malformed_artifacts_to_structured_fail(
    ...
    assert report["status"] == "FAIL_PRODUCER_POSTRUN_AUDIT_V4"
...
def _run_v4_cli(
    ...
    *,
    ...
    if include_dash_b:
        command.append("-B")
    if module:
        command += ["-m", "scripts.audit_noaa_gfs_multiseason_raw_postrun_v4"]
    else:
        command += [str(Path(subject.__file__).resolve())]
    command += ["--root", str(root.resolve())]
    if include_controls:
        command += [
            "--authorization", str((root / subject.V4_AUTH_RELATIVE).resolve()),
            "--independent-go", str((root / subject.V4_GO_RELATIVE).resolve()),
        ]
...
def test_direct_file_entrypoint_is_rejected_before_authority_or_data(
    ...
    result = _run_v4_cli(tmp_path, module=False)
    assert result.returncode == 1
    report = json.loads(result.stdout)
...
def test_missing_dash_b_is_rejected_before_authority_or_data(
    ...
    result = _run_v4_cli(tmp_path, include_dash_b=False)
    assert result.returncode == 1
    report = json.loads(result.stdout)
...
def test_omitted_explicit_control_args_are_rejected_before_data(tmp_path: Path) -> None:
    result = _run_v4_cli(tmp_path, include_controls=False)
    assert result.returncode == 1
    report = json.loads(result.stdout)
...
def _assert_v4_predata_failure_without_data(
    ...
    monkeypatch.setattr(subject, "_V4_CANONICAL_MAIN_ACTIVE", True)
    with pytest.raises(subject.AuditFailure, match=match):
        subject.audit(
            root,
            contract=subject.PRODUCTION_CONTRACT,
            runner_path=subject.RUNNER_DEFAULT,
            authorization_path=authorization_path,
            independent_go_path=go_path,
        )
