...
from scripts import audit_noaa_gfs_multiseason_raw_postrun_v5 as subject
...
def test_complete_synthetic_producer_passes_and_audit_is_read_only(tmp_path: Path) -> None:
    ...
    assert report["status"] == "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V5"
...
def test_v4_review_transport_truncation_incident_and_frozen_chain_are_exact() -> None:
    ...
    v5_tree = ast.parse(Path(subject.__file__).read_text(encoding="utf-8"))
    v4_functions = {
        node.name: node
...
def test_v5_auditor_is_stdout_only_and_writes_zero_files(tmp_path: Path) -> None:
    root, runner, contract = build_complete_fixture(tmp_path)
    before = snapshot(tmp_path)
    report = audit_fixture(root, runner, contract)
    assert report["artifact_type"] == "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V5"
    assert report["audit_files_written"] == 0
    assert report["audit_network_requests"] == 0
    assert snapshot(tmp_path) == before
