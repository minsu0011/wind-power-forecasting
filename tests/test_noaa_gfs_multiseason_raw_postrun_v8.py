...
from scripts import audit_noaa_gfs_multiseason_raw_postrun_v8 as v8_subject
...
from scripts import seal_noaa_gfs_multiseason_postrun_audit_v8 as v8_sealer
...
def test_full_recovered_production_shape_reaches_transaction_raw_decoded_and_offline_replay(
    ...
    root, runner, contract = build_recovered_fixture(tmp_path)
    report = v8_subject.audit(
        root,
        contract=contract,
        runner_path=runner,
        _test_message_decoder=synthetic_decoder,
    )
    assert report["status"] == "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V8"
    assert report["launch_and_transaction"]["transaction_inventory_recursively_closed"] is True
...
def test_network_target_label_model_and_submission_routes_remain_absent() -> None:
    source = Path(v8_subject.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source)
    imports = {
...
def test_compact_commitment_field_or_inventory_digest_tamper_fails(
    ...
    root = tmp_path / "r"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    authorization = json.loads(authorization_path.read_text(encoding="utf-8"))
    review = json.loads(review_path.read_text(encoding="utf-8"))
    go = json.loads(go_path.read_text(encoding="utf-8"))
    value = authorization["recovered_afterstate_commitment"][field]
    ...
    go["independent_review"] = artifact(review_path, root)
    write_json(go_path, go)
    with pytest.raises(v8_subject.AuditFailure, match="commitment"):
        v8_subject._v8_validate_predata_authority(root, authorization_path, go_path)
...
def test_unbound_matching_control_namespace_entry_fails_before_data(
    ...
    root = tmp_path / "v8-unbound-namespace"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    extra = root / "incidents" / ".TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_EVIL.json"
    extra.write_text("{}\n", encoding="utf-8")
    called = False
    ...
    monkeypatch.setattr(v8_subject, "preflight_zero_target_parquet_schemas", forbidden)
    with pytest.raises(v8_subject.AuditFailure, match="namespace inventory"):
        v8_subject._v8_validate_predata_authority(root, authorization_path, go_path)
...
def test_duplicate_json_key_rejection_is_not_bypassed_by_standard_last_wins_parse(
    ...
    root = tmp_path / "v8-duplicate-key"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    raw = authorization_path.read_text(encoding="utf-8")
    authorization_path.write_text('{"schema_version":8,' + raw[1:], encoding="utf-8")
    assert json.loads(authorization_path.read_text(encoding="utf-8"))["schema_version"] == 8
    with pytest.raises(v8_subject.AuditFailure, match="duplicate JSON key"):
        v8_subject._v8_validate_predata_authority(root, authorization_path, go_path)
...
def test_frozen_v6_reproduces_exact_sealer_timestamp_precision_false_reject() -> None:
    ...
    direct_payload = v8_subject._v5_load_strict_json(
        root / v8_subject.RECOVERY_SEALER_ENTRYPOINT_INCIDENT_RELATIVE,
        label="actual immutable 9e74 timestamp source",
    )
    assert direct_payload["created_utc"] == (
        "2026-08-10T18:45:00.5807528Z"
    )
    ...
    frozen_inputs = {
        ...
        "v5_authority_base": dict(frozen_v6_sealer.V5_AUTHORITY_BASE),
        "historical_failed_head_identity_contract": (
            frozen_v6_sealer.expected_historical_failed_head_identity_contract()
        ),
        "recovered_afterstate_commitment": dict(
            frozen_v6_subject.V5_RECOVERED_AFTERSTATE_COMMITMENT
        ),
        "historical_incident_created_utc": direct_payload["created_utc"],
        "v5_authorization_created_utc": "must not be parsed",
        "v5_review_created_utc": "must not be parsed",
        "v5_go_created_utc": "must not be parsed",
        "incident_created_utc": "must not be parsed",
    }
...
def test_v7_actual_9e74_collect_build_and_self_validator_passes() -> None:
    """Bind the original preseal proof to the now-sealed immutable V7 chain."""

    root = subject.ROOT_DEFAULT.resolve()
    base = v8_subject._v8_load_and_validate_v7_authority_base(
        root, v8_subject._v8_expected_v7_authority_base()
    )
    direct_payload = v8_subject._v5_load_strict_json(
        root / v8_subject.RECOVERY_SEALER_ENTRYPOINT_INCIDENT_RELATIVE,
        label="sealed V7 actual immutable 9e74",
    )
    assert direct_payload["created_utc"] == (
        "2026-08-10T18:45:00.5807528Z"
    )
    inherited = v8_subject._v8_frozen_v7_preseal_test_evidence(base)
    assert inherited["original_full_runs"]["v7_auditor_tests"]["exit_code"] == 0
    assert inherited["original_full_runs"]["v7_sealer_tests"]["exit_code"] == 0
    assert base["summary"] == v8_subject._v8_expected_v7_authority_base()
...
def copy_v8_predata_controls(root: Path) -> None:
    """Copy immutable metadata needed by the V8 authority gate; no data files."""

    copy_v7_predata_controls(root)
    for relative in (
        v8_subject.V7_AUTH_RELATIVE,
        v8_subject.V7_REVIEW_RELATIVE,
        v8_subject.V7_GO_RELATIVE,
        v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
        v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE,
    ):
        source = v8_subject.ROOT_DEFAULT / relative
        destination = root / relative
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, destination)


def synthetic_v8_test_evidence(
    created_utc: str, bound: dict[str, dict[str, object]]
) -> dict[str, object]:
    runs: dict[str, object] = {}
    for role, relative in {
        ...
        runs[role] = {
            "command": v8_subject._v8_test_command(relative),
            "exit_code": 0,
            "summary": summaries.get(role, "1 passed in 0.01s"),
            "stdout_sha256": "0" * 64,
            "stderr_sha256": (
                "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
            ),
            ...
            "selection_mode": (
                ...
                if role.startswith("frozen_v7_") else "FULL_FILE"
            ),
            "deselected_nodeids": (
                list(v8_subject.V8_FROZEN_V7_TEST_DESELECTS[relative])
                if role.startswith("frozen_v7_") else []
            ),
            "deselected_count": (
                len(v8_subject.V8_FROZEN_V7_TEST_DESELECTS[relative])
                if role.startswith("frozen_v7_") else 0
            ),
        }
    return {
        "schema_version": 1,
        ...
        "source_compile": {
            ...
            "result": "PASS",
            "files": [
                bound["v8_auditor"], bound["v8_auditor_test"],
                bound["v8_sealer"], bound["v8_sealer_test"],
            ],
        },
        "pytest_isolation": "ONE_TEST_FILE_PER_CLEAN_NETWORK_DENIED_SUBPROCESS",
        "test_runs": runs,
        "required_test_names": list(v8_subject.V8_REQUIRED_TEST_NAMES),
        "required_test_names_present": True,
        "all_exit_codes_zero": True,
        "production_shape_regression": {
            key: True for key in v8_subject.V8_PRODUCTION_SHAPE_KEYS
        },
        "real_seven_spawn_regression": {
            "start_method": "spawn",
        ...
        "models_fit": 0,
        "submission_csv_created": False,
        "frozen_v7_preseal_test_evidence": {
            "authorization": dict(
                v8_subject.FROZEN_V7_CONTROL_IDENTITIES["authorization"]
            ),
            ...
            "state_obsolete_current_rerun_nodeids": [
                *v8_subject.V8_FROZEN_V7_TEST_DESELECTS[
...
def build_valid_v8_authority(
    root: Path,
) -> tuple[Path, Path, Path, dict[str, object]]:
    copy_v8_predata_controls(root)
    primary_record = artifact(root / v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE, root)
    correction_record = artifact(
        root / v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE, root
    )
    failure_chain = v8_subject._v8_validate_duration_suffix_incident_chain(
        root, primary_record, correction_record
    )
    preflight = v8_subject._v8_validate_actual_v2_preflight_evidence(root)
    source_identities = {
        **v8_subject.FROZEN_V7_SOURCE_IDENTITIES,
        "v8_auditor": artifact(Path(v8_subject.__file__).resolve()),
        "v8_auditor_test": artifact(v8_subject.AUDITOR_TEST),
        "v8_sealer": artifact(v8_subject.V8_SEALER),
        "v8_sealer_test": artifact(v8_subject.V8_SEALER_TEST),
    }
    bound = {
        "v7_authorization": dict(
            v8_subject.FROZEN_V7_CONTROL_IDENTITIES["authorization"]
        ),
        "v7_independent_review": dict(
            v8_subject.FROZEN_V7_CONTROL_IDENTITIES["independent_review"]
        ),
        "v7_independent_go": dict(
            v8_subject.FROZEN_V7_CONTROL_IDENTITIES["independent_go"]
        ),
        "primary_duration_suffix_incident": primary_record,
        "afterstate_commitment_correction": correction_record,
        "immutable_v2_preflight": preflight["preflight_identity"],
        **source_identities,
    }
    created_utc = "2026-08-11T07:00:00.000000Z"
    attempt_id = "postrun_audit_v8__20260811T070000000000Z"
    authorization_path = root / v8_subject.V8_AUTH_RELATIVE
    review_path = root / v8_subject.V8_REVIEW_RELATIVE
    go_path = root / v8_subject.V8_GO_RELATIVE
    policy = {
        "max_spawn_processes": 7,
        "network_requests_allowed": 0,
        "audit_files_written_allowed": 0,
    ...
    authorization = {
        ...
        "created_utc": created_utc,
        "audit_attempt_id": attempt_id,
        "v7_authority_base": v8_subject._v8_expected_v7_authority_base(),
        "v7_live_failure_state": failure_chain["state"],
        "immutable_v2_preflight_evidence": preflight["commitment"],
        **source_identities,
        "recovery_attempt_id": v8_subject.FROZEN_V7_RECOVERY_ATTEMPT_ID,
        "recovered_afterstate_commitment": dict(
            v8_subject.V5_RECOVERED_AFTERSTATE_COMMITMENT
        ),
        "preaudit_zero_mutation_snapshot": {
            "v7_live_failure_state_canonical_sha256": (
                v8_subject.canonical_payload_sha256(failure_chain["state"])
            ),
            "recovered_afterstate_commitment_canonical_sha256": (
                v8_subject.V5_RECOVERED_AFTERSTATE_COMMITMENT_CANONICAL_SHA256
            ),
            ...
            "active_locks": {
                "raw": {"path": "raw/RAW_LAUNCH_ACTIVE.lock", "present": False},
                "decoded": {
                    "path": "decoded/DECODE_RECOVERY_ACTIVE.lock", "present": False,
                },
            },
            "v8_control_temporary_files": {
                "checked_destination_relative_paths": list(
                    v8_subject._v8_control_destination_relatives()
                ),
                "matching_temporary_files": [],
            },
            "postrun_audit_output_files_present": 0,
        ...
        "runtime_identity_sha256": v8_subject.V4_RUNTIME_IDENTITY_SHA256,
        "test_evidence": synthetic_v8_test_evidence(created_utc, bound),
        "required_command": v8_subject._v8_required_command(
            root, authorization_path, go_path
        ),
        **policy,
        "independent_review_required": True,
    ...
    assert set(authorization) == v8_subject.V8_AUTH_KEYS
    write_json(authorization_path, authorization)
    common = {
        field: authorization[field]
        for field in (
            ...
            *v8_subject.FROZEN_V7_SOURCE_IDENTITIES.keys(),
    ...
    review = {
        "schema_version": 8,
        "artifact_type": "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_REVIEW_V8",
        "status": "PASS_PENDING_INDEPENDENT_GO_V8",
        "verdict": "GO_RECOMMENDED",
        "created_utc": "2026-08-11T07:01:00.000000Z",
        "audit_attempt_id": attempt_id,
        "authorization": artifact(authorization_path, root),
        **common,
        "test_evidence_recheck": {
            "authorization_test_evidence_canonical_sha256": (
                v8_subject.canonical_payload_sha256(authorization["test_evidence"])
            ),
            "bound_identities_rehashed": True,
            "source_compile_exact": True,
            "required_test_names_exact": True,
        ...
        "independent_checks": {key: True for key in v8_subject.V8_REVIEW_CHECKS},
        **policy,
        "auditor_execution_started": False,
        "independent_go_required": True,
    }
    assert set(review) == v8_subject.V8_REVIEW_KEYS
    write_json(review_path, review)
    go = {
        "schema_version": 8,
        "artifact_type": "TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_GO_V8",
        "status": "GO_V8_READ_ONLY_NETWORK_ZERO_FULL_OFFLINE_REPLAY",
        "created_utc": "2026-08-11T07:02:00.000000Z",
        "audit_attempt_id": attempt_id,
        "authorization": artifact(authorization_path, root),
        "independent_review": artifact(review_path, root),
        **common,
    ...
    assert set(go) == v8_subject.V8_GO_KEYS
    write_json(go_path, go)
    return authorization_path, review_path, go_path, authorization


def write_rebound_v8_chain(
    root: Path,
    authorization_path: Path,
    review_path: Path,
    go_path: Path,
...
def test_real_immutable_v7_authority_base_passes() -> None:
    result = v8_subject._v8_load_and_validate_v7_authority_base(
        v8_subject.ROOT_DEFAULT, v8_subject._v8_expected_v7_authority_base()
    )
    assert result["summary"] == v8_subject._v8_expected_v7_authority_base()
    assert result["authorization_created"]["ticks_100ns"] < result["review_created"]["ticks_100ns"] < result["go_created"]["ticks_100ns"]


@pytest.mark.parametrize(
    ("field", "replacement"),
    (
        ("authorization", {"path": "wrong.json", "size_bytes": 1, "sha256": "0" * 64}),
        ("authorization_status", "WRONG"),
        ("authorization_payload_canonical_sha256", "0" * 64),
        ("audit_attempt_id", "postrun_audit_v7__20000101T000000000000Z"),
    ),
)
def test_v7_authority_base_path_size_sha_schema_status_or_digest_tamper_fails(
    field: str, replacement: object
) -> None:
    base = v8_subject._v8_expected_v7_authority_base()
    base[field] = replacement
    with pytest.raises(v8_subject.AuditFailure, match="authority-base summary"):
        v8_subject._v8_load_and_validate_v7_authority_base(
            v8_subject.ROOT_DEFAULT, base
        )


def test_v7_duration_suffix_false_reject_primary_incident_is_exact() -> None:
    path = v8_subject.ROOT_DEFAULT / v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE
    assert artifact(path, v8_subject.ROOT_DEFAULT) == {
        "path": v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
        "size_bytes": v8_subject.V8_DURATION_SUFFIX_INCIDENT_SIZE_BYTES,
        "sha256": v8_subject.V8_DURATION_SUFFIX_INCIDENT_SHA256,
    }
    payload = v8_subject._v5_load_strict_json(path, label="V8 primary incident test")
    assert len(v8_subject._v4_canonical_bytes(payload)) == v8_subject.V8_DURATION_SUFFIX_INCIDENT_CANONICAL_BYTES
    assert v8_subject.canonical_payload_sha256(payload) == v8_subject.V8_DURATION_SUFFIX_INCIDENT_CANONICAL_SHA256


def test_v7_duration_suffix_afterstate_commitment_correction_is_exact() -> None:
    path = v8_subject.ROOT_DEFAULT / v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE
    assert artifact(path, v8_subject.ROOT_DEFAULT) == {
        "path": v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE,
        "size_bytes": v8_subject.V8_DURATION_SUFFIX_CORRECTION_SIZE_BYTES,
        "sha256": v8_subject.V8_DURATION_SUFFIX_CORRECTION_SHA256,
    }
    chain = v8_subject._v8_validate_duration_suffix_incident_chain(
        v8_subject.ROOT_DEFAULT,
        {
            "path": v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
            "size_bytes": v8_subject.V8_DURATION_SUFFIX_INCIDENT_SIZE_BYTES,
            "sha256": v8_subject.V8_DURATION_SUFFIX_INCIDENT_SHA256,
    ...
    assert chain["state"]["corrected_logical_primary_incident"]["canonical_sha256"] == v8_subject.V8_CORRECTED_DURATION_INCIDENT_CANONICAL_SHA256


def test_primary_incident_alone_fails_and_dual_bound_correction_passes() -> None:
    primary = {
        "path": v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
        "size_bytes": v8_subject.V8_DURATION_SUFFIX_INCIDENT_SIZE_BYTES,
        "sha256": v8_subject.V8_DURATION_SUFFIX_INCIDENT_SHA256,
    ...
    with pytest.raises(v8_subject.AuditFailure):
        v8_subject._v8_validate_duration_suffix_incident_chain(
            v8_subject.ROOT_DEFAULT, primary, None
        )
    assert v8_subject._v8_validate_duration_suffix_incident_chain(
        v8_subject.ROOT_DEFAULT, primary, correction
    )["state"]["primary_incident_alone_must_fail"] is True


@pytest.mark.parametrize("field", ("path", "size_bytes", "sha256"))
def test_correction_pointer_value_identity_or_logical_digest_tamper_fails(
    field: str,
) -> None:
    correction = {
        "path": v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE,
    ...
    correction[field] = (
        "wrong.json" if field == "path" else correction[field] + 1
        if field == "size_bytes" else "0" * 64
    )
    with pytest.raises(v8_subject.AuditFailure):
        v8_subject._v8_validate_duration_suffix_incident_chain(
            v8_subject.ROOT_DEFAULT,
...
def test_frozen_v7_reproduces_exact_immutable_pytest_duration_suffix_false_reject() -> None:
    root = subject.ROOT_DEFAULT
    authorization = subject._v5_load_strict_json(
        root / subject.RECOVERY_AUTH_RELATIVE, label="frozen V7 recovery auth"
    )
    preflight = subject._v5_load_strict_json(
        root / subject.RECOVERY_REAL_PREFLIGHT_RELATIVE,
        label="frozen V7 recovery preflight",
    )
    external_paths = {
        "original_runner": subject.RUNNER_DEFAULT,
        "recovery_runner": subject.RECOVERY_RUNNER,
        "recovery_module": subject.RECOVERY_MODULE,
        "recovery_bootstrap": subject.RECOVERY_BOOTSTRAP,
        "recovery_test": subject.RECOVERY_TEST,
    ...
    external = {
        role: subject.verify_identity(authorization[role], path, root=None, label=role)
        for role, path in external_paths.items()
    }
    sealers = {
        role: subject.verify_identity(preflight[role], path, root=None, label=role)
        for role, path in {
            "recovery_sealer": subject.RECOVERY_SEALER,
            "recovery_sealer_test": subject.RECOVERY_SEALER_TEST,
        }.items()
    }
    with pytest.raises(
        subject.AuditFailure,
        match="immutable recovery pytest result invalid: tests/test_noaa_gfs_multiseason_raw_postrun_v2.py",
    ):
        subject._validate_recovery_test_evidence(
            preflight["test_evidence"], preflight, external, sealers
        )


def test_v8_pytest_success_summary_parser_accepts_actual_six_summaries() -> None:
    result = v8_subject._v8_validate_actual_v2_preflight_evidence(
        v8_subject.ROOT_DEFAULT
    )
    parsed = [
        v8_subject._v8_parse_pytest_success_summary(value, label=value)
        for value in result["summaries"]
    ]
    assert len(parsed) == 6
    assert parsed[-1]["duration_hundredths"] == 6_533
    assert parsed[-1]["suffix_total_seconds"] == 65


@pytest.mark.parametrize(
    "summary",
    (
        "1 passed in 65s", "1 passed in 65.3s", "1 passed in 65.333s",
        "1 passed in 65.33s", "1 passed in 60.00s",
        "1 passed in 60.00s (0:01:00)", "1 passed in 61.00s (0:01:00)",
        "1 passed in 61.00s (0:01:01)",
    ),
)
def test_v8_pytest_success_summary_parser_accepts_legitimate_rounding_boundaries(
    summary: str,
) -> None:
    assert v8_subject._v8_parse_pytest_success_summary(summary, label=summary)["passed"] == 1


@pytest.mark.parametrize(
    "summary",
    (
        "1 failed in 1.00s", "1 passed in 65.3s (0:01:05)",
        "1 passed in 65.33s (00:01:05)", "1 passed in 65.33s (0:01:04)",
        "1 passed in 59.99s (0:00:59)", "1 passed in 65.33s (+00:01:05)",
        "1 passed in 65.33s (0:1:05)", "１ passed in 65.33s",
    ),
)
def test_v8_pytest_success_summary_parser_rejects_malformed_or_inconsistent_suffixes(
    summary: str,
) -> None:
    with pytest.raises(v8_subject.AuditFailure):
        v8_subject._v8_parse_pytest_success_summary(summary, label="invalid")


def test_actual_v2_preflight_evidence_commitments_are_exact() -> None:
    actual = v8_subject._v8_validate_actual_v2_preflight_evidence(
        v8_subject.ROOT_DEFAULT
    )
    assert actual["commitment"] == v8_subject._v8_expected_immutable_v2_preflight_evidence()
    assert len(actual["summary_pairs"]) == 6
    assert actual["aggregate"] == actual["preflight"]["focused_test_result"]


def test_actual_v2_preflight_evidence_validator_passes() -> None:
    actual = v8_subject._v8_validate_actual_v2_preflight_evidence(
        v8_subject.ROOT_DEFAULT
    )
    assert actual["preflight_identity"] == v8_subject.V8_PREFLIGHT_IDENTITY
    assert actual["summaries"][5] == v8_subject.V8_PREFLIGHT_LONG_DURATION_SUMMARY


@pytest.mark.parametrize(
    "constant",
    (
        "V8_PREFLIGHT_SIZE", "V8_PREFLIGHT_CANONICAL",
        "V8_PREFLIGHT_EVIDENCE", "V8_PREFLIGHT_SUMMARIES",
    ),
)
def test_v2_preflight_physical_canonical_evidence_or_summary_commitment_tamper_fails(
    monkeypatch: pytest.MonkeyPatch, constant: str
) -> None:
    if constant == "V8_PREFLIGHT_SIZE":
        monkeypatch.setitem(v8_subject.V8_PREFLIGHT_IDENTITY, "size_bytes", 1)
    elif constant == "V8_PREFLIGHT_CANONICAL":
        monkeypatch.setattr(v8_subject, "V8_PREFLIGHT_CANONICAL_SHA256", "0" * 64)
    elif constant == "V8_PREFLIGHT_EVIDENCE":
        monkeypatch.setattr(
            v8_subject, "V8_PREFLIGHT_TEST_EVIDENCE_CANONICAL_SHA256", "0" * 64
        )
    else:
        monkeypatch.setattr(
            v8_subject, "V8_PREFLIGHT_ORDERED_SUMMARIES_CANONICAL_SHA256", "0" * 64
        )
    with pytest.raises(v8_subject.AuditFailure):
        v8_subject._v8_validate_actual_v2_preflight_evidence(v8_subject.ROOT_DEFAULT)


def test_v2_preflight_gate_precedes_schema_parquet_data_and_replay(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "v8-preflight-before-data"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    with (root / v8_subject.V8_PREFLIGHT_RELATIVE).open("ab") as handle:
        handle.write(b" ")
    reached: list[str] = []

    def forbidden(*_args: object, **_kwargs: object) -> object:
        reached.append("data")
        raise AssertionError("data route reached")

    monkeypatch.setattr(v8_subject, "preflight_zero_target_parquet_schemas", forbidden)
    monkeypatch.setattr(v8_subject, "_V8_CANONICAL_MAIN_ACTIVE", True)
    with pytest.raises(v8_subject.AuditFailure, match="preflight"):
        v8_subject.audit(
            root,
            contract=v8_subject.PRODUCTION_CONTRACT,
            authorization_path=authorization_path,
            independent_go_path=go_path,
        )
    assert reached == []
...
@pytest.mark.parametrize("drift", ("primary", "correction", "preflight"))
def test_late_reuse_revalidates_v7_base_failure_chain_and_v2_preflight(
    tmp_path: Path, drift: str
) -> None:
    root = tmp_path / f"v8-late-{drift}"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    authority = v8_subject._v8_validate_predata_authority(
        root, authorization_path, go_path
    )
    assert v8_subject._v8_validate_postauthority_control_state(root, authority)
    relative = {
        "primary": v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
        "correction": v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE,
        "preflight": v8_subject.V8_PREFLIGHT_RELATIVE,
    }[drift]
    with (root / relative).open("ab") as handle:
        handle.write(b" ")
    with pytest.raises(v8_subject.AuditFailure):
        v8_subject._v8_validate_postauthority_control_state(root, authority)


def test_v7_production_data_and_replay_core_ast_is_identical() -> None:
    ...
    baseline = functions(Path(frozen_v5_subject.__file__))
    frozen = functions(Path(subject.__file__))
    current = functions(Path(v8_subject.__file__))
    names = list(baseline)
    start = names.index("recovery_progress_counts")
    end = names.index("_audit_full_offline_redecode") + 1
    core = names[start:end]
    assert len(core) == 39
    assert all(
        ast.dump(baseline[name], include_attributes=False)
        == ast.dump(frozen[name], include_attributes=False)
        == ast.dump(current[name], include_attributes=False)
        for name in core
    )


def test_compact_recovered_afterstate_commitment_matches_v7_authority() -> None:
    base = v8_subject._v8_load_and_validate_v7_authority_base(
        v8_subject.ROOT_DEFAULT, v8_subject._v8_expected_v7_authority_base()
    )
    assert v8_subject._v5_recovered_afterstate_commitment(
        base["recovered_afterstate"]
    ) == v8_subject.V5_RECOVERED_AFTERSTATE_COMMITMENT


def test_full_recovered_afterstate_is_not_embedded_in_v8_authority_controls(
    tmp_path: Path,
) -> None:
    root = tmp_path / "v8-compact"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    for path, cap in ((authorization_path, 32_768), (review_path, 12_000), (go_path, 12_000)):
        payload = v8_subject._v5_load_strict_json(path, label=path.name)
        v8_subject._v8_assert_compact_control(payload, label=path.name)
        encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
        assert '"recovered_afterstate"' not in encoded
        assert '"recovery_progress"' not in encoded
        assert '"inventory"' not in encoded
...
def test_real_v8_authority_and_full_synthetic_core_proofs_are_compositional(
    tmp_path: Path,
) -> None:
    authority_root = tmp_path / "authority"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(
        authority_root
    )
    authority = v8_subject._v8_validate_predata_authority(
        authority_root, authorization_path, go_path
    )
    assert authority["immutable_v2_preflight"]["commitment"]["pytest_run_count"] == 6
    fixture_parent = tmp_path / "synthetic"
    fixture_parent.mkdir()
    root, runner, contract = build_recovered_fixture(fixture_parent)
    report = v8_subject.audit(
        root, contract=contract, runner_path=runner,
        _test_message_decoder=synthetic_decoder,
    )
    assert report["status"] == "PASS_PRODUCER_POSTRUN_READONLY_AUDIT_V8"
    assert report["full_offline_raw_redecode"]["messages"] == contract.range_rows
...
@pytest.mark.parametrize("relative", v8_subject.V8_POSTRUN_REPORT_CANDIDATES)
def test_v2_v3_v4_v5_v6_v7_v8_report_candidate_fails_before_data(
    tmp_path: Path, relative: str
) -> None:
    root = tmp_path / "v8-report-candidate"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    report = root / relative
    report.parent.mkdir(parents=True, exist_ok=True)
    report.write_text("{}\n", encoding="utf-8")
    with pytest.raises(v8_subject.AuditFailure, match="postrun report candidate"):
        v8_subject._v8_validate_predata_authority(root, authorization_path, go_path)
...
def test_v8_authority_schema_path_status_crosslink_or_command_tamper_fails(
    tmp_path: Path, mutation: str
) -> None:
    root = tmp_path / f"v8-authority-tamper-{mutation}"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    authorization = json.loads(authorization_path.read_text(encoding="utf-8"))
    review = json.loads(review_path.read_text(encoding="utf-8"))
    go = json.loads(go_path.read_text(encoding="utf-8"))
    ...
    if mutation == "authorization_extra":
        ...
        review["immutable_v2_preflight_evidence"]["pytest_run_count"] = 5
    elif mutation == "go_status":
        go["status"] = "WRONG"
    elif mutation == "go_crosslink":
        go["v7_live_failure_state"]["failure_reason"] = "WRONG"
    elif mutation == "required_command":
        authorization["required_command"] = ["python", "-m", "wrong.module"]
    elif mutation == "authorization_alias":
        called_authorization = authorization_path.with_name("authorization-alias.json")
        ...
        go["created_utc"] = review["created_utc"]
    if mutation != "authorization_alias":
        write_rebound_v8_chain(
            root, authorization_path, review_path, go_path,
            authorization, review, go,
        )
    with pytest.raises(
        v8_subject.AuditFailure,
        match="schema|header|cross-binding|command|canonical|chronology|mismatch|transport",
    ):
        v8_subject._v8_validate_predata_authority(
            root, called_authorization, go_path
        )


def test_v8_review_and_go_never_embed_full_progress_inventory(
    tmp_path: Path,
) -> None:
    root = tmp_path / "v8-compact-review-go"
    _authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    for path in (review_path, go_path):
        payload = v8_subject._v5_load_strict_json(path, label=path.name)
        v8_subject._v8_assert_compact_control(payload, label=path.name)
        encoded = json.dumps(payload, ensure_ascii=False, sort_keys=True)
...
@pytest.mark.parametrize("role", ("primary", "correction"))
def test_incidents_cannot_authorize_v7_or_v8_execution(
    tmp_path: Path, role: str
) -> None:
    root = tmp_path / f"v8-nonauthority-{role}"
    _authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    primary = v8_subject._v5_load_strict_json(
        root / v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
        label="V8 primary incident nonauthority",
    )
    correction = v8_subject._v5_load_strict_json(
        root / v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE,
        label="V8 correction record nonauthority",
    )
    assert primary["authority_scope"] == {
        "documentary_only": True,
        "v7_retry_authorized": False,
        "v8_authorized": False,
    }
    assert correction["authority_scope"]["documentary_only"] is True
    assert correction["authority_scope"][
        "v8_code_test_control_or_execution_authorized_by_this_record"
    ] is False
    nonauthority = root / (
        v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE
        if role == "primary" else v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE
    )
    with pytest.raises(v8_subject.AuditFailure, match="canonical"):
        v8_subject._v8_validate_predata_authority(root, nonauthority, go_path)
    with pytest.raises(subject.AuditFailure, match="canonical"):
        subject._v7_validate_predata_authority(
            subject.ROOT_DEFAULT,
            v8_subject.ROOT_DEFAULT / v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
            subject.ROOT_DEFAULT / subject.V7_GO_RELATIVE,
        )


def test_v8_zero_snapshot_and_normalized_namespace_commitments_are_exact(
    tmp_path: Path,
) -> None:
    root = tmp_path / "v8-zero-namespace"
    authorization_path, _review_path, go_path, authorization = (
        build_valid_v8_authority(root)
    )
    snapshot_payload = authorization["preaudit_zero_mutation_snapshot"]
    assert set(snapshot_payload) == v8_subject.V8_ZERO_SNAPSHOT_KEYS
    assert snapshot_payload["preauthorization_control_namespace"] == {
        "counts": {"prereg": 5, "independent_redteam": 6, "incidents": 8},
        "canonical_size_bytes": 1_388,
        "canonical_sha256": (
            "c16a239873847da42d138b5cab941c3eb81e5141aaebdabb2695140a3f90347c"
    ...
    failure = v8_subject._v8_validate_duration_suffix_incident_chain(
        root,
        artifact(root / v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE, root),
        artifact(root / v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE, root),
    )
    v8_subject._v8_validate_zero_snapshot(
        root, snapshot_payload, failure_state=failure["state"]
    )
    authority = v8_subject._v8_validate_predata_authority(
        root, authorization_path, go_path
    )
    assert {
        key: len(value) for key, value in authority["control_namespace"].items()
    } == {
        "prereg": 6, "independent_redteam": 8, "incidents": 8,
    }


def test_v8_control_temporary_destination_set_is_exact27() -> None:
    destinations = v8_subject._v8_control_destination_relatives()
    assert len(destinations) == 27
    assert len(set(destinations)) == 27
    assert destinations[-2:] == (
        v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE,
        v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE,
    )
    assert destinations[16:19] == (
        v8_subject.V8_AUTH_RELATIVE,
        v8_subject.V8_REVIEW_RELATIVE,
        v8_subject.V8_GO_RELATIVE,
    )


def test_v8_compact_control_size_caps_and_forbidden_inventory_fields(
    tmp_path: Path,
) -> None:
    root = tmp_path / "v8-size-caps"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    for path, cap in (
        (authorization_path, 32_768), (review_path, 12_000), (go_path, 12_000),
    ):
        payload = v8_subject._v5_load_strict_json(path, label=path.name)
        v8_subject._v8_assert_compact_control(payload, label=path.name)
        assert path.stat().st_size <= cap
        ...
        assert all(
            ...
            for forbidden in ("inventory", "recovered_afterstate", "recovery_progress")
        )


def test_v8_review_and_go_dual_bind_primary_and_correction(
    tmp_path: Path,
) -> None:
    root = tmp_path / "v8-dual-review-go"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    authorization = v8_subject._v5_load_strict_json(
        authorization_path, label="V8 AUTH"
    )
    for path in (review_path, go_path):
        payload = v8_subject._v5_load_strict_json(path, label=path.name)
        assert payload["v7_live_failure_state"] == authorization[
            "v7_live_failure_state"
        ]
        state = payload["v7_live_failure_state"]
        assert state["primary_incident"] == artifact(
            root / v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE, root
        )
        assert state["correction_record"] == artifact(
            root / v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE, root
        )


def test_v8_parser_uses_integer_hundredths_and_canonical_hms_without_float() -> None:
    source = Path(v8_subject.__file__).read_text(encoding="utf-8")
    tree = ast.parse(source)
    parser = next(
        node for node in tree.body
        if isinstance(node, ast.FunctionDef)
        and node.name == "_v8_parse_pytest_success_summary"
    )
    dumped = ast.dump(parser, include_attributes=False)
    assert "float" not in dumped
    assert "fromisoformat" not in dumped
    assert "%f" not in dumped
    parsed = v8_subject._v8_parse_pytest_success_summary(
        "85 passed in 65.33s (0:01:05)", label="integer hundredths"
    )
    assert parsed["duration_hundredths"] == 6_533
    assert parsed["suffix_total_seconds"] == 65


def test_frozen_v7_original_preseal_full_suite_evidence_is_exact() -> None:
    base = v8_subject._v8_load_and_validate_v7_authority_base(
        v8_subject.ROOT_DEFAULT, v8_subject._v8_expected_v7_authority_base()
    )
    inherited = v8_subject._v8_frozen_v7_preseal_test_evidence(base)
    ...
    assert inherited["canonical_size_bytes"] == 14_174
    assert inherited["canonical_sha256"] == (
        "d86248d24b3af7f7b286d1d85ad56a4ccc0a449a0a493495c21193d6b16bf647"
    )
    assert inherited["original_full_runs"]["v7_auditor_tests"]["summary"] == (
        "359 passed, 3 skipped in 153.94s (0:02:33)"
    )
    assert inherited["original_full_runs"]["v7_sealer_tests"]["summary"] == (
        "182 passed, 1 skipped in 5.58s"
    )
...
@pytest.mark.parametrize(
    "mutation",
    (
        "wrong_node", "multiple_nodes", "hidden_k", "wrong_count",
        "wrong_summary", "sealed_summary", "sealed_digest",
    ),
)
def test_frozen_v7_postseal_rerun_deselects_only_three_obsolete_preseal_state_dependent_tests(
    tmp_path: Path, mutation: str
) -> None:
    root = tmp_path / f"v8-deselect-{mutation}"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    authorization = json.loads(authorization_path.read_text(encoding="utf-8"))
    review = json.loads(review_path.read_text(encoding="utf-8"))
    go = json.loads(go_path.read_text(encoding="utf-8"))
    record = authorization["test_evidence"]["test_runs"][
        "frozen_v7_auditor_tests"
    ]
    if mutation == "wrong_node":
        record["deselected_nodeids"] = ["tests/wrong.py::test_wrong"]
    elif mutation == "multiple_nodes":
        record["deselected_nodeids"].append("tests/wrong.py::test_wrong")
        record["deselected_count"] = 2
    elif mutation == "hidden_k":
        record["command"].extend(["-k", "not actual_root"])
    elif mutation == "wrong_count":
        record["deselected_count"] = 0
    elif mutation == "wrong_summary":
        record["summary"] = "359 passed, 3 skipped in 1.00s"
    elif mutation == "sealed_summary":
        authorization["test_evidence"]["frozen_v7_preseal_test_evidence"][
            "original_full_runs"
        ]["v7_sealer_tests"]["summary"] = "181 passed, 1 skipped in 5.58s"
    else:
        authorization["test_evidence"]["frozen_v7_preseal_test_evidence"][
            "canonical_sha256"
        ] = "0" * 64
    ...
    with pytest.raises(
        v8_subject.AuditFailure,
        match="pytest|selection|deselect|preseal test-evidence inheritance",
    ):
        v8_subject._v8_validate_predata_authority(
            root, authorization_path, go_path
        )
...
def build_v8_postauthority_metadata_closure(
    root: Path,
    authority: dict[str, object],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    build_v5_postauthority_metadata_closure(
        root,
        {"v4_base": authority["v7_base"]["v5_base"]["v4_base"]},
        monkeypatch,
    )
    monkeypatch.setattr(v8_subject, "identity", subject.identity)


@pytest.mark.parametrize(
    ("mutation", "expected"),
    (
        ("primary", "duration-suffix incident.*(physical|size|identity)"),
        ("correction", "correction.*(physical|size|identity)"),
        ("preflight", "preflight.*(physical|size|identity)"),
        ("namespace", "namespace inventory mismatch"),
        ("commitment", "afterstate/commitment/temp/report recheck mismatch"),
        ("report", "afterstate/commitment/temp/report recheck mismatch"),
    ),
)
def test_v8_postaudit_rechecks_v7_chain_failure_correction_preflight_namespace_and_commitment(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    mutation: str,
    expected: str,
) -> None:
    root = tmp_path / f"v8-postaudit-{mutation}"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    authority = v8_subject._v8_validate_predata_authority(
        root, authorization_path, go_path
    )
    build_v8_postauthority_metadata_closure(root, authority, monkeypatch)
    v8_subject._v8_validate_postauthority_afterstate(root, authority)
    if mutation == "primary":
        path = root / v8_subject.V8_DURATION_SUFFIX_INCIDENT_RELATIVE
        with path.open("ab") as stream:
            stream.write(b" ")
    elif mutation == "correction":
        path = root / v8_subject.V8_DURATION_SUFFIX_CORRECTION_RELATIVE
        with path.open("ab") as stream:
            stream.write(b" ")
    elif mutation == "preflight":
        path = root / v8_subject.V8_PREFLIGHT_RELATIVE
        with path.open("ab") as stream:
            stream.write(b" ")
    elif mutation == "namespace":
        extra = (
            root / "incidents"
            / ".TRACK_A_DECODE_RECOVERY_POSTRUN_AUDITOR_UNBOUND_V8.json"
        )
        extra.write_text("{}\n", encoding="utf-8")
    elif mutation == "commitment":
        authority["recovered_afterstate_commitment"] = {
        ...
        report = root / v8_subject.V8_POSTRUN_REPORT_CANDIDATES[-1]
        report.parent.mkdir(parents=True, exist_ok=True)
        report.write_text("{}\n", encoding="utf-8")
    with pytest.raises(v8_subject.AuditFailure, match=expected):
        v8_subject._v8_validate_postauthority_afterstate(root, authority)


def test_v8_auditor_is_stdout_only_and_writes_zero_files(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
) -> None:
    root = tmp_path / "v8-main-stdout-only"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    review_path.unlink()
    before = snapshot(root)
    original_open = Path.open
    ...
    def deny_write_open(
        ...
        assert not any(flag in mode for flag in ("w", "a", "x", "+")), (
            f"V8 main attempted a filesystem write: {path} {mode}"
        )
        return original_open(path, mode, *args, **kwargs)

    monkeypatch.setattr(Path, "open", deny_write_open)
    monkeypatch.setattr(
        v8_subject, "_v8_require_canonical_runtime_entrypoint", lambda *_args: None
    )
    rc = v8_subject.main(
        [
            "--root", str(root), "--authorization", str(authorization_path),
    ...
    assert rc == 1
    assert captured.err == ""
    assert payload["artifact_type"] == "TRACK_A_RAW_PRODUCER_POSTRUN_READONLY_AUDIT_V8"
    assert payload["status"] == "FAIL_PRODUCER_POSTRUN_AUDIT_V8"
    assert payload["audit_network_requests"] == 0
    assert payload["audit_files_written"] == 0
    assert snapshot(root) == before
    assert v8_subject._v8_postrun_report_candidates(root) == []
    source_tree = ast.parse(Path(v8_subject.__file__).read_text(encoding="utf-8"))
    forbidden_writers = {
        "write_text", "write_bytes", "unlink", "rename", "mkdir",
        "touch", "symlink_to", "hardlink_to",
    }
...
def test_v8_worker_is_importable_in_exactly_seven_spawn_processes() -> None:
    pilot = (
        v8_subject.REPO
        / "artifacts" / "baram2026_ncei_scada_research_20260810_210756"
        / "track_a" / "provenance" / "raw" / "f039"
    ...
    coordinate_path = (
        v8_subject.ROOT_DEFAULT
        / "prereg" / "authoritative_turbine_coordinate_lock_v1.json"
    )
    assert pilot.stat().st_size == 1_455_839
    assert v8_subject.sha256_file(pilot) == (
        "575fe7876ab93e5470c43dc09c78895385925f5662cca75bd0d8c7f91e677407"
    )
    coordinate = json.loads(coordinate_path.read_text(encoding="utf-8"))
    runtime_identity = json.loads(
        (v8_subject.ROOT_DEFAULT / "prereg" / "raw_launch_authorization_v1.json")
        .read_text(encoding="utf-8")
    )["runtime_identity"]
    sites = coordinate["sites"]
    info = {
    ...
    expected = v8_subject.decode_offline_grib_message(info, sites)["site_values"]
    context = multiprocessing.get_context("spawn")
    barrier = context.Barrier(v8_subject.DECODE_MAX_WORKERS)
    with concurrent.futures.ProcessPoolExecutor(
        max_workers=v8_subject.DECODE_MAX_WORKERS,
        mp_context=context,
        initializer=v8_subject.v8_spawn_probe_barrier_initializer,
        initargs=(barrier,),
    ) as pool:
        futures = [
            pool.submit(v8_subject.decode_offline_grib_process_worker, info, sites)
            for _index in range(v8_subject.DECODE_MAX_WORKERS)
        ]
        decoded = [future.result(timeout=180) for future in futures]
    assert len({int(report["decoder_process_id"]) for report in decoded}) == 7
    assert {report["decoder_runtime_identity_sha256"] for report in decoded} == {
        v8_subject.canonical_payload_sha256(runtime_identity)
    }
    assert all(
        report["site_values"] == pytest.approx(expected, abs=1e-12, rel=0.0)
        for report in decoded
...
def test_v8_actual_root_collect_build_and_self_validator_passes() -> None:
    workspace = v8_subject.REPO
    root = v8_subject.ROOT_DEFAULT
    code_state = v8_sealer.collect_code_state(workspace)
    inputs = v8_sealer.collect_root_inputs(root, code_state)
    bound = v8_sealer._evidence_bound_identities(
        root, code_state["bound_identities"]
    )
    created_utc = "2026-08-11T07:30:00.000000Z"
    evidence = synthetic_v8_test_evidence(created_utc, bound)
    payload = v8_sealer.build_authorization(
        root, code_state, inputs, evidence, created_utc=created_utc
    )
    v8_sealer.validate_authorization_payload(root, payload, code_state, inputs)
    assert payload["schema_version"] == 8
    assert payload["immutable_v2_preflight_evidence"]["pytest_run_count"] == 6
    assert payload["v7_live_failure_state"]["primary_incident_alone_must_fail"] is True


@pytest.mark.parametrize("failure", ("authorization", "primary", "correction", "preflight"))
def test_v8_authority_and_failure_preflight_gate_precede_any_parquet_or_data_read(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, failure: str
) -> None:
    root = tmp_path / f"v8-order-{failure}"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    ...
    def forbidden(name: str):
        def fail(*_args: object, **_kwargs: object) -> object:
            calls.append(name)
            raise AssertionError(f"data route reached before V8 authority gate: {name}")
        return fail
    ...
    for name in (
        ...
        monkeypatch.setattr(v8_subject, name, forbidden(name))
    monkeypatch.setattr(v8_subject, "_V8_CANONICAL_MAIN_ACTIVE", True)
    with pytest.raises(v8_subject.AuditFailure):
        v8_subject.audit(
            root,
            contract=v8_subject.PRODUCTION_CONTRACT,
            runner_path=v8_subject.RUNNER_DEFAULT,
            authorization_path=authorization_path,
            independent_go_path=go_path,
        )
    assert calls == []
    audit_node = next(
        node for node in ast.parse(
            Path(v8_subject.__file__).read_text(encoding="utf-8")
        ).body
        if isinstance(node, ast.FunctionDef) and node.name == "audit"
    )
    call_lines = {
        ...
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Name)
        and node.func.id in {
            "_v8_validate_predata_authority", "preflight_zero_target_parquet_schemas"
        }
    }
    assert call_lines["_v8_validate_predata_authority"] < (
        call_lines["preflight_zero_target_parquet_schemas"]
    )
...
@pytest.mark.parametrize(
    "mutation",
    (
        "primary_equal_v7_go", "primary_equal_correction", "correction_backdated",
        "evidence_backdated", "review_equal_authorization", "go_equal_review",
    ),
)
def test_v8_primary_correction_chronology_equality_or_backdate_fails(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, mutation: str
) -> None:
    root = tmp_path / f"v8-chronology-{mutation}"
    authorization_path, review_path, go_path, _ = build_valid_v8_authority(root)
    authorization = json.loads(authorization_path.read_text(encoding="utf-8"))
    review = json.loads(review_path.read_text(encoding="utf-8"))
    go = json.loads(go_path.read_text(encoding="utf-8"))
    if mutation in {
        ...
        original = v8_subject._v8_validate_duration_suffix_incident_chain
        v7_go_ticks = v8_subject._v8_load_and_validate_v7_authority_base(
        ...
        def altered(*args: object, **kwargs: object) -> dict[str, object]:
            ...
            if mutation == "primary_equal_v7_go":
                result["primary_created"]["ticks_100ns"] = v7_go_ticks
            elif mutation == "primary_equal_correction":
                result["correction_created"] = result["primary_created"]
            else:
                result["correction_created"]["ticks_100ns"] = (
                    result["primary_created"]["ticks_100ns"] - 1
                )
            return result

        monkeypatch.setattr(
            v8_subject, "_v8_validate_duration_suffix_incident_chain", altered
        )
    elif mutation == "evidence_backdated":
        authorization["test_evidence"]["created_utc"] = (
            "2026-08-11T06:43:59.084808Z"
        )
    elif mutation == "review_equal_authorization":
        review["created_utc"] = authorization["created_utc"]
    else:
        go["created_utc"] = review["created_utc"]
    write_rebound_v8_chain(
    ...
    with pytest.raises(v8_subject.AuditFailure, match="chronology"):
        v8_subject._v8_validate_predata_authority(root, authorization_path, go_path)


def test_v8_postauthority_start_end_call_count_and_midrun_drift_fail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    root = tmp_path / "v8-start-end-drift"
    authorization_path, _review_path, go_path, _ = build_valid_v8_authority(root)
    authority = v8_subject._v8_validate_predata_authority(
        root, authorization_path, go_path
    )
    build_v8_postauthority_metadata_closure(root, authority, monkeypatch)
    runner = tmp_path / "runner-v8.py"
    runner.write_text("# V8 postauthority start/end sentinel\n", encoding="utf-8")
    original_poststate = v8_subject._v8_validate_postauthority_afterstate
    poststate_calls: list[int] = []
    ...
    monkeypatch.setattr(
        v8_subject, "_v8_validate_postauthority_afterstate", counted_poststate
    )
    monkeypatch.setattr(
        v8_subject, "_v8_validate_predata_authority", lambda *_args: authority
    )
    monkeypatch.setattr(v8_subject, "_V8_CANONICAL_MAIN_ACTIVE", True)
    monkeypatch.setattr(
        v8_subject, "require_no_symlink_chain", lambda *_args, **_kwargs: None
    )
    monkeypatch.setattr(v8_subject, "_audit_active_lock", lambda *_args: None)
    monkeypatch.setattr(
        v8_subject, "_audit_recovery_control_temporary_files_absent",
        lambda *_args: {"matching_temporary_files": 0},
    )
    monkeypatch.setattr(
        v8_subject, "preflight_zero_target_parquet_schemas", lambda *_args: {}
    )
    monkeypatch.setattr(
        v8_subject, "_audit_provenance",
        lambda *_args: {
            "field_census_path": tmp_path / "unused-census.json", "coordinate": {},
            "runtime_identity_sha256": v8_subject.V4_RUNTIME_IDENTITY_SHA256,
            "authorization_identity": {}, "go_identity": {},
            "preflight_identity": {}, "preflight_amendment_identity": {},
            "independent_prelaunch_identity": {},
    ...
    monkeypatch.setattr(
        v8_subject, "_audit_transaction_and_launch",
        lambda *_args: {
            "outputs": {"raw_parquet": None, "raw_csv": None},
            "attempt_id": "synthetic", "complete_lock_identity": {},
            "launch_history_lock_count": 1, "plan_identity": {},
            "commit_identity": {}, "transaction_inventory_recursively_closed": True,
            "planned_staged_files_present": 0,
        },
    )
    monkeypatch.setattr(v8_subject, "_audit_census", lambda *_args: (None, []))
    monkeypatch.setattr(
        v8_subject, "_audit_raw_ranges",
        lambda *_args: {"raw_info": [], "raw_rows": 0, "raw_bytes": 0},
    )
    monkeypatch.setattr(
        v8_subject, "_audit_request_events",
        lambda *_args: {
            "starts": 0, "completions": 0, "errors": 0,
            "indeterminate_starts": 0, "max_global_attempt_number": 0,
        },
    )
    monkeypatch.setattr(v8_subject, "_audit_decoded", lambda *_args: {})
    monkeypatch.setattr(
        v8_subject, "_audit_full_offline_redecode", lambda *_args, **_kwargs: {}
    )

    def drift_between_start_and_end(*_args: object) -> dict[str, object]:
        authority["failure_chain"]["state"]["failure_reason"] = "MIDRUN_DRIFT"
        return {
            "external_identity_unique_file_count": 0,
            "external_identity_unique_file_bytes": 0,
        }
    ...
    monkeypatch.setattr(
        v8_subject, "_audit_canonical_metadata", drift_between_start_and_end
    )
    with pytest.raises(v8_subject.AuditFailure, match="chain changed during audit"):
        v8_subject.audit(
            root,
            contract=v8_subject.PRODUCTION_CONTRACT,
            runner_path=runner,
    ...
    assert called_names.count("_v8_validate_predata_authority") == 1
    assert called_names.count("_v8_validate_postauthority_afterstate") == 2
