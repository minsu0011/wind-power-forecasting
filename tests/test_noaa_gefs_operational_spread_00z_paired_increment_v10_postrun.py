...
SOURCE = ROOT / "artifacts/external/noaa_gefs_operational_spread_00z_original_v10/stage1_source_through_operating_2023"
STAGE2_SOURCE = ROOT / "artifacts/external/noaa_gefs_operational_spread_00z_original_v10/stage2_source_operating_2024"
MODEL_OUTPUT = ROOT / "artifacts/postgate/noaa_gefs_operational_spread_00z_paired_increment_strict_v10"
SOURCE_LAUNCH_LOCK = ROOT / "artifacts/audits/noaa_gefs_operational_spread_00z_original_v10_stage1_launch_lock.json"
...
V10_REVIEW = ROOT / "artifacts/audits/noaa_gefs_operational_spread_00z_original_v10_prelaunch_independent_review_v1.json"
REGRESSION = ROOT / "artifacts/audits/noaa_gefs_operational_spread_00z_v10_decoder_gate_regression_v1.json"
...
def test_sealed_v7_preflight_fixture_is_immutable() -> None:
...
def test_stage1_source_manifest_and_artifact_closure_if_present() -> None:
    if not SOURCE.exists():
        pytest.skip("v10 full Stage1 source has not run")
    manifest = json.loads((SOURCE / "manifest.json").read_text(encoding="utf-8"))
    summary = json.loads((SOURCE / "extraction_summary.json").read_text(encoding="utf-8"))
    assert manifest["status"] == summary["status"] == "PASS_V10_STAGE1_SOURCE_EXTRACTION"
    assert manifest["nonmutation"] == {"stage2_namespace_created": False, "label_model_metric_csv_writes": 0, "old_v2_partial_reused": 0, "terminal_v7_partial_reused": 0}
    ...
    assert summary["request_accounting"]["terminal_v7_partial_reused"] == 0
    ...
    assert summary["decoder_gate"] == {"ready": True, "failure": None, "first_success_count": 1}
...
def test_full_launch_lock_requires_distinct_post_preflight_review_if_present() -> None:
    if not SOURCE_LAUNCH_LOCK.exists():
        pytest.skip("v10 full launch lock has not been frozen")
    assert POST_PREFLIGHT_REVIEW.is_file()
    assert V10_REVIEW.is_file()
    ...
    assert lock["independent_review"]["sha256"] == sha256(V10_REVIEW)
    assert lock["offline_decoder_gate_regression"]["sha256"] == sha256(REGRESSION)
    assert lock["scope"]["workers"] == 24 and lock["scope"]["retries"] == 5
